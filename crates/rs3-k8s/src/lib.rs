//! Kubernetes checkpoint anchoring integration.

#[cfg(test)]
mod anchor_tests;
mod bootstrap_journal;
mod lease_guard;
#[cfg(test)]
mod test_support;

use async_trait::async_trait;
use k8s_openapi::api::coordination::v1::Lease;
use k8s_openapi::apimachinery::pkg::apis::meta::v1::ObjectMeta;
use rs3_repository::v3::{
    V3AnchorState, V3CommitAnchor, V3FormatError, V3FormatRef, V3MaintenanceGuard, V3Result,
};
use rs3_types::{BackendObjectId, BackendVersionId, KeyId, Sequence};
use std::collections::BTreeMap;

pub use bootstrap_journal::{
    BootstrapJournalError, KubernetesBootstrapJournal, MAX_BOOTSTRAP_JOURNAL_BYTES,
};

pub use lease_guard::{
    KubernetesLeaseGuard, LeaseGuard, LeaseGuardApi, LeaseGuardError, LeaseGuardState, WriterFence,
};
use lease_guard::{
    KubernetesLeaseGuardApi, WriterFenceClaim, lease_has_writer_coordination, lease_holds_claim,
};

const REPOSITORY_FORMAT_ANNOTATION: &str = "rs3.rs/repository-format-generation";
const ANCHOR_FENCE_ANNOTATION: &str = "rs3.rs/anchor-fence";
const V3_SEQUENCE_ANNOTATION: &str = "rs3.rs/v3-sequence";
const V3_COMMIT_KEY_ANNOTATION: &str = "rs3.rs/v3-commit-key";
const V3_BODY_DIGEST_ANNOTATION: &str = "rs3.rs/v3-body-digest";
const V3_VERSION_ID_ANNOTATION: &str = "rs3.rs/v3-version-id";
const V3_SIGNING_KEY_ID_ANNOTATION: &str = "rs3.rs/v3-signing-key-id";
const V3_FORMAT_GENERATION_ANNOTATION: &str = "rs3.rs/v3-format-generation";
const V3_FORMAT_DIGEST_ANNOTATION: &str = "rs3.rs/v3-format-digest";
const V3_FORMAT_OBJECT_ID_ANNOTATION: &str = "rs3.rs/v3-format-object-id";
const V3_FORMAT_VERSION_ID_ANNOTATION: &str = "rs3.rs/v3-format-version-id";
const MAX_ADVANCE_ATTEMPTS: usize = 16;

/// Kubernetes object settings used for checkpoint anchoring.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct LeaseSettings {
    /// Namespace that stores the anchor object.
    pub namespace: String,
    /// Name of the anchor object.
    pub name: String,
    /// Field manager used by server-side apply compatible deployments.
    pub field_manager: String,
}

/// Checkpoint anchor stored in a Kubernetes Lease object.
///
/// The anchor state is encoded in Lease annotations and updated with Kubernetes
/// resource-version compare-and-swap semantics. This makes concurrent writers
/// retry on update conflicts instead of silently overwriting each other.
pub struct KubernetesLeaseAnchor {
    settings: LeaseSettings,
    writer_fence: Option<WriterFence>,
    api: KubernetesLeaseGuardApi,
}

impl KubernetesLeaseAnchor {
    /// Creates a Lease-backed checkpoint anchor.
    pub fn new(settings: LeaseSettings) -> Self {
        Self {
            settings,
            writer_fence: None,
            api: KubernetesLeaseGuardApi::default(),
        }
    }

    /// Creates an anchor whose updates must prove ownership of the live writer
    /// epoch stored on this same Lease object.
    pub fn new_fenced(settings: LeaseSettings, writer_fence: WriterFence) -> Self {
        Self {
            settings,
            writer_fence: Some(writer_fence),
            api: KubernetesLeaseGuardApi::default(),
        }
    }
}

#[async_trait]
impl V3MaintenanceGuard for WriterFence {
    async fn verify_v3_maintenance(&self, _base_anchor: Option<&V3AnchorState>) -> V3Result<()> {
        if self.is_live() {
            Ok(())
        } else {
            Err(V3FormatError::MaintenanceAccessRequired)
        }
    }
}

#[async_trait]
impl V3CommitAnchor for KubernetesLeaseAnchor {
    async fn read_v3(&self) -> V3Result<Option<V3AnchorState>> {
        let lease = match self
            .api
            .get_lease(&self.settings.namespace, &self.settings.name)
            .await
        {
            Ok(Some(lease)) => lease,
            Ok(None) => return Ok(None),
            Err(_) => return Err(V3FormatError::AnchorReadFailed),
        };
        v3_anchor_state_from_lease_optional(&lease)
    }

    async fn compare_and_advance_v3(
        &self,
        expected: Option<&V3AnchorState>,
        next: V3AnchorState,
    ) -> V3Result<V3AnchorState> {
        compare_and_advance_lease(
            &self.api,
            &self.settings,
            self.writer_fence.as_ref(),
            expected,
            next,
        )
        .await
    }

    async fn fence_and_read_v3(&self) -> V3Result<Option<V3AnchorState>> {
        fence_and_read_lease(&self.api, &self.settings, self.writer_fence.as_ref()).await
    }
}

/// Performs a resource-version-guarded no-op update on the Lease, then returns
/// the anchor state that update carried.
///
/// Every Lease update is a replace against the resource version the writer
/// read. Once this write lands, any earlier in-flight update still carrying an
/// older resource version is refused by the API server, so the returned state
/// is the final answer for an advance whose reply was lost. The fencing
/// annotation is a counter that never affects anchor decoding.
async fn fence_and_read_lease(
    api: &impl LeaseGuardApi,
    settings: &LeaseSettings,
    writer_fence: Option<&WriterFence>,
) -> V3Result<Option<V3AnchorState>> {
    for _attempt in 0..MAX_ADVANCE_ATTEMPTS {
        match api.get_lease(&settings.namespace, &settings.name).await {
            Ok(Some(mut lease)) => {
                validate_writer_fence(&lease, writer_fence)?;
                let annotations = lease.metadata.annotations.get_or_insert_with(BTreeMap::new);
                let next = annotations
                    .get(ANCHOR_FENCE_ANNOTATION)
                    .and_then(|value| value.parse::<u64>().ok())
                    .unwrap_or(0)
                    .wrapping_add(1);
                annotations.insert(ANCHOR_FENCE_ANNOTATION.to_owned(), next.to_string());
                match api
                    .replace_lease(&settings.namespace, &settings.name, &lease)
                    .await
                {
                    Ok(lease) => return v3_anchor_state_from_lease_optional(&lease),
                    Err(LeaseGuardError::Conflict) => continue,
                    Err(_) => return Err(V3FormatError::AnchorAdvanceFailed),
                }
            }
            // Without a Lease there is nothing an in-flight advance could
            // have applied against, and a fenced writer never creates one.
            Ok(None) => return Ok(None),
            Err(_) => return Err(V3FormatError::AnchorReadFailed),
        }
    }
    Err(V3FormatError::AnchorAdvanceFailed)
}

async fn compare_and_advance_lease(
    api: &impl LeaseGuardApi,
    settings: &LeaseSettings,
    writer_fence: Option<&WriterFence>,
    expected: Option<&V3AnchorState>,
    next: V3AnchorState,
) -> V3Result<V3AnchorState> {
    for _attempt in 0..MAX_ADVANCE_ATTEMPTS {
        match api.get_lease(&settings.namespace, &settings.name).await {
            Ok(Some(lease)) => {
                let current = v3_anchor_state_from_lease_optional(&lease)?;
                if expected != current.as_ref() {
                    return Err(V3FormatError::StaleAnchor);
                }
                if current
                    .as_ref()
                    .is_some_and(|current| next != *current && next.sequence <= current.sequence)
                {
                    return Err(V3FormatError::StaleAnchor);
                }

                let updated = v3_lease_for_anchor_advance(lease, &next, writer_fence)?;
                match api
                    .replace_lease(&settings.namespace, &settings.name, &updated)
                    .await
                {
                    Ok(lease) => return v3_anchor_state_from_lease(&lease),
                    Err(LeaseGuardError::Conflict) => continue,
                    Err(_) => return Err(V3FormatError::AnchorAdvanceFailed),
                }
            }
            Ok(None) => {
                if expected.is_some() {
                    return Err(V3FormatError::StaleAnchor);
                }
                if writer_fence.is_some() {
                    return Err(V3FormatError::AnchorAdvanceFailed);
                }
                let lease = new_v3_lease(&settings.name, &next);
                match api.create_lease(&settings.namespace, &lease).await {
                    Ok(lease) => return v3_anchor_state_from_lease(&lease),
                    Err(LeaseGuardError::Conflict) => continue,
                    Err(_) => return Err(V3FormatError::AnchorAdvanceFailed),
                }
            }
            Err(_) => return Err(V3FormatError::AnchorReadFailed),
        }
    }

    Err(V3FormatError::AnchorAdvanceFailed)
}

fn new_v3_lease(name: &str, state: &V3AnchorState) -> Lease {
    v3_lease_with_state(
        Lease {
            metadata: ObjectMeta {
                name: Some(name.to_owned()),
                ..ObjectMeta::default()
            },
            spec: None,
        },
        state,
    )
}

fn v3_lease_with_fenced_state(
    lease: Lease,
    state: &V3AnchorState,
    fence: &WriterFence,
) -> V3Result<Lease> {
    let claim = fence
        .live_claim()
        .ok_or(V3FormatError::AnchorAdvanceFailed)?;
    v3_lease_with_claim(lease, state, &claim)
}

fn v3_lease_for_anchor_advance(
    lease: Lease,
    state: &V3AnchorState,
    writer_fence: Option<&WriterFence>,
) -> V3Result<Lease> {
    validate_writer_fence(&lease, writer_fence)?;
    match writer_fence {
        Some(fence) => v3_lease_with_fenced_state(lease, state, fence),
        None => Ok(v3_lease_with_state(lease, state)),
    }
}

fn validate_writer_fence(lease: &Lease, writer_fence: Option<&WriterFence>) -> V3Result<()> {
    match writer_fence {
        Some(fence) => {
            let claim = fence
                .live_claim()
                .ok_or(V3FormatError::AnchorAdvanceFailed)?;
            if lease_holds_claim(lease, &claim).map_err(|_| V3FormatError::AnchorAdvanceFailed)? {
                Ok(())
            } else {
                Err(V3FormatError::AnchorAdvanceFailed)
            }
        }
        None if lease_has_writer_coordination(lease) => Err(V3FormatError::AnchorAdvanceFailed),
        None => Ok(()),
    }
}

fn v3_lease_with_claim(
    lease: Lease,
    state: &V3AnchorState,
    claim: &WriterFenceClaim,
) -> V3Result<Lease> {
    if !lease_holds_claim(&lease, claim).map_err(|_| V3FormatError::AnchorAdvanceFailed)? {
        return Err(V3FormatError::AnchorAdvanceFailed);
    }
    Ok(v3_lease_with_state(lease, state))
}

fn v3_lease_with_state(mut lease: Lease, state: &V3AnchorState) -> Lease {
    let annotations = lease.metadata.annotations.get_or_insert_with(BTreeMap::new);
    annotations.insert(
        REPOSITORY_FORMAT_ANNOTATION.to_owned(),
        rs3_repository::v3::V3_FORMAT_VERSION.to_string(),
    );
    annotations.insert(
        V3_SEQUENCE_ANNOTATION.to_owned(),
        state.sequence.get().to_string(),
    );
    annotations.insert(
        V3_COMMIT_KEY_ANNOTATION.to_owned(),
        state.commit_key.as_str().to_owned(),
    );
    annotations.insert(
        V3_BODY_DIGEST_ANNOTATION.to_owned(),
        hex::encode(state.body_digest),
    );
    match state.version_id.as_ref() {
        Some(version_id) => {
            annotations.insert(
                V3_VERSION_ID_ANNOTATION.to_owned(),
                version_id.as_str().to_owned(),
            );
        }
        None => {
            annotations.remove(V3_VERSION_ID_ANNOTATION);
        }
    }
    annotations.insert(
        V3_SIGNING_KEY_ID_ANNOTATION.to_owned(),
        state.signing_key_id.as_str().to_owned(),
    );
    annotations.insert(
        V3_FORMAT_GENERATION_ANNOTATION.to_owned(),
        state.format_ref.generation.to_string(),
    );
    annotations.insert(
        V3_FORMAT_DIGEST_ANNOTATION.to_owned(),
        state.format_ref.digest.clone(),
    );
    annotations.insert(
        V3_FORMAT_OBJECT_ID_ANNOTATION.to_owned(),
        state.format_ref.object_id.as_str().to_owned(),
    );
    match state.format_ref.version_id.as_ref() {
        Some(version_id) => {
            annotations.insert(
                V3_FORMAT_VERSION_ID_ANNOTATION.to_owned(),
                version_id.as_str().to_owned(),
            );
        }
        None => {
            annotations.remove(V3_FORMAT_VERSION_ID_ANNOTATION);
        }
    }

    lease
}

fn v3_anchor_state_from_lease_optional(lease: &Lease) -> V3Result<Option<V3AnchorState>> {
    let Some(annotations) = lease.metadata.annotations.as_ref() else {
        return Ok(None);
    };
    if annotations.keys().any(|key| key.starts_with("rs3.rs/v2-")) {
        return Err(V3FormatError::AnchorReadFailed);
    }
    const ANCHOR_ANNOTATIONS: &[&str] = &[
        REPOSITORY_FORMAT_ANNOTATION,
        V3_SEQUENCE_ANNOTATION,
        V3_COMMIT_KEY_ANNOTATION,
        V3_BODY_DIGEST_ANNOTATION,
        V3_VERSION_ID_ANNOTATION,
        V3_SIGNING_KEY_ID_ANNOTATION,
        V3_FORMAT_GENERATION_ANNOTATION,
        V3_FORMAT_DIGEST_ANNOTATION,
        V3_FORMAT_OBJECT_ID_ANNOTATION,
        V3_FORMAT_VERSION_ID_ANNOTATION,
    ];
    if !ANCHOR_ANNOTATIONS
        .iter()
        .any(|key| annotations.contains_key(*key))
    {
        return Ok(None);
    }
    v3_anchor_state_from_lease(lease).map(Some)
}

fn v3_anchor_state_from_lease(lease: &Lease) -> V3Result<V3AnchorState> {
    let annotations = lease
        .metadata
        .annotations
        .as_ref()
        .ok_or(V3FormatError::AnchorReadFailed)?;
    if v3_annotation(annotations, REPOSITORY_FORMAT_ANNOTATION)? != "3"
        || annotations.keys().any(|key| key.starts_with("rs3.rs/v2-"))
    {
        return Err(V3FormatError::AnchorReadFailed);
    }
    let sequence = v3_annotation(annotations, V3_SEQUENCE_ANNOTATION)?
        .parse::<u64>()
        .map_err(|_| V3FormatError::AnchorReadFailed)?;
    let commit_key =
        BackendObjectId::new(v3_annotation(annotations, V3_COMMIT_KEY_ANNOTATION)?.to_owned())?;
    let body_digest = hex_digest(v3_annotation(annotations, V3_BODY_DIGEST_ANNOTATION)?)?;
    let version_id = annotations
        .get(V3_VERSION_ID_ANNOTATION)
        .map(|value| BackendVersionId::new(value.to_owned()))
        .transpose()?;
    let signing_key_id =
        KeyId::new(v3_annotation(annotations, V3_SIGNING_KEY_ID_ANNOTATION)?.to_owned())?;
    let format_generation = v3_annotation(annotations, V3_FORMAT_GENERATION_ANNOTATION)?
        .parse::<u64>()
        .map_err(|_| V3FormatError::AnchorReadFailed)?;
    let format_digest = v3_annotation(annotations, V3_FORMAT_DIGEST_ANNOTATION)?.to_owned();
    let format_object_id = BackendObjectId::new(
        v3_annotation(annotations, V3_FORMAT_OBJECT_ID_ANNOTATION)?.to_owned(),
    )?;
    let format_version_id = annotations
        .get(V3_FORMAT_VERSION_ID_ANNOTATION)
        .map(|value| BackendVersionId::new(value.to_owned()))
        .transpose()?;

    if format_digest.is_empty() {
        return Err(V3FormatError::AnchorReadFailed);
    }

    Ok(V3AnchorState {
        sequence: Sequence::new(sequence),
        commit_key,
        body_digest,
        version_id,
        signing_key_id,
        format_ref: V3FormatRef {
            generation: format_generation,
            digest: format_digest,
            object_id: format_object_id,
            version_id: format_version_id,
        },
    })
}

fn v3_annotation<'a>(
    annotations: &'a BTreeMap<String, String>,
    key: &'static str,
) -> V3Result<&'a str> {
    annotations
        .get(key)
        .map(String::as_str)
        .ok_or(V3FormatError::AnchorReadFailed)
}

fn hex_digest(value: &str) -> V3Result<[u8; 32]> {
    let bytes = hex::decode(value).map_err(|_| V3FormatError::AnchorReadFailed)?;
    bytes
        .try_into()
        .map_err(|_| V3FormatError::AnchorReadFailed)
}

fn is_kube_status(error: &kube::Error, code: u16) -> bool {
    matches!(error, kube::Error::Api(api_error) if api_error.code == code)
}

#[cfg(test)]
mod tests {
    use super::{
        V3_FORMAT_VERSION_ID_ANNOTATION, WriterFenceClaim, lease_has_writer_coordination,
        v3_anchor_state_from_lease, v3_anchor_state_from_lease_optional,
        v3_lease_for_anchor_advance, v3_lease_with_claim, v3_lease_with_state,
    };
    use crate::lease_guard::lease_with_guard_state;
    use k8s_openapi::api::coordination::v1::Lease;
    use k8s_openapi::jiff::Timestamp;
    use rs3_repository::v3::{V3AnchorState, V3FormatError, V3FormatRef};
    use rs3_types::{BackendObjectId, BackendVersionId, KeyId, Sequence};

    pub(super) fn v3_state(sequence: u64) -> V3AnchorState {
        V3AnchorState {
            sequence: Sequence::new(sequence),
            commit_key: BackendObjectId::new(format!(
                "commits/v03/{sequence:020}/AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
            ))
            .expect("valid backend object id"),
            body_digest: [sequence as u8; 32],
            version_id: Some(
                BackendVersionId::new(format!("commit-version-{sequence}"))
                    .expect("valid version id"),
            ),
            signing_key_id: KeyId::new("signing").expect("valid key id"),
            format_ref: V3FormatRef {
                generation: 1,
                digest: hex::encode([7_u8; 32]),
                object_id: BackendObjectId::new(format!(
                    "format/{:020}-{}",
                    1_u64,
                    hex::encode([7_u8; 32])
                ))
                .expect("valid backend object id"),
                version_id: Some(
                    BackendVersionId::new("format-version-1").expect("valid version id"),
                ),
            },
        }
    }

    #[test]
    fn lease_annotations_round_trip_v3_anchor_state() {
        let expected = v3_state(7);
        let lease = v3_lease_with_state(Lease::default(), &expected);

        let actual = v3_anchor_state_from_lease(&lease);

        assert_eq!(actual.ok(), Some(expected));
    }

    #[test]
    fn lease_annotations_round_trip_v3_format_version() {
        let expected = v3_state(8);
        let lease = v3_lease_with_state(Lease::default(), &expected);

        assert_eq!(
            lease
                .metadata
                .annotations
                .as_ref()
                .and_then(|annotations| annotations.get(V3_FORMAT_VERSION_ID_ANNOTATION))
                .map(String::as_str),
            Some("format-version-1")
        );
        assert_eq!(v3_anchor_state_from_lease(&lease).ok(), Some(expected));
    }

    #[test]
    fn v3_anchor_state_fails_closed_when_annotations_are_missing() {
        let mut lease = v3_lease_with_state(Lease::default(), &v3_state(1));
        lease
            .metadata
            .annotations
            .as_mut()
            .expect("annotations exist")
            .remove(super::V3_FORMAT_DIGEST_ANNOTATION);

        let actual = v3_anchor_state_from_lease(&lease);

        assert!(matches!(actual, Err(V3FormatError::AnchorReadFailed)));
    }

    #[test]
    fn anchors_require_current_repository_generation_and_reject_old_annotations() {
        let state = v3_state(7);
        let lease = super::new_v3_lease("anchor", &state);
        for generation in [None, Some("2"), Some("4"), Some("03")] {
            let mut changed = lease.clone();
            let annotations = changed.metadata.annotations.as_mut().expect("annotations");
            match generation {
                None => {
                    annotations.remove(super::REPOSITORY_FORMAT_ANNOTATION);
                }
                Some(value) => {
                    annotations.insert(
                        super::REPOSITORY_FORMAT_ANNOTATION.to_owned(),
                        value.to_owned(),
                    );
                }
            }
            assert!(super::v3_anchor_state_from_lease_optional(&changed).is_err());
        }
        let mut old = Lease::default();
        old.metadata.annotations = Some(std::collections::BTreeMap::from([(
            "rs3.rs/v2-sequence".to_owned(),
            "7".to_owned(),
        )]));
        assert!(
            super::v3_anchor_state_from_lease_optional(&old).is_err(),
            "old authority is not absent authority"
        );
        let mut mixed = lease;
        mixed
            .metadata
            .annotations
            .as_mut()
            .expect("annotations")
            .insert("rs3.rs/v2-sequence".to_owned(), "7".to_owned());
        assert!(super::v3_anchor_state_from_lease_optional(&mixed).is_err());
    }

    #[test]
    fn coordination_only_lease_is_an_empty_anchor() {
        let lease = lease_with_guard_state(
            Lease::default(),
            &WriterFenceClaim {
                holder_identity: "pod-a/process-1".to_owned(),
                token: 3,
            },
            9,
            std::time::Duration::from_secs(30),
            Timestamp::now(),
            true,
        )
        .expect("valid guard lease");

        assert_eq!(v3_anchor_state_from_lease_optional(&lease).ok(), Some(None));
    }

    #[test]
    fn anchor_update_preserves_live_writer_coordination_state() {
        let claim = WriterFenceClaim {
            holder_identity: "pod-a/process-1".to_owned(),
            token: 3,
        };
        let lease = lease_with_guard_state(
            Lease::default(),
            &claim,
            9,
            std::time::Duration::from_secs(30),
            Timestamp::now(),
            true,
        )
        .expect("valid guard lease");

        let updated =
            v3_lease_with_claim(lease, &v3_state(1), &claim).expect("live fence accepts update");
        let spec = updated.spec.expect("coordination spec");
        assert_eq!(spec.holder_identity.as_deref(), Some("pod-a/process-1"));
        assert_eq!(spec.lease_duration_seconds, Some(30));
    }

    #[test]
    fn stale_writer_fence_cannot_advance_anchor_after_handoff() {
        let current_claim = WriterFenceClaim {
            holder_identity: "pod-b/process-2".to_owned(),
            token: 4,
        };
        let lease = lease_with_guard_state(
            Lease::default(),
            &current_claim,
            12,
            std::time::Duration::from_secs(30),
            Timestamp::now(),
            true,
        )
        .expect("valid guard lease");
        let stale_claim = WriterFenceClaim {
            holder_identity: "pod-a/process-1".to_owned(),
            token: 3,
        };

        let result = v3_lease_with_claim(lease, &v3_state(2), &stale_claim);

        assert!(matches!(result, Err(V3FormatError::AnchorAdvanceFailed)));
    }

    #[test]
    fn unfenced_maintenance_detects_writer_coordination_state() {
        let claim = WriterFenceClaim {
            holder_identity: "pod-a/process-1".to_owned(),
            token: 3,
        };
        let lease = lease_with_guard_state(
            Lease::default(),
            &claim,
            9,
            std::time::Duration::from_secs(30),
            Timestamp::now(),
            true,
        )
        .expect("valid guard lease");

        assert!(lease_has_writer_coordination(&lease));
        assert_eq!(v3_anchor_state_from_lease_optional(&lease).ok(), Some(None));
        assert!(matches!(
            v3_lease_for_anchor_advance(lease, &v3_state(1), None),
            Err(V3FormatError::AnchorAdvanceFailed)
        ));
    }
}
