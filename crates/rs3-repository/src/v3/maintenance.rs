//! v3 maintenance planning and conservative apply paths.

pub(in crate::v3) mod capacity;
mod packed_usage;
mod recovery;

pub(in crate::v3) mod introduced;

pub(in crate::v3) mod coverage;

use super::commit::{V3_SECTION_FLAG_MUST_UNDERSTAND, V3CommitKey};
use super::error::{V3FormatError, V3Result};
use super::provider::V3ProviderProfile;
use super::repository::{
    V3AnchorState, V3CommitAnchor, V3CommitStore, V3ReplayChain, V3ReplayCommit, V3ReplayLimits,
};
use super::service::packed::{
    V3PackedIndexRunReplay, apply_packed_index_run, repository_context_from_refs,
};
use super::standalone::validate_v3_standalone_object;
use super::{V3CommitKind, V3IndexRoot, V3IndexRootRunRef, V3SectionType, open_v3_index_root};
use crate::state::RepositoryState;
use async_trait::async_trait;
use bytes::Bytes;
use rs3_index::{NamespaceEntry, PayloadReference, V3StandaloneStreamCarrierReference};
use rs3_storage::{
...
struct V3MaintenanceBudgetedStore<'a, S> {
    inner: &'a S,
    budgets: V3MaintenanceBudgets,
    usage: Arc<RwLock<V3MaintenanceIoUsage>>,
}

impl<S> Clone for V3MaintenanceBudgetedStore<'_, S> {
    fn clone(&self) -> Self {
        Self {
            inner: self.inner,
...
impl<'a, S> V3MaintenanceBudgetedStore<'a, S> {
    fn new(inner: &'a S, budgets: V3MaintenanceBudgets) -> Self {
        Self {
            inner,
            budgets,
            usage: Arc::new(RwLock::new(V3MaintenanceIoUsage::default())),
        }
    }

    fn usage(&self) -> rs3_storage::Result<V3MaintenanceIoUsage> {
        self.usage
            .read()
            .map(|usage| *usage)
...
struct V3MaintenanceBudgetedList {
...
#[async_trait]
impl BlobList for V3MaintenanceBudgetedList {
...
#[async_trait]
impl<S> BlobStore for V3MaintenanceBudgetedStore<'_, S>
where
    S: BlobStore,
{
    async fn put(
    ...
    async fn open_bounded_list(
        ...
        Ok(Box::new(V3MaintenanceBudgetedList {
            inner,
            budgets: self.budgets,
            usage: Arc::clone(&self.usage),
            mode,
...
fn charge_maintenance_io(
    budgets: V3MaintenanceBudgets,
    usage: &RwLock<V3MaintenanceIoUsage>,
    request_count: u64,
    head_count: u64,
    range_read_bytes: u64,
    list_mode: Option<BlobListMode>,
...
/// Broad path-private class of one v3 orphan candidate.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum V3OrphanObjectClass {
...
/// Unanchored v3 backend object discovered by orphan reporting.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3OrphanCandidate {
    /// Broad backend-visible object class.
    pub object_class: V3OrphanObjectClass,
    ...
    /// Parsed sequence when the object key has a valid v3 commit shape.
...
/// Redacted v3 orphan report.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3OrphanReport {
    ...
    /// Candidate objects under the v3 commit and object prefixes that are not reachable.
    pub candidates: Vec<V3OrphanCandidate>,
}

/// Conservative v3 orphan garbage-collection policy.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct V3OrphanGcOptions {
...
impl V3OrphanGcOptions {
    /// Creates conservative orphan-GC options.
    pub fn new(min_age: Duration) -> V3Result<Self> {
        if min_age < MIN_ORPHAN_GC_AGE {
            return Err(V3FormatError::OrphanGcMinAgeTooLow);
        }
        Ok(Self {
            min_age,
...
/// Result of one conservative v3 orphan garbage-collection pass.
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct V3OrphanGcReport {
    ...
    pub aborted: Option<V3FormatError>,
}

/// Redacted v3 quick-maintenance report.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3MaintenanceReport {
    ...
    /// Orphan candidate count under the v3 commit prefix.
    pub orphan_candidate_count: usize,
    /// Total bytes held by orphan candidates under the v3 commit prefix.
...
/// Operator-accepted budgets for v3 full-maintenance dry runs and apply plans.
///
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct V3MaintenanceBudgets {
...
impl Default for V3MaintenanceBudgets {
    fn default() -> Self {
        Self {
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3QuickMaintenanceOptions {
...
impl Default for V3QuickMaintenanceOptions {
    fn default() -> Self {
        Self {
            budgets: V3MaintenanceBudgets::default(),
...
/// Observed planning reads and planned mutations for v3 full maintenance.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub struct V3MaintenancePlanCost {
...
impl V3MaintenancePlanCost {
    /// Returns true when this plan fits the supplied operator budgets.
    pub fn fits_budgets(self, budgets: V3MaintenanceBudgets) -> bool {
...
/// Options for v3 full-maintenance dry runs.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3FullGcDryRunOptions {
    ...
    /// protected-root maintenance fails with [`V3FormatError::InvalidFormatRoot`].
    pub protected_roots: Vec<V3AnchorState>,
}

impl Default for V3FullGcDryRunOptions {
    fn default() -> Self {
        Self {
            budgets: V3MaintenanceBudgets::default(),
            retention_renewal_horizon: DEFAULT_RETENTION_RENEWAL_HORIZON,
...
/// Path-redacted v3 full-maintenance dry-run report.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3FullGcDryRunReport {
    ...
    pub planned_cost: V3MaintenancePlanCost,
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3FullGcPlanPreview {
...
/// Guard required before destructive v3 maintenance can mutate storage.
#[async_trait]
pub trait V3MaintenanceGuard: Send + Sync {
    /// Verifies that the maintenance process still owns its exclusion window.
    async fn verify_v3_maintenance(&self, base_anchor: Option<&V3AnchorState>) -> V3Result<()>;
}

#[async_trait]
impl<G> V3MaintenanceGuard for Arc<G>
where
    G: V3MaintenanceGuard + ?Sized,
{
    async fn verify_v3_maintenance(&self, base_anchor: Option<&V3AnchorState>) -> V3Result<()> {
        self.as_ref().verify_v3_maintenance(base_anchor).await
    }
}

/// Cooperative cancellation signal for destructive v3 maintenance runs.
///
...
pub struct V3MaintenanceCancellation {
    cancelled: AtomicBool,
}

impl V3MaintenanceCancellation {
...
/// that opt in (for example `xtask v3 gc-rehearsal --unenforced-guard` against
/// a fresh single-process prefix).
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct UnenforcedQuiescedMaintenanceGuard;

#[async_trait]
impl V3MaintenanceGuard for UnenforcedQuiescedMaintenanceGuard {
    async fn verify_v3_maintenance(&self, _base_anchor: Option<&V3AnchorState>) -> V3Result<()> {
        Ok(())
    }
}

/// Options for destructive v3 full-maintenance apply.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3FullGcApplyOptions {
...
/// Result of destructive v3 full-maintenance apply.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3FullGcApplyReport {
    ...
    pub orphan_gc: V3OrphanGcReport,
}

#[derive(Clone, Debug, Default, PartialEq, Eq)]
struct V3RetentionRenewalPlan {
    commit_count: usize,
    bytes: u64,
    blocked_count: usize,
    blocked_bytes: u64,
    ...
    targets: Vec<V3RetentionTarget>,
}

#[derive(Clone, Debug, PartialEq, Eq)]
struct V3RetentionTarget {
    object_id: BackendObjectId,
    version_id: Option<BackendVersionId>,
    stored_len: u64,
...
#[derive(Clone, Debug, PartialEq, Eq)]
struct V3FullGcPlan {
    report: V3FullGcDryRunReport,
    base_anchor: Option<V3AnchorState>,
    recovery_expire_before_ms: Option<i64>,
    verified_current_recovery_floor_ms: Option<i64>,
    retention_renewal: V3RetentionRenewalPlan,
    orphans: V3OrphanReport,
    current_chain: Option<V3ReplayChain>,
    current_state: Option<RepositoryState>,
}

pub(super) struct V3PreparedFullGcPlan {
    pub(super) plan_digest: String,
    pub(super) options: V3FullGcApplyOptions,
    plan: V3FullGcPlan,
}

impl V3PreparedFullGcPlan {
    pub(super) fn report(&self) -> &V3FullGcDryRunReport {
        &self.plan.report
    }

    pub(super) fn current(&self) -> Option<(&V3ReplayChain, &RepositoryState)> {
        self.plan
            .current_chain
            .as_ref()
...
fn encode_plan_anchor(anchor: &V3AnchorState) -> Vec<u8> {
...
fn encode_plan_budgets(encoded: &mut Vec<u8>, budgets: V3MaintenanceBudgets) {
...
fn encode_plan_options(options: &V3FullGcApplyOptions) -> Vec<u8> {
    let mut encoded = Vec::new();
...
fn encode_plan_report(report: &V3FullGcDryRunReport) -> Vec<u8> {
    let mut encoded = Vec::new();
...
fn encode_plan_orphan(candidate: &V3OrphanCandidate) -> Vec<u8> {
    let mut encoded = Vec::new();
    encoded.push(match candidate.object_class {
        V3OrphanObjectClass::Commit => 0,
        V3OrphanObjectClass::Object => 1,
    });
...
fn encode_plan_renewal(target: &V3RetentionTarget) -> Vec<u8> {
    let mut encoded = Vec::new();
...
fn full_gc_plan_digest(plan: &V3FullGcPlan, options: &V3FullGcApplyOptions) -> String {
...
#[derive(Clone, Debug, PartialEq, Eq)]
struct V3StandalonePayloadRoot {
    object_id: BackendObjectId,
    version_id: Option<BackendVersionId>,
    stored_len: u64,
    object_digest: [u8; 32],
...
struct V3ReachabilityState {
    packed_usage: packed_usage::PackedUsage,
    anchor_state: Option<V3AnchorState>,
    current_chain: Option<V3ReplayChain>,
    current_state: Option<RepositoryState>,
    recovery_expire_before_ms: Option<i64>,
    ...
    verified_commits: BTreeMap<(BackendObjectId, Option<BackendVersionId>), Arc<V3ReplayCommit>>,
    reachable: BTreeSet<BackendObjectId>,
    reachable_versions: BTreeSet<(BackendObjectId, Option<BackendVersionId>)>,
    renewal_targets: BTreeMap<(BackendObjectId, Option<BackendVersionId>), V3RetentionTarget>,
    ...
    standalone_facts:
        BTreeMap<(BackendObjectId, Option<BackendVersionId>), V3StandalonePayloadRoot>,
    chain_get_count: u64,
...
pub(crate) struct V3ResolvedIndexRoot {
    pub(crate) runs: Vec<V3IndexRootRunRef>,
    referenced_commits: Vec<V3ReplayCommit>,
}

impl V3ReachabilityState {
    fn include_chain(
        &mut self,
        chain: &V3ReplayChain,
        protected: bool,
    ...
    S: BlobStore,
{
    ...
    pub(crate) fn open_index_root_without_replay(
        &self,
        containing_commit: &V3ReplayCommit,
        section_ordinal: u32,
        stored_root: &[u8],
        limits: V3ReplayLimits,
    ) -> V3Result<V3IndexRoot> {
        ...
        .map_err(|_| V3FormatError::InvalidIndexRoot)?;
        ...
        match header.parent.as_ref() {
            ...
            None if header.self_ref.sequence == Sequence::new(1)
                ...
                && root.runs().is_empty() => {}
            _ => return Err(V3FormatError::InvalidIndexRoot),
        }
        ...
        *state = RepositoryState::default();
            ...
            if replay.version_id != location.version_id
                ...
                || replay.parsed_header.sections_start
                    != usize::try_from(location.sections_start)
                        .map_err(|_| V3FormatError::SectionBounds)?
                || referenced_header.kind != V3CommitKind::Delta
                || !valid_lineage
        ...
        if state.next_sequence != root.covered_generation()
            || u64::try_from(state.list_entries.len())
                .map_err(|_| V3FormatError::IndexRootLimitExceeded)?
        ...
        Ok(V3ResolvedIndexRoot {
            runs,
            referenced_commits,
        })
    }

    /// Reports unanchored v3 objects without deleting anything.
    pub async fn report_orphans<A>(&self, anchor: &A) -> V3Result<V3OrphanReport>
    where
        A: V3CommitAnchor,
    {
    ...
    pub async fn report_orphans_with_protected_roots<A>(
        &self,
        anchor: &A,
        protected_roots: &[V3AnchorState],
    ) -> V3Result<V3OrphanReport>
    where
        A: V3CommitAnchor,
    {
        let reachability = self
            ...
            .await?;
        self.report_orphans_from_reachability(&reachability, V3MaintenanceBudgets::default())
            .await
    ...
    async fn report_orphans_from_reachability(
        &self,
        reachability: &V3ReachabilityState,
        budgets: V3MaintenanceBudgets,
    ) -> V3Result<V3OrphanInventory> {
        ...
        let retained_profile =
            self.provider_profile() == V3ProviderProfile::RetainedVersionObjectLock;
        ...
        for (prefix, object_class) in [
            ("commits/v03/", V3OrphanObjectClass::Commit),
            ("objects/v03/", V3OrphanObjectClass::Object),
        ] {
            ...
            loop {
                ...
                for mut metadata in page.entries {
                    if object_class == V3OrphanObjectClass::Object
                    ...
                    let sequence = if object_class == V3OrphanObjectClass::Commit {
                    ...
                    candidates.push(V3OrphanCandidate {
                        object_class,
    ...
    /// Deletes expired, unprotected v3 orphan objects.
    ///
    ...
    pub async fn delete_expired_orphans<A>(
        &self,
        anchor: &A,
        guard: &impl V3MaintenanceGuard,
        options: V3OrphanGcOptions,
    ) -> V3Result<V3OrphanGcReport>
    where
        A: V3CommitAnchor,
    {
        guard.verify_v3_maintenance(None).await?;
        let base_anchor = anchor.read_v3().await?;
        guard.verify_v3_maintenance(base_anchor.as_ref()).await?;
        ...
        self.delete_expired_orphan_candidates(
            ...
            report,
            options,
            V3OrphanDeleteBounds {
                max_delete_count: None,
                cancellation: &V3MaintenanceCancellation::new(),
                op_pacing_delay: None,
            },
        )
        .await
    ...
    async fn delete_expired_orphan_candidates<A>(
        ...
        report: V3OrphanReport,
        options: V3OrphanGcOptions,
        bounds: V3OrphanDeleteBounds<'_>,
    ) -> V3Result<V3OrphanGcReport>
    where
        A: V3CommitAnchor,
    {
        let V3OrphanDeleteBounds {
            max_delete_count,
            cancellation,
            op_pacing_delay,
        } = bounds;
        ...
        for candidate in report.candidates {
            ...
            if cancellation.is_cancelled() {
                gc.aborted = Some(V3FormatError::MaintenanceCancelled);
                return Ok(gc);
            }
            if let Err(error) = guard.verify_v3_maintenance(base_anchor).await {
                gc.aborted = Some(error);
                return Ok(gc);
            }
            if anchor.read_v3().await? != base_anchor.cloned() {
                gc.aborted = Some(V3FormatError::StaleAnchor);
                return Ok(gc);
            }
            pace_maintenance_operation(op_pacing_delay).await;

            let delete = if self.provider_profile() == V3ProviderProfile::RetainedVersionObjectLock
            {
    ...
    ) -> V3Result<V3ReachabilityState>
    where
        A: V3CommitAnchor,
    {
        self.load_reachability_from_state(
            anchor.read_v3().await?,
            protected_roots,
            budgets,
    ...
    async fn live_payload_roots_from_chain(
        ...
        for entry in state.namespace.live_entries() {
            ...
            let root = match &entry.payload_ref {
                Some(PayloadReference::V3Pack { carrier, .. }) => commit_payload_root(
    ...
    async fn replay_chain_to_namespace_state(
        ...
        chain: &V3ReplayChain,
        limits: V3ReplayLimits,
    ) -> V3Result<(RepositoryState, Vec<V3ReplayCommit>)> {
                ...
                V3SectionType::IndexRoot => {
                    ...
                    let resolved = self
                        .apply_index_root_to_state(
                            state,
                            commit,
                            u32::try_from(index).map_err(|_| V3FormatError::SectionBounds)?,
                            section_bytes,
                            limits,
                        )
                        .await?;
    ...
    pub async fn quick_maintenance<A>(&self, anchor: &A) -> V3Result<V3MaintenanceReport>
    where
        A: V3CommitAnchor,
    {
        self.quick_maintenance_with_options(anchor, V3QuickMaintenanceOptions::default())
            .await
    }
    ...
    ) -> V3Result<V3MaintenanceReport>
    where
        A: V3CommitAnchor,
    {
        let budgeted_store = V3MaintenanceBudgetedStore::new(self.store(), options.budgets);
    ...
    ///
    /// This first-stage planner is intentionally limited to v3 object inventory
    ...
    pub(super) async fn full_gc_dry_run_with_state<A>(
        ...
        Option<(V3ReplayChain, RepositoryState)>,
    )>
    where
        A: V3CommitAnchor,
    {
        ...
        A: V3CommitAnchor,
    {
        ...
        let planned_cost = V3MaintenancePlanCost {
        ...
        let report = V3FullGcDryRunReport {
    ...
    ) -> V3Result<V3FullGcApplyReport>
    where
        A: V3CommitAnchor,
        G: V3MaintenanceGuard,
    {
        self.apply_full_gc_cancellable(anchor, guard, options, &V3MaintenanceCancellation::new())
            .await
    }
    ...
    /// [`V3FormatError::MaintenanceCancelled`] before the next renewal call;
    ...
    where
        ...
        G: V3MaintenanceGuard,
    {
        let V3PreparedFullGcPlan { options, plan, .. } = prepared;
        guard.verify_v3_maintenance(None).await?;
        ...
        if anchor.read_v3().await? != base_anchor {
        ...
        {
            guard.verify_v3_maintenance(Some(base_anchor)).await?;
            if anchor.read_v3().await?.as_ref() != Some(base_anchor) {
                return Err(V3FormatError::StaleAnchor);
            }
        ...
        G: V3MaintenanceGuard + ?Sized,
    {
        ...
        for (object_id, version_id, stored_len) in targets {
            ...
            let observed_deadline = observed
                ...
                .ok_or(V3FormatError::ProviderProfileFailed)?;
            let observed_policy =
                active_retention(observed.retention).ok_or(V3FormatError::ProviderProfileFailed)?;
    ...
    where
        ...
        G: V3MaintenanceGuard + ?Sized,
    {
        guard.verify_v3_maintenance(base_anchor).await?;
        if anchor.read_v3().await? != base_anchor.cloned() {
            return Err(V3FormatError::StaleAnchor);
        }
        if self.provider_profile() == V3ProviderProfile::RetainedVersionObjectLock
        ...
        let required_deadline = match target.required_deadline {
            ...
            Some(_) => return Err(V3FormatError::ProviderProfileFailed),
        ...
        G: V3MaintenanceGuard,
    {
        ...
        for target in targets {
            ...
            if cancellation.is_cancelled() {
                return Err(V3FormatError::MaintenanceCancelled);
            }
            pace_maintenance_operation(op_pacing_delay).await;
    ...
    async fn plan_retention_renewal(
        ...
        for target in targets {
                    ...
                    }))
            {
                ...
                plan.targets.push(V3RetentionTarget {
                    object_id: metadata.object_id,
                    version_id: metadata.version_id,
...
fn commit_payload_root(
    ...
    format_ref: super::V3FormatRef,
    retention: Option<RetentionPolicy>,
    legal_hold: Option<LegalHoldStatus>,
) -> V3Result<V3LivePayloadRoot> {
    let parsed_key = V3CommitKey::parse(&commit_key)?;
    Ok(V3LivePayloadRoot::Commit {
        root: V3AnchorState {
...
fn standalone_payload_root(
    carrier: &V3StandaloneStreamCarrierReference,
) -> V3StandalonePayloadRoot {
    V3StandalonePayloadRoot {
...
fn validate_standalone_payload_root(root: &V3StandalonePayloadRoot) -> V3Result<()> {
    validate_v3_standalone_object(&root.object_id, root.stored_len)
}
...
fn ensure_next_budgeted_operation(limit: Option<u64>, used: u64) -> V3Result<()> {
...
fn planned_renewal_requirements(
    ...
    now_ms: i64,
) -> V3Result<(i64, Option<LegalHoldStatus>)> {
...
fn required_retain_until_ms(policy: RetentionPolicy, now_ms: i64) -> V3Result<i64> {
    now_ms
        .checked_add(
...
fn planned_retain_until_ms_at(policy: RetentionPolicy, now_ms: i64) -> V3Result<i64> {
    super::recovery::policy::ceil_physical_deadline_ms(required_retain_until_ms(policy, now_ms)?)
}

fn commit_section_bytes(commit: &V3ReplayCommit, index: usize) -> V3Result<&[u8]> {
    commit
        .retained_sections
        .get(index)
...
mod tests {
    use super::{
        V3FormatError, V3FullGcApplyOptions, V3FullGcDryRunOptions, V3FullGcDryRunReport,
        V3FullGcPlan, V3MaintenanceBudgetedStore, V3MaintenanceBudgets, V3MaintenancePlanCost,
        V3OrphanCandidate, V3OrphanGcOptions, V3OrphanObjectClass, V3OrphanReport,
        V3ReachabilityState, V3RetentionRenewalPlan, V3RetentionTarget, V3StandalonePayloadRoot,
    ...
    async fn bounded_full_read_charges_the_declared_object_length() {
        ...
        store
            ...
            .await
            .expect("store body");
        let budgeted = V3MaintenanceBudgetedStore::new(
            &store,
    ...
    fn standalone_root(byte: u8) -> V3StandalonePayloadRoot {
        V3StandalonePayloadRoot {
    ...
    fn digest_plan(candidate_ids: [&str; 2]) -> V3FullGcPlan {
        let candidates = candidate_ids
            .into_iter()
            .enumerate()
            .map(|(ordinal, object_id)| V3OrphanCandidate {
                object_class: V3OrphanObjectClass::Object,
        ...
        V3FullGcPlan {
            report: V3FullGcDryRunReport {
                ...
                planned_cost: V3MaintenancePlanCost {
                    delete_count: 2,
            ...
            retention_renewal: V3RetentionRenewalPlan::default(),
            orphans: V3OrphanReport {
    ...
    fn digest_options() -> V3FullGcApplyOptions {
    ...
    fn standalone_roots_mark_exact_live_and_protected_versions() {
        ...
        let version_key = (root.object_id.clone(), root.version_id.clone());
        let mut reachability = V3ReachabilityState::default();
    ...
    fn planned_renewal_preserves_observed_deadline_and_legal_hold() {
        ...
        let no_fixed_floor = V3RetentionTarget {
