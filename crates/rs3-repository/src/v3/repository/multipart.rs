...
impl V3UploadedPart {
    ...
    pub(in crate::v3) fn selected_attempts_digest(parts: &[Self]) -> [u8; 32] {
...
pub struct V3MultipartUpload {
    ...
    inflight: V3InflightStandaloneObject,
...
pub struct V3VerifiedMultipartUpload {
    ...
    pub(crate) stored: Option<V3StoredStandalonePayload>,
    pub(crate) _inflight: V3InflightStandaloneObject,
}
...
impl V3MultipartUpload {
    ...
    pub async fn upload_part(
        ...
        {
            return Err(V3FormatError::ContentMd5Mismatch);
        }
        let backend = backend.map_err(storage_to_v3)?;
        if backend.index() != index || backend.content_len() != ciphertext_len {
            return Err(V3FormatError::ProviderProfileFailed);
        }
        let digests = digests.ok_or(V3FormatError::ProviderProfileFailed)?;
    ...
    pub(in crate::v3) fn list_parts(
        &self,
        after: u32,
        limit: usize,
    ) -> V3Result<Vec<V3UploadedPart>> {
        let parts = self
            .parts
            .read()
            .map_err(|_| V3FormatError::StorageOperationFailed)?;
    ...
    pub(in crate::v3) fn select_parts(
        &self,
    ...
    pub async fn abort(self) -> V3Result<()> {
...
impl<S: BlobStore> V3CommitStore<S> {
    ...
    pub async fn create_client_multipart_upload(
        ...
        legal_hold: Option<LegalHoldStatus>,
        checksum_policy: Option<MultipartChecksumPolicy>,
    ) -> V3Result<V3MultipartUpload> {
        self.validate_write_protection_profile(retention, legal_hold)?;
        let object_id = super::super::standalone::generate_v3_standalone_object_id()?;
    ...
    pub async fn complete_client_multipart_upload(
    ...
    ) -> V3Result<V3VerifiedMultipartUpload> {
        ...
        Ok(V3VerifiedMultipartUpload {
            etag,
            stored: Some(V3StoredStandalonePayload {
                etag,
    ...
    ) -> V3Result<Option<SegmentedPayloadLayout>> {
        let context = super::super::service::packed::repository_context_from_refs(
            &self.options.repository_id,
            &self.options.keyring_envelope_ref,
        )
...
async fn verify_selected_parts<S: BlobStore>(
    ...
    if !pending.is_empty() || reader.next_chunk().await.map_err(storage_to_v3)?.is_some() {
        return Err(V3FormatError::ProviderProfileFailed);
    }
    Ok(whole.finalize())
}

async fn abort_client_upload(backend: Box<dyn BlobMultipartSession>) -> V3Result<()> {
