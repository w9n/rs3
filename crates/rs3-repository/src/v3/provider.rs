//! Storage-provider conformance checks for v3 profiles.

use super::error::{V3ErrorClass, V3FormatError, V3Result};
use bytes::Bytes;
...
pub fn required_v3_provider_check_names(profile: V3ProviderProfile) -> Vec<&'static str> {
    let profile_checks = match profile {
        V3ProviderProfile::Dev => &[][..],
        V3ProviderProfile::AtomicCreate => ATOMIC_CREATE_PROVIDER_CHECKS,
        V3ProviderProfile::RetainedVersionObjectLock => RETAINED_PROVIDER_CHECKS,
    };
...
/// v3 storage-provider profile selected for a repository.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub enum V3ProviderProfile {
...
/// Options for a v3 provider conformance run.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3ProviderConformanceOptions {
    /// Profile whose requirements are being checked.
    pub profile: V3ProviderProfile,
...
impl V3ProviderConformanceOptions {
    /// Creates conformance options for a profile with an explicit probe prefix.
    pub fn new(profile: V3ProviderProfile, probe_prefix: impl Into<String>) -> Self {
        Self {
            profile,
...
pub enum V3ProviderCheckStatus {
    /// The check passed.
    Passed,
    /// The check failed.
    Failed,
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3ProviderConformanceCheck {
    /// Stable check name.
    pub name: &'static str,
    /// Check status.
    pub status: V3ProviderCheckStatus,
    /// Redacted failure reason, when status is failed.
    pub reason: Option<&'static str>,
}

impl V3ProviderConformanceCheck {
    ...
    fn failed(name: &'static str, reason: &'static str) -> Self {
        Self {
            name,
            status: V3ProviderCheckStatus::Failed,
            reason: Some(reason),
        }
    }
}
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3ProviderConformanceReport {
    /// Profile checked by this report.
    pub profile: V3ProviderProfile,
    /// Individual check results.
    pub checks: Vec<V3ProviderConformanceCheck>,
}

impl V3ProviderConformanceReport {
    /// Returns true when every conformance check passed.
    pub fn passed(&self) -> bool {
        let required = required_v3_provider_check_names(self.profile);
        self.checks.len() == required.len()
            && required.iter().all(|required_name| {
                self.checks.iter().any(|check| {
                    check.name == *required_name && check.status == V3ProviderCheckStatus::Passed
                })
            })
    }

    /// Converts the report into a v3 result, preserving the report on success.
    pub fn require_pass(self) -> V3Result<Self> {
        if self.passed() {
            Ok(self)
        } else {
    ...
    pub const fn failure_class(&self) -> V3ErrorClass {
...
/// Runs redacted v3 provider conformance checks against a `BlobStore`.
pub async fn check_v3_provider_conformance<S>(
    store: &S,
    options: &V3ProviderConformanceOptions,
) -> V3Result<V3ProviderConformanceReport>
where
    S: BlobStore,
{
    let mut checks = Vec::new();
    if options.profile == V3ProviderProfile::RetainedVersionObjectLock {
        checks.push(if store.supports_provider_delete_probe() {
            V3ProviderConformanceCheck::passed("protected-delete-provider-probe")
        } else {
            V3ProviderConformanceCheck::failed(
                "protected-delete-provider-probe",
                "provider delete probes require an isolated capable handle",
            )
        });
        if !store.supports_provider_delete_probe() {
            return Ok(V3ProviderConformanceReport {
    ...
    Ok(V3ProviderConformanceReport {
        profile: options.profile,
        checks,
    })
}
...
async fn run_multipart_checks<S>(
    store: &S,
    options: &V3ProviderConformanceOptions,
    checks: &mut Vec<V3ProviderConformanceCheck>,
) -> V3Result<()>
where
    S: BlobStore,
{
    if !store.supports_multipart_upload() {
        checks.push(V3ProviderConformanceCheck::failed(
            "multipart-supported",
            "multipart upload unsupported",
        ));
        return Ok(());
    }
    checks.push(V3ProviderConformanceCheck::passed("multipart-supported"));
    ...
    let mut upload = match store
        .create_multipart_upload(
            ...
            PutOptions {
                ...
                do_not_recreate: options.profile == V3ProviderProfile::AtomicCreate,
            },
        )
        .await
    {
        Ok(upload) => {
            checks.push(V3ProviderConformanceCheck::passed("multipart-create"));
            upload
        }
        Err(_) => {
            checks.push(V3ProviderConformanceCheck::failed(
    ...
    checks.push(V3ProviderConformanceCheck::passed("multipart-put-part"));

    let metadata = match upload.complete().await {
        Ok(metadata) if metadata.content_len == content_len => {
            checks.push(V3ProviderConformanceCheck::passed("multipart-complete"));
            metadata
        }
        Ok(_) => {
            checks.push(V3ProviderConformanceCheck::failed(
    ...
    match store.head(&object_id).await {
        Ok(head) if head.content_len == content_len => {
            checks.push(V3ProviderConformanceCheck::passed("multipart-head"));
        }
        Ok(_) => checks.push(V3ProviderConformanceCheck::failed(
            "multipart-head",
            "metadata mismatch",
        )),
        Err(_) => checks.push(V3ProviderConformanceCheck::failed(
            "multipart-head",
            "head failed",
        )),
    }
        ...
        .await
    {
        Ok(read) if read == Bytes::from_static(b"aaaatail") => {
            checks.push(V3ProviderConformanceCheck::passed(
                "multipart-boundary-range-get",
            ));
        }
        Ok(_) => checks.push(V3ProviderConformanceCheck::failed(
    ...
    if options.profile == V3ProviderProfile::RetainedVersionObjectLock {
        match metadata.version_id.as_ref() {
            ...
            None => checks.push(V3ProviderConformanceCheck::failed(
                "multipart-retained-version-id",
                "missing version id",
            )),
        }
    ...
    if options.profile == V3ProviderProfile::AtomicCreate {
    ...
    S: BlobStore,
{
    ...
    match upload.complete().await {
        Err(StorageError::AlreadyExists(_)) => checks.push(V3ProviderConformanceCheck::passed(
            "multipart-atomic-complete-rejected",
        )),
        Ok(_) => checks.push(V3ProviderConformanceCheck::failed(
            "multipart-atomic-complete-rejected",
    ...
    match read_probe_body_at(store, &object_id, None, original.len()).await {
        Ok(read) if read == original => checks.push(V3ProviderConformanceCheck::passed(
            "multipart-atomic-preserves-existing",
        )),
        Ok(_) => checks.push(V3ProviderConformanceCheck::failed(
            "multipart-atomic-preserves-existing",
    ...
    S: BlobStore,
{
    ...
    match store
        .put(
            ...
            PutOptions {
                ...
                legal_hold: None,
                content_type: Some("application/octet-stream".to_owned()),
                do_not_recreate: options.profile != V3ProviderProfile::RetainedVersionObjectLock,
            },
        )
        .await
    {
        Ok(metadata) if metadata.content_len == body.len() as u64 => {
            checks.push(V3ProviderConformanceCheck::passed("basic-put"));
        }
        Ok(_) => checks.push(V3ProviderConformanceCheck::failed(
            "basic-put",
            "metadata mismatch",
    ...
    match store.head(&object_id).await {
        Ok(metadata) if metadata.content_len == body.len() as u64 => {
            checks.push(V3ProviderConformanceCheck::passed("basic-head"));
        }
        Ok(_) => checks.push(V3ProviderConformanceCheck::failed(
            "basic-head",
            "metadata mismatch",
    ...
    match read_probe_body_at(store, &object_id, None, body.len()).await {
        Ok(read) if read == body => checks.push(V3ProviderConformanceCheck::passed("basic-get")),
        Ok(_) => checks.push(V3ProviderConformanceCheck::failed(
            "basic-get",
            "body mismatch",
        )),
        ...
        .await
    {
        Ok(read) if read == Bytes::from_static(b"provider") => {
            checks.push(V3ProviderConformanceCheck::passed("basic-range-get"));
        }
        Ok(_) => checks.push(V3ProviderConformanceCheck::failed(
            "basic-range-get",
            "range mismatch",
    ...
    match list_probe_prefix_bounded(store, &options.probe_prefix, BlobListMode::Current).await {
        Ok(entries) if entries.iter().any(|entry| entry.object_id == object_id) => {
            checks.push(V3ProviderConformanceCheck::passed("basic-list"));
        }
        Ok(_) => checks.push(V3ProviderConformanceCheck::failed(
            "basic-list",
    ...
    S: BlobStore,
{
    ...
    match store.head_at(&object_id, Some(&first_version)).await {
        Ok(metadata) if retention_satisfies(metadata.retention.as_ref(), &options.retention) => {
            checks.push(V3ProviderConformanceCheck::passed("retained-exact-head"));
        }
        Ok(_) => checks.push(V3ProviderConformanceCheck::failed(
            "retained-exact-head",
            "retention metadata mismatch",
    ...
    match read_probe_body_at(store, &object_id, Some(&first_version), first_body.len()).await {
        Ok(read) if read == first_body => {
            checks.push(V3ProviderConformanceCheck::passed("retained-exact-get"));
        }
        Ok(_) => checks.push(V3ProviderConformanceCheck::failed(
            "retained-exact-get",
            "body mismatch",
    ...
    match second.version_id.as_ref() {
        ...
        Some(_) => checks.push(V3ProviderConformanceCheck::failed(
            "retained-overwrite-version-id",
            "version id reused",
        )),
        None => checks.push(V3ProviderConformanceCheck::failed(
    ...
    match read_probe_body_at(store, &object_id, None, second_body.len()).await {
        Ok(read) if read == second_body => {
            checks.push(V3ProviderConformanceCheck::passed("retained-latest-get"));
        }
        Ok(_) => checks.push(V3ProviderConformanceCheck::failed(
            "retained-latest-get",
    ...
    match read_probe_body_at(store, &object_id, Some(&first_version), first_body.len()).await {
        Ok(read) if read == first_body => checks.push(V3ProviderConformanceCheck::passed(
            "retained-old-version-survives",
        )),
        Ok(_) => checks.push(V3ProviderConformanceCheck::failed(
            "retained-old-version-survives",
    ...
    match store.delete_at(&object_id, Some(&first_version)).await {
        ...
        Ok(()) => checks.push(V3ProviderConformanceCheck::failed(
            "retained-active-exact-delete-blocked",
    ...
    match store.delete(&object_id).await {
        Err(StorageError::RetentionBlocked | StorageError::LegalHoldBlocked) => checks.push(
            V3ProviderConformanceCheck::passed("retained-delete-blocked"),
        ),
        Ok(()) => checks.push(V3ProviderConformanceCheck::failed(
            "retained-delete-blocked",
            "delete succeeded",
    ...
    S: BlobStore,
{
    ...
    match store.delete_at(&object_id, Some(&version_id)).await {
        Ok(()) => match store.head_at(&object_id, Some(&version_id)).await {
            Err(StorageError::NotFound(_)) => checks.push(V3ProviderConformanceCheck::passed(
                "retained-unprotected-exact-delete",
            )),
            Ok(_) => checks.push(V3ProviderConformanceCheck::failed(
                "retained-unprotected-exact-delete",
    ...
    S: BlobStore,
{
    ...
    match store.head_at(&object_id, Some(&version_id)).await {
        Ok(metadata) if metadata.legal_hold == Some(LegalHoldStatus::On) => {
            checks.push(V3ProviderConformanceCheck::passed("legal-hold-verifiable"));
        }
        Ok(_) => checks.push(V3ProviderConformanceCheck::failed(
            "legal-hold-verifiable",
...
fn retention_for_profile(options: &V3ProviderConformanceOptions) -> Option<RetentionPolicy> {
    (options.profile == V3ProviderProfile::RetainedVersionObjectLock).then_some(options.retention)
}
