use super::*;
use crate::v3::{RecoveryPolicy, V3QuickMaintenanceOptions};
...
async fn mixed_pack(history: bool) {
    ...
    let report = must_v3(repository.commit_store().quick_maintenance(&anchor).await);
    ...
    let alias_report = must_v3(repository.commit_store().quick_maintenance(&anchor).await);
...
async fn packed_usage_budget_exhaustion_returns_no_partial_report() {
    ...
    assert_eq!(error, V3FormatError::MaintenanceBudgetExceeded);
    assert!(
        repository
            .commit_store()
