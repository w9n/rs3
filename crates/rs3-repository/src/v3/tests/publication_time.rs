...
async fn publication_times_are_strict_under_equal_backward_clocks_without_parent_reads() {
    ...
    let mut previous = must_v3(writer.write_genesis_snapshot(&anchor).await);
    for sampled_now in [1_000, 1_000, 999, 1_001] {
        ...
        let next = must_v3(write_empty_metadata_child(&writer, &anchor, false).await);
    ...
    let restarted = V3CommitStore::new(store.clone(), signing_keyring(), options());
    restarted.set_publication_time_for_tests(1_000);
    must_v3(restarted.load_replay_chain_from_anchor(&anchor).await).expect("verified chain");
    let before = store.operation_log().expect("operation log").len();
    let next = must_v3(write_empty_metadata_child(&restarted, &anchor, false).await);
    assert_eq!(next.publish_time_ms, previous.publish_time_ms + 1);
    assert!(
        !store.operation_log().expect("operation log")[before..]
            .iter()
...
async fn future_parent_refuses_publication_after_restart_and_future_candidate_refuses_adoption() {
    ...
    let restarted = V3CommitStore::new(store.clone(), signing_keyring(), options());
    restarted.set_publication_time_for_tests(1_000);
    must_v3(restarted.load_replay_chain_from_anchor(&anchor).await);
    ...
    let candidate = must_v3(write_empty_metadata_child(&writer, &temporary, true).await);
    writer.set_publication_time_for_tests(100_000);
    assert_eq!(
        writer
...
async fn replay_rejects_equal_and_backward_signed_parent_edges() {
    for bad_time in [999, 1_000] {
        ...
        let writer = V3CommitStore::new(store.clone(), keys.clone(), options());
        writer.set_publication_time_for_tests(1_000);
        let anchor = V3MemoryAnchor::new();
        must_v3(writer.write_genesis_snapshot(&anchor).await);
        let child = must_v3(write_empty_metadata_child(&writer, &anchor, false).await);
        let body = store
            .get_range_at(
        ...
        let header = must_v3(header.sign_with_keyring(&keys));
        let replacement =
            must_v3(header.encode_object(&parsed.body[parsed.parsed_header.sections_start..]));
        let changed = store
            .put(
                &child.commit_key.object_id,
                replacement,
        ...
        let bad_anchor = V3AnchorState {
            version_id: changed.version_id,
            ..child.anchor_state
        };
        let restarted = V3CommitStore::new(store, keys, options());
        assert_eq!(
...
async fn siblings_share_a_parent_time_and_checkpoint_replay_does_not_fetch_pruned_parent() {
    ...
    writer.set_publication_time_for_tests(1_000);
    let anchor = V3MemoryAnchor::new();
    let base = must_v3(writer.write_genesis_snapshot(&anchor).await);
    let mut siblings = Vec::new();
    for _ in 0..8 {
        let temporary = V3MemoryAnchor::with_state(base.anchor_state.clone());
        siblings.push(must_v3(
    ...
    let restarted = V3CommitStore::new(store, signing_keyring(), options());
    restarted.set_publication_time_for_tests(1_000);
    let chain =
        must_v3(restarted.load_replay_chain_from_anchor(&anchor).await).expect("checkpoint");
    assert_eq!(
        chain.commits_newest_first.len(),
        1,
        "trusted exact root is the replay cut"
    );
    let child = must_v3(write_empty_metadata_child(&restarted, &anchor, false).await);
...
async fn prepared_genesis_keeps_chosen_time_across_exact_retries() {
    let writer = V3CommitStore::new(MemoryBlobStore::new(), signing_keyring(), options());
    writer.set_publication_time_for_tests(1_000);
