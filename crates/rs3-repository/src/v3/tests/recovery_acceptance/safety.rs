...
async fn recovery_acceptance_ambiguous_renewal_prevents_deletion_and_retry_is_safe() {
    ...
    let result = fixture
        ...
        .await;
    assert!(matches!(result, Err(V3FormatError::StorageOperationFailed)));
...
async fn recovery_acceptance_interrupted_renewal_and_stale_history_plan_cannot_delete() {
    ...
    fixture
        .restart(must_v3(RecoveryPolicy::new(1, 120, 60_000)))
        .await;
    fixture
        .put(
            "acceptance/stale",
    ...
    let rejected = fixture
        ...
        .await;
    assert!(matches!(rejected, Err(V3FormatError::StaleAnchor)));
    assert_eq!(
