...
pub(super) fn options(budgets: V3MaintenanceBudgets, reclaim: bool) -> V3FullGcApplyOptions {
    let mut options = apply_options(reclaim);
    options.dry_run.budgets = budgets;
    options
}

async fn check_graph(scale: &Scale, budgets: V3MaintenanceBudgets) {
    must_v3(
        scale
            .repository
            .commit_store()
            .quick_maintenance_with_options(
                ...
                crate::v3::V3QuickMaintenanceOptions {
                    budgets,
                    ..crate::v3::V3QuickMaintenanceOptions::default()
                },
            )
            .await,
    );
...
async fn fill(
...
) -> (Scale, V3AnchorState, LogicalPath, Bytes) {
    ...
    for index in 0..200 {
        ...
        let before = must_v3(scale.anchor.read_v3().await);
        match coordinator
            ...
            .await
        {
            ...
            Err(error) => {
                ...
                assert_eq!(must_v3(scale.anchor.read_v3().await), before);
...
async fn recovery_capacity_warm_and_restarted_writers_preserve_maintainable_head() {
    for mode in ["overwrite", "append"] {
        let budgets = V3MaintenanceBudgets {
...
async fn recovery_capacity_exact_target_ceiling_applies_to_cached_growth() {
    ...
    scale.restart().await;
    assert_eq!(must_v3(scale.anchor.read_v3().await), Some(last));
    check_graph(&scale, budgets).await;
    assert_eq!(
        must_repo(scale.repository.get_range(&key, ByteRange::Full).await),
        body
...
impl V3MaintenanceGuard for LoseGuardDuringCandidate {
    async fn verify_v3_maintenance(
        &self,
        _base: Option<&V3AnchorState>,
    ) -> crate::v3::V3Result<()> {
        if self.store.operation_counts().expect("counts").get > self.get_threshold {
            return Err(crate::v3::V3FormatError::MaintenanceAccessRequired);
        }
        Ok(())
    }
}
...
async fn recovery_capacity_guard_loss_during_candidate_walk_never_installs_credit() {
    ...
    *scale
    ...
    check_graph(&scale, V3MaintenanceBudgets::default()).await;
}
