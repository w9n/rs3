...
struct Scale {
    ...
    store: ScaleStore,
    repository: Arc<V3Repository<ScaleStore>>,
...
impl Scale {
    async fn new(mode: &'static str) -> Self {
        Self::with_budgets(mode, V3MaintenanceBudgets::default()).await
    }

    async fn with_budgets(mode: &'static str, budgets: V3MaintenanceBudgets) -> Self {
    ...
    fn coordinator(&self) -> V3CommitCoordinator<ScaleStore, V3MemoryAnchor> {
    ...
    async fn record(
        ...
        let sequence = must_v3(self.anchor.read_v3().await)
            .expect("anchor")
            .sequence
            .get();
        println!(
    ...
    async fn restore(&self, selected: &V3AnchorState, key: &LogicalPath, expected: &[u8]) {
        let before = must_v3(self.anchor.read_v3().await);
        let view = must_repo(
            self.repository
        ...
        assert_eq!(
            must_v3(self.anchor.read_v3().await),
            before,
...
async fn run(mode: &'static str, writes: usize, budgets: V3MaintenanceBudgets) {
    ...
    let report = scale
        ...
        .quick_maintenance_with_options(
            &scale.anchor,
            crate::v3::V3QuickMaintenanceOptions {
                budgets,
                ..Default::default()
            },
        )
    ...
    scale
        ...
        .await;
    assert!(must_v3(renewed).retention_renewed_object_count > 0);
    let since = Instant::now();
    let before = scale.counts();
    for (anchor, key, body) in &samples {
        scale.restore(anchor, key, body).await;
    ...
    scale
        ...
        .await;
    assert!(must_v3(reclaimed).orphan_gc.deleted_count > 0);
...
async fn recovery_scale_smoke_overwrite() {
    run("overwrite", 40, V3MaintenanceBudgets::default()).await;
}

#[tokio::test]
async fn recovery_scale_smoke_append() {
    run("append", 40, V3MaintenanceBudgets::default()).await;
}
...
async fn recovery_history_scale() {
    ...
    let mut budgets = V3MaintenanceBudgets::default();
