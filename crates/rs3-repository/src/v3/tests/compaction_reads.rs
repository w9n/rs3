...
async fn compaction_source_reads_skip_recovery_and_payload_sections() {
    ...
    let fresh = V3Repository::new(store, keys, RepositoryOptions::default(), options);
...
async fn compaction_prefetch_preserves_unselected_exact_reference_through_fresh_replay() {
    ...
    let decode_root = |commit: &crate::v3::repository::V3ReplayCommit| {
        assert_eq!(
            commit.parsed_header.header.section_index[0].section_type,
            V3SectionType::IndexRoot
        );
        must_v3(crate::v3::open_v3_index_root(
            &keys,
            &context,
    ...
    let historical = V3Repository::new(store, keys, RepositoryOptions::default(), options);
    let old_anchor = V3MemoryAnchor::with_state(old_checkpoint.anchor_state);
