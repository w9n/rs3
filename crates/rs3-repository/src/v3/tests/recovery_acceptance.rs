...
struct Fixture {
    store: TestStore,
    repository: Arc<V3Repository<TestStore>>,
...
impl Fixture {
    async fn new(policy: RecoveryPolicy) -> Self {
        ...
        let root = V3KeyringEnvelopeRootRef {
            generation: 1,
    ...
    async fn put(&self, path: &str, body: &'static [u8]) -> V3AnchorState {
        must_repo(
            self.repository
    ...
    async fn current(&self) -> V3AnchorState {
        must_v3(self.anchor.read_v3().await).expect("accepted anchor")
    }

    async fn orphan(&self, sequence: u64, hold: Option<LegalHoldStatus>) -> BlobMetadata {
        let id = must_v3(generate_v3_commit_key(Sequence::new(sequence))).object_id;
        self.store
            .put(
                &id,
    ...
    async fn expire(&self) -> bool {
        ...
        let mutation = crate::v3::service::V3CoordinatedMutation::new(&lease, &self.anchor)
    ...
    async fn restore(&self, selected: &V3AnchorState, path: &str, expected: &'static [u8]) {
