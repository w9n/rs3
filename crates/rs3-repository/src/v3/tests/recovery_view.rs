use super::*;
use crate::v3::{RecoveryPolicy, V3RecoveryCursor};
...
async fn recovery_view_keeps_original_namespace_through_overwrite_delete_restart_and_root_cut() {
    ...
    let selected = must_v3(anchor.read_v3().await).expect("selected anchor");
    ...
    let current = must_v3(anchor.read_v3().await).expect("live anchor");
    ...
    let restarted = V3Repository::new(store, keys, RepositoryOptions::default(), options());
    assert!(
    ...
    assert_eq!(must_v3(anchor.read_v3().await), Some(current));
    assert!(
        recovered
            .check_authority(&V3MemoryAnchor::new())
            .await
            .is_err()
    );
    assert!(
...
async fn recovery_point_pagination_is_bounded_stale_and_never_resurrects_expired_history() {
    ...
    let cursor = must_repo(V3RecoveryCursor::decode(&encoded));
    ...
    assert!(V3RecoveryCursor::decode(&"f".repeat(1025)).is_err());

    let old = V3Repository::new(
        MemoryBlobStore::new(),
        signing_keyring(),
        RepositoryOptions::default(),
        options(),
    ...
    let old_anchor = V3MemoryAnchor::new();
    ...
    let current = must_v3(old_anchor.read_v3().await).expect("current");
    assert!(
