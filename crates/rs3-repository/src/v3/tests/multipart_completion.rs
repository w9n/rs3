use super::*;
use crate::v3::{V3ClientMultipartUpload, V3MultipartSelection};
...
impl Fixture {
    ...
    fn coordinator(&self) -> V3CommitCoordinator<MemoryBlobStore, V3MemoryAnchor> {
    ...
    async fn reopen(&self) -> V3Repository<MemoryBlobStore> {
...
async fn multipart_receipt_survives_overwrite_checkpoint_compaction_and_restart() {
    for empty in [false, true] {
        ...
        let anchor = must_v3(f.anchor.read_v3().await).expect("accepted");
        ...
        let overwritten = must_v3(f.anchor.read_v3().await).expect("newer");
        assert_eq!(
            must_repo(
                f.repository
        ...
        assert_eq!(must_v3(f.anchor.read_v3().await), Some(overwritten));
        must_repo(coordinator.write_index_snapshot().await);
        let fresh = f.reopen().await;
        assert_eq!(
            must_repo(fresh.accepted_multipart_completion(&id, &key(), &selection)),
...
async fn multipart_failed_anchor_publishes_neither_value_nor_receipt() {
    ...
    let coordinator = must_repo(V3CommitCoordinator::new(
        ...
        FailOnceV3Anchor::new(f.anchor.clone()),
    ));
...
#[async_trait::async_trait]
impl V3CommitAnchor for LostCompletionReply {
...
async fn multipart_lost_anchor_reply_is_reconciled_or_requires_recovery() {
    for lose_reconciliation in [false, true] {
        ...
        let result = coordinator
            .complete_multipart_upload(upload, selection.clone())
            .await;
        let accepted = must_v3(f.anchor.read_v3().await).expect("accepted");
        ...
        assert_eq!(must_v3(f.anchor.read_v3().await), Some(accepted));
    }
}
