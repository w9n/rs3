...
pub(super) fn require(valid: bool) -> V3Result<()> {
    if valid {
        Ok(())
    } else {
        Err(V3FormatError::MalformedCbor)
    }
}

pub(super) fn write_text(out: &mut Vec<u8>, text: &str, maximum: usize) -> V3Result<()> {
    require(text.len() <= maximum)?;
    cbor::write_text(out, text);
    Ok(())
}

pub(super) fn write_optional_text(out: &mut Vec<u8>, text: Option<&str>) -> V3Result<()> {
    match text {
...
pub(super) fn read_optional_text(reader: &mut Reader<'_>) -> V3Result<Option<String>> {
...
pub(super) fn read_digest(reader: &mut Reader<'_>) -> V3Result<[u8; 32]> {
    reader
...
pub(super) fn digest_bytes(hex_digest: &str) -> V3Result<[u8; 32]> {
...
pub(super) fn read_envelope_ref(
    reader: &mut Reader<'_>,
) -> V3Result<(u64, String, BackendObjectId, Option<BackendVersionId>)> {
