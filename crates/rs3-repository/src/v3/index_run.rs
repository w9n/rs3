...
use super::{V3FormatError, V3Result, digest_v3_section};
use bytes::Bytes;
...
pub const V3_INDEX_RUN_ID_LEN: usize = 32;
/// Maximum number of independently authenticated data frames in one run.
pub const V3_INDEX_RUN_MAX_FRAME_COUNT: usize = 4_096;
/// Maximum plaintext bytes in one independently authenticated data frame.
pub const V3_INDEX_RUN_MAX_FRAME_PLAINTEXT_BYTES: usize = 1024 * 1024;
/// Maximum complete encrypted index-run section or standalone object size.
pub const V3_INDEX_RUN_MAX_OBJECT_BYTES: usize = 8 * 1024 * 1024;
/// Fixed byte count required for the first range probe.
pub const V3_INDEX_RUN_FIXED_HEADER_BYTES: usize = 76;
/// Maximum encrypted-directory plaintext bytes.
pub const V3_INDEX_RUN_MAX_DIRECTORY_PLAINTEXT_BYTES: usize = 1024 * 1024;
...
const INDEX_RUN_FIXED_HEADER_LEN: usize = V3_INDEX_RUN_FIXED_HEADER_BYTES;
...
pub struct V3IndexRunId([u8; V3_INDEX_RUN_ID_LEN]);

impl V3IndexRunId {
    /// Returns the exact random identity bytes.
    #[must_use]
    pub const fn as_bytes(&self) -> &[u8; V3_INDEX_RUN_ID_LEN] {
        &self.0
    }
...
pub struct V3SealedIndexRun {
    run_id: V3IndexRunId,
    bytes: Bytes,
}

impl V3SealedIndexRun {
    /// Returns the random identity authenticated by the run.
    #[must_use]
    pub const fn run_id(&self) -> V3IndexRunId {
        self.run_id
    }
...
pub struct V3IndexRunFrameDescriptor {
    ordinal: u32,
...
impl V3IndexRunFrameDescriptor {
...
#[derive(Clone, PartialEq, Eq)]
pub struct V3VerifiedIndexRunDirectory {
    header: RunHeader,
    ...
    container_count: u32,
    directory_end: usize,
    frames: Vec<V3IndexRunFrameDescriptor>,
}
...
/// returned facts become trusted only after [`open_v3_index_run_directory`]
/// authenticates the encrypted directory under the exact repository context.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct V3IndexRunHeaderProbe {
    header_len: usize,
...
impl V3VerifiedIndexRunDirectory {
    ...
    #[must_use]
    pub fn frames(&self) -> &[V3IndexRunFrameDescriptor] {
        &self.frames
    }

    /// Returns one authenticated frame descriptor by physical ordinal.
    #[must_use]
    pub fn frame(&self, ordinal: u32) -> Option<&V3IndexRunFrameDescriptor> {
...
/// `directory_prefix_len`, and then call [`open_v3_index_run_directory`].
pub fn probe_v3_index_run_header(stored_fixed_header: &[u8]) -> V3Result<V3IndexRunHeaderProbe> {
    if stored_fixed_header.len() < INDEX_RUN_FIXED_HEADER_LEN {
        return Err(V3FormatError::InvalidIndexRun);
    }
    ...
    let _run_id: [u8; V3_INDEX_RUN_ID_LEN] = reader.read_array()?;
    ...
    let directory_prefix_len = header_len
        ...
        .ok_or(V3FormatError::IndexRunLimitExceeded)?;
    if key_id_len == 0
        || key_id_len > INDEX_RUN_MAX_KEY_ID_LEN
        || header_len != INDEX_RUN_FIXED_HEADER_LEN + key_id_len
        || frame_count == 0
        || frame_count as usize > V3_INDEX_RUN_MAX_FRAME_COUNT
        || stored_len > V3_INDEX_RUN_MAX_OBJECT_BYTES as u64
        || directory_plaintext_len == 0
        || directory_plaintext_len as usize > V3_INDEX_RUN_MAX_DIRECTORY_PLAINTEXT_BYTES
        || directory_ciphertext_len != directory_plaintext_len
        || directory_prefix_len as u64 > stored_len
    {
        return Err(V3FormatError::IndexRunLimitExceeded);
    }
    Ok(V3IndexRunHeaderProbe {
        header_len,
...
pub fn seal_v3_index_run(
...
) -> V3Result<V3SealedIndexRun> {
    validate_context(repository_context, containing_object, limits)?;
    let encoded =
        encode_index_run_frames(run, limits).map_err(|_| V3FormatError::InvalidIndexRun)?;
    if encoded.frames.is_empty() || encoded.frames.len() > V3_INDEX_RUN_MAX_FRAME_COUNT {
        return Err(V3FormatError::IndexRunLimitExceeded);
    }
    ...
    let run_id = V3IndexRunId::generate()?;
    ...
    if provisional_directory.len() > V3_INDEX_RUN_MAX_DIRECTORY_PLAINTEXT_BYTES {
        return Err(V3FormatError::IndexRunLimitExceeded);
    }
    ...
    let stored_len = descriptors
        ...
        .ok_or(V3FormatError::InvalidIndexRun)?;
    if stored_len > V3_INDEX_RUN_MAX_OBJECT_BYTES as u64 {
        return Err(V3FormatError::IndexRunLimitExceeded);
    }
    ...
    let directory_digest = digest_v3_section(&directory);
...
pub fn open_v3_index_run_directory(
    keyring: &KeyRing,
    repository_context: &[u8],
    containing_object: &BackendObjectId,
    ...
    stored_prefix: &[u8],
    limits: &IndexRunLimits,
) -> V3Result<V3VerifiedIndexRunDirectory> {
    validate_context(repository_context, containing_object, limits)?;
    ...
    Ok(V3VerifiedIndexRunDirectory {
        header,
        directory_digest,
...
pub fn open_v3_index_run_frame(
    keyring: &KeyRing,
    repository_context: &[u8],
    containing_object: &BackendObjectId,
    expected_section_ordinal: u32,
    directory: &V3VerifiedIndexRunDirectory,
    frame_ordinal: u32,
...
pub fn open_v3_index_run_frames<F>(
    keyring: &KeyRing,
    repository_context: &[u8],
    containing_object: &BackendObjectId,
    expected_section_ordinal: u32,
...
where
    F: FnMut(&V3IndexRunFrameDescriptor, Bytes) -> V3Result<()>,
{
    let directory = open_v3_index_run_directory(
        keyring,
        repository_context,
        containing_object,
...
fn descriptors_with_first_offset(
    ...
    first_offset: u64,
) -> V3Result<Vec<V3IndexRunFrameDescriptor>> {
    ...
    for (ordinal, frame) in frames.iter().enumerate() {
        if frame.bytes.is_empty() || frame.bytes.len() > V3_INDEX_RUN_MAX_FRAME_PLAINTEXT_BYTES {
            return Err(V3FormatError::IndexRunLimitExceeded);
        }
        ...
        descriptors.push(V3IndexRunFrameDescriptor {
...
fn descriptor_matches_encoded(
    descriptor: &V3IndexRunFrameDescriptor,
...
fn encode_header(output: &mut Vec<u8>, header: &RunHeader) -> V3Result<()> {
...
fn parse_header(stored: &[u8]) -> V3Result<RunHeader> {
    ...
    let section_ordinal = reader.read_u32()?;
    let frame_count = reader.read_u32()?;
    let run_id = V3IndexRunId(reader.read_array()?);
    let key_id_len = usize::from(reader.read_u16()?);
    if reader.read_u16()? != 0 {
        return Err(V3FormatError::InvalidIndexRun);
    }
    ...
    let key_id_text =
        std::str::from_utf8(key_id_bytes).map_err(|_| V3FormatError::InvalidIndexRun)?;
    let key_id = KeyId::new(key_id_text.to_owned()).map_err(|_| V3FormatError::InvalidIndexRun)?;
    Ok(RunHeader {
        header_len,
        stored_len,
        section_ordinal,
...
fn encode_directory(
    ...
    mutation_count: u32,
    container_count: u32,
    frames: &[V3IndexRunFrameDescriptor],
) -> V3Result<Vec<u8>> {
...
fn encode_descriptor(output: &mut Vec<u8>, descriptor: &V3IndexRunFrameDescriptor) -> V3Result<()> {
...
fn decode_descriptor(
    ...
    limits: &IndexRunLimits,
) -> V3Result<V3IndexRunFrameDescriptor> {
...
fn encode_bound(output: &mut Vec<u8>, bound: Option<&IndexRunSearchBound>) -> V3Result<()> {
    match bound {
        None => output.push(0),
...
fn decode_bound(
    reader: &mut RunReader<'_>,
    limits: &IndexRunLimits,
) -> V3Result<Option<IndexRunSearchBound>> {
    match reader.read_u8()? {
        ...
        _ => Err(V3FormatError::InvalidIndexRun),
    }
}
...
fn validate_directory_layout(
    ...
    for descriptor in frames {
        if descriptor.offset != expected_offset
            ...
            || descriptor.plaintext_len as usize > V3_INDEX_RUN_MAX_FRAME_PLAINTEXT_BYTES
...
fn compare_bounds(left: &IndexRunSearchBound, right: &IndexRunSearchBound) -> V3Result<Ordering> {
    match (left, right) {
        (
            IndexRunSearchBound::Namespace {
                blind_key: left_key,
...
fn validate_context(
    ...
    if limits.max_frame_bytes == 0
        || limits.max_frame_bytes > V3_INDEX_RUN_MAX_FRAME_PLAINTEXT_BYTES
        || limits.max_total_bytes > V3_INDEX_RUN_MAX_OBJECT_BYTES
    {
        return Err(V3FormatError::IndexRunLimitExceeded);
    }
    Ok(())
...
fn decode_role(tag: u8) -> V3Result<IndexRunFrameRole> {
    match tag {
...
fn push_u16(output: &mut Vec<u8>, value: usize) -> V3Result<()> {
...
fn push_u32(output: &mut Vec<u8>, value: usize) -> V3Result<()> {
    output.extend_from_slice(&to_u32(value)?.to_be_bytes());
    Ok(())
}

fn to_u32(value: usize) -> V3Result<u32> {
    u32::try_from(value).map_err(|_| V3FormatError::IndexRunLimitExceeded)
}

fn to_u64(value: usize) -> V3Result<u64> {
    u64::try_from(value).map_err(|_| V3FormatError::IndexRunLimitExceeded)
}

fn to_usize(value: u64) -> V3Result<usize> {
    usize::try_from(value).map_err(|_| V3FormatError::IndexRunLimitExceeded)
}
...
impl<'a> RunReader<'a> {
    ...
    fn read_u8(&mut self) -> V3Result<u8> {
        self.take(1)?
            .first()
            .copied()
            .ok_or(V3FormatError::InvalidIndexRun)
    ...
    fn read_u16(&mut self) -> V3Result<u16> {
        Ok(u16::from_be_bytes(self.read_array()?))
    }

    fn read_u32(&mut self) -> V3Result<u32> {
        Ok(u32::from_be_bytes(self.read_array()?))
    }

    fn read_u64(&mut self) -> V3Result<u64> {
        Ok(u64::from_be_bytes(self.read_array()?))
    }

    fn read_array<const N: usize>(&mut self) -> V3Result<[u8; N]> {
        self.take(N)?
            .try_into()
            .map_err(|_| V3FormatError::InvalidIndexRun)
    }
...
mod tests {
    use super::{
        INDEX_RUN_FIXED_HEADER_LEN, INDEX_RUN_SEAL_OVERHEAD, V3FormatError,
        V3IndexRunFrameDescriptor, common_associated_data, encode_directory, open_v3_index_run,
        open_v3_index_run_directory, open_v3_index_run_frame, open_v3_index_run_frames,
        parse_header, probe_v3_index_run_header, seal_v3_index_run,
    };
    use bytes::Bytes;
    ...
    fn sealed() -> (KeyRing, BackendObjectId, IndexRunLimits, IndexRun, Bytes) {
        ...
        let limits = limits();
        let run = fixture();
        let sealed = must(seal_v3_index_run(
            &keyring,
            REPOSITORY_CONTEXT,
            &object,
            2,
    ...
    fn round_trips_and_opens_selected_exact_ranges() {
        ...
        let probe = must(probe_v3_index_run_header(
        ...
        let directory = must(open_v3_index_run_directory(
            &keyring,
            REPOSITORY_CONTEXT,
            &object,
            2,
        ...
        for (descriptor, expected) in directory.frames().iter().zip(canonical.frames) {
            let range = descriptor.stored_range();
            let selected = must(open_v3_index_run_frame(
                &keyring,
                REPOSITORY_CONTEXT,
                &object,
                2,
    ...
    fn streamed_payload_carrier_frames_reject_tampering() {
        ...
        let limits = IndexRunLimits::default();
        {
            ...
            assert!(
                open_v3_index_run(&keyring, REPOSITORY_CONTEXT, &object, 1, &tampered, &limits,)
                    .is_err(),
    ...
    fn rejects_every_complete_run_truncation_and_trailing_bytes() {
        ...
        trailing.push(0);
        assert_eq!(
            open_v3_index_run(&keyring, REPOSITORY_CONTEXT, &object, 2, &trailing, &limits,),
            Err(V3FormatError::InvalidIndexRun)
        );
    }
    ...
    fn fixed_header_probe_rejects_hostile_lengths() {
        ...
        for length in 0..INDEX_RUN_FIXED_HEADER_LEN {
            assert!(probe_v3_index_run_header(&stored[..length]).is_err());
        }
        ...
        assert!(probe_v3_index_run_header(&oversized).is_err());
    }
    ...
    fn rewrite_directory(
        ...
        limits: &IndexRunLimits,
        stored: &[u8],
        mutate: impl FnOnce(&mut Vec<V3IndexRunFrameDescriptor>),
    ) -> Vec<u8> {
        let verified = must(open_v3_index_run_directory(
            keyring,
            REPOSITORY_CONTEXT,
            object,
            2,
