...
const V3_STREAM_PAYLOAD_CACHE_DOMAIN: &[u8] = b"rs3:v02-stream-segment-cache:v1\n";
...
pub(crate) struct V3StreamPayloadCacheIdentity<'a> {
    pub(crate) repository_keyring_context: &'a [u8],
    pub(crate) carrier: V3StreamPayloadCarrierCacheIdentity<'a>,
    pub(crate) payload_id: &'a BackendObjectId,
    pub(crate) payload_layout: &'a PayloadLayout,
    pub(crate) content_len: u64,
}

pub(crate) enum V3StreamPayloadCarrierCacheIdentity<'a> {
    Standalone {
...
impl V3StreamPayloadCacheIdentity<'_> {
    /// Validates the exact carrier range and derives its plaintext-cache identity.
    pub(crate) fn cache_ref(&self) -> V3Result<BackendObjectRef> {
...
fn update_digest_field(digest: &mut Sha256Hasher, value: &[u8]) -> V3Result<()> {
    let length = u64::try_from(value.len()).map_err(|_| V3FormatError::SectionBounds)?;
...
#[cfg(test)]
mod tests {
    use super::{V3StreamPayloadCacheIdentity, V3StreamPayloadCarrierCacheIdentity};
    use crate::v3::V3FormatError;
    use rs3_index::PayloadLayout;
    ...
    impl Fixture {
        fn identity(&self) -> V3StreamPayloadCacheIdentity<'_> {
            V3StreamPayloadCacheIdentity {
                repository_keyring_context: &self.context,
                carrier: V3StreamPayloadCarrierCacheIdentity::Standalone {
