...
use super::*;
use crate::v3::payload_pack::{
    V3PayloadPackFacts, V3PayloadPackId, V3PayloadPackRecordRef, plan_v3_payload_pack_record_range,
};
use rs3_index::V3PackCarrierReference;
...
impl V3ReachabilityState {
    pub(super) fn include_packed_usage(
        ...
        let Some(PayloadReference::V3Pack { carrier, record }) = &entry.payload_ref else {
            return Ok(());
        };
        let key = (
        ...
        if commit.parsed_header.header.body_digest != carrier.body_digest
            ...
            || section.section_type != V3SectionType::PayloadPack
        ...
        let facts = V3PayloadPackFacts::new(
            V3PayloadPackId::from_bytes(carrier.pack_id),
            carrier.attempt_id,
            carrier.content_key_id.clone(),
            u32::try_from(carrier.length).map_err(|_| V3FormatError::InvalidPayloadPack)?,
            carrier.pack_record_count,
        )?;
        let span = plan_v3_payload_pack_record_range(
            &facts,
            &V3PayloadPackRecordRef::new(record.record_ordinal, record.record_offset),
            entry.content_len,
            0..entry.content_len,
        )?;
        let span = (
...
pub(super) fn pack_fact_bytes(carrier: &V3PackCarrierReference) -> u64 {
    512 + usize_to_u64(std::mem::size_of::<V3PackCarrierReference>())
        + 2 * usize_to_u64(
...
impl PackedUsage {
    ...
    pub(super) fn totals(&self) -> V3Result<(u64, u64)> {
...
impl<S: BlobStore> V3CommitStore<S> {
    pub(in crate::v3) async fn packed_usage_mark_for_tests<A: V3CommitAnchor>(
        &self,
        anchor: &A,
        enabled: bool,
    ) -> V3Result<(u64, u64)> {
        self.load_reachability(anchor, &[], V3MaintenanceBudgets::default(), true, enabled)
            .await?
            .packed_usage
            .totals()
    }
