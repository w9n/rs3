//! Bounded authenticated reads for streamed v3 payload carriers.

use super::{V3FormatError, v3_repository_error};
...
pub struct V3AuthenticatedReadBody {
    content_len: u64,
    working_set_bytes: u64,
    inner: Pin<Box<dyn Stream<Item = Result<Bytes>> + Send>>,
}

impl V3AuthenticatedReadBody {
...
impl Stream for V3AuthenticatedReadBody {
...
) -> Result<V3AuthenticatedReadBody> {
...
fn stream_storage_error() -> RepositoryError {
    v3_repository_error(V3FormatError::StorageOperationFailed)
}
