...
use super::{IndexRun, PackedCompactionSourceRun, V3FormatError, v3_repository_error};
...
fn compaction_challenger(
    sizes: &[(u32, u64)],
) -> crate::v3::V3Result<Option<std::ops::Range<usize>>> {
...
where
    ...
    ) -> Result<crate::v3::V3Result<Vec<IndexRun>>>,
{
    let full = 0..sizes.len();
    if sizes.len() < 2 || super::compaction_window(sizes).map_err(v3_repository_error)? != full {
    ...
    let cost = |window: Range<usize>, outputs| {
        CompactionCost::new(&sizes[window], outputs)
            .ok_or_else(|| v3_repository_error(V3FormatError::IndexRootLimitExceeded))
    };
    let mut primary = full.clone();
    let mut secondary = compaction_challenger(sizes).map_err(v3_repository_error)?;
...
fn reducing_output(output: crate::v3::V3Result<Vec<IndexRun>>) -> Result<Option<Vec<IndexRun>>> {
    match output {
        Ok(output) => Ok(Some(output)),
        Err(V3FormatError::MaintenanceBudgetExceeded) => Ok(None),
        // Selected-source corruption is fatal, never a reason to try fallback.
        Err(error) => Err(v3_repository_error(error)),
    }
}
