    ...
    I::IntoIter: ExactSizeIterator,
{
    ...
    for mut source in sources {
        ...
        encode_index_run_frames(&source.run, limits).map_err(|_| V3FormatError::InvalidIndexRun)?;
    ...
    for winner in winners.into_values() {
            ...
            (current_namespace, &winner.mutation)
        {
            ...
            match namespace.head(&blind_key) {
                ...
                Some(_) => return Err(V3FormatError::InvalidIndexRun),
            }
        }
        ordered.push(winner);
    }
...
fn validate_self_containers(source: &PackedCompactionSourceRun) -> V3Result<()> {
    match (&source.run.self_pack, &source.self_pack_container) {
        ...
        _ => return Err(V3FormatError::InvalidIndexRun),
    }
    Ok(())
}
...
fn encode_group_chunk(
    ...
    match build_and_validate_run(mutations, limits) {
        ...
        Err(_) if chunk.len() == 1 => Err(V3FormatError::IndexRunLimitExceeded),
        Err(_) => {
...
fn build_and_validate_run(
    ...
    for (index, resolved) in mutations.iter().enumerate() {
        let ordinal = u32::try_from(index).map_err(|_| V3FormatError::IndexRunLimitExceeded)?;
    ...
    encode_index_run_frames(&run, limits).map_err(|_| V3FormatError::IndexRunLimitExceeded)?;
    Ok(run)
}
...
mod tests {
    ...
    fn single_oversize_generation_group_is_rejected() {
        ...
        assert_eq!(
            ...
            Err(V3FormatError::IndexRunLimitExceeded)
        );
    }
