...
use super::{PendingV3Snapshot, V3Repository, commit_protection_for_deltas, v3_repository_error};
...
use rs3_index::run::{
    ...
    IndexRunContainer, IndexRunKeyringRef, IndexRunLimits, IndexRunSelfPack,
    IndexRunStandaloneStreamContainer, IndexTombstone, IndexUpsert,
};
use rs3_index::{
    IndexDelta, NamespaceEntry, PayloadReference, V3PackCarrierReference, V3PackRecordReference,
    V3StandaloneStreamCarrierReference,
};
...
use crate::v3::standalone::validate_v3_standalone_object;
use crate::v3::{
    V3_SECTION_FLAG_MUST_UNDERSTAND, V3CommitKey, V3CommitSection, V3FormatError,
    V3KeyringEnvelopeRef, V3ParsedCommitHeader, V3PayloadPackLayout, V3PayloadPackRecord,
    V3PayloadPackRecordInput, V3ProviderProfile, V3SectionType, V3StoredCommit, digest_v3_section,
    open_v3_index_run, open_v3_index_run_directory, probe_v3_index_run_header, seal_v3_index_run,
    seal_v3_payload_pack,
};

pub(super) struct PendingV3PackRecordLocation {
...
pub(super) struct PendingV3PackedCommitSections {
    pub(super) sections: Vec<V3CommitSection>,
    pub(super) locations: Vec<PendingV3PackRecordLocation>,
    pub(super) run: PendingV3IndexRunFacts,
    pub(super) retention: Option<rs3_types::RetentionPolicy>,
    pub(super) legal_hold: Option<rs3_types::LegalHoldStatus>,
}

pub(super) struct PendingV3IndexRunFacts {
    ...
    pub(super) keyring_envelope_ref: V3KeyringEnvelopeRef,
...
pub(in crate::v3) struct IndexRunBounds {
    pub(in crate::v3) minimum_generation: Sequence,
    pub(in crate::v3) maximum_generation: Sequence,
    pub(in crate::v3) namespace: (IndexBlindKey, IndexBlindKey),
    pub(in crate::v3) listing: (LogicalPath, LogicalPath),
}
    ...
    S: BlobStore + Clone,
{
    pub(super) fn pending_packed_sections_for_commit(
    ...
    ) -> Result<Option<PendingV3PackedCommitSections>> {
        if pending
            .completion_receipt
            .as_ref()
        ...
        for payload in pending.payloads() {
            ...
            let ordinal = u32::try_from(pack_inputs.len())
                .map_err(|_| v3_repository_error(V3FormatError::PayloadPackLimitExceeded))?;
            if record_ordinals
            ...
            pack_inputs.push(V3PayloadPackRecordInput {
        ...
        for delta in &deltas {
            ...
            match entry.payload_ref.as_ref() {
                ...
                _ => return Err(v3_repository_error(V3FormatError::InvalidHeaderField)),
            }
        }
        ...
        let sealed_pack = if pack_inputs.is_empty() {
            ...
            Some(
                match seal_v3_payload_pack(
                    ...
                    Ok(pack) => pack,
                    Err(V3FormatError::PayloadPackLimitExceeded) => return Ok(None),
                    Err(error) => return Err(v3_repository_error(error)),
                },
            )
        };
        ...
        for (ordinal, delta) in deltas.iter().enumerate() {
            let mutation_ordinal = u32::try_from(ordinal)
                .map_err(|_| v3_repository_error(V3FormatError::IndexRunLimitExceeded))?;
            match *delta {
                IndexDelta::Upsert { entry, .. } => {
                        ...
                        record_ordinals.get(&entry.manifest_id)
                    {
                        let record = sealed_pack
                            ...
                            .ok_or_else(|| {
                                v3_repository_error(V3FormatError::InvalidPayloadPack)
                            })?;
                    ...
                    } else if let Some(PayloadReference::V3Pack { carrier, record }) =
                        entry.payload_ref.as_ref()
                    {
                        ...
                        let container_ordinal = containers
                            ...
                            .map_err(|_| v3_repository_error(V3FormatError::InvalidHeaderField))?;
                        IndexPayloadPointer::ExternalPack {
                            container_ordinal: u32::try_from(container_ordinal).map_err(|_| {
                                v3_repository_error(V3FormatError::IndexRunLimitExceeded)
                            })?,
                            record: IndexPackRecordPointer {
                    ...
                    } else if let Some(PayloadReference::V3StandaloneStream { carrier }) =
                        entry.payload_ref.as_ref()
                    {
        ...
        if let (Some(pack_section_ordinal), Some(pack)) = (pack_section_ordinal, sealed_pack) {
            ...
            for (manifest_id, record_ordinal) in &record_ordinals {
                ...
                locations.push(PendingV3PackRecordLocation {
            ...
            sections.push(V3CommitSection::new(
        ...
        let sealed_run = match seal_v3_index_run(
            &keyring,
            &context,
            &commit_key.object_id,
            ...
            Ok(run) => run,
            Err(V3FormatError::IndexRunLimitExceeded) => return Ok(None),
            Err(error) => return Err(v3_repository_error(error)),
        };
        let probe = probe_v3_index_run_header(sealed_run.bytes()).map_err(v3_repository_error)?;
        let section_offset = sections.iter().try_fold(0_u64, |offset, section| {
            offset.checked_add(u64::try_from(section.bytes.len()).ok()?)
        });
        let section_offset =
            section_offset.ok_or_else(|| v3_repository_error(V3FormatError::SectionBounds))?;
        let section_len = u64::try_from(sealed_run.bytes().len())
            .map_err(|_| v3_repository_error(V3FormatError::SectionBounds))?;
        let run_facts = PendingV3IndexRunFacts {
            ...
            section_offset,
            section_len,
            section_digest: digest_v3_section(sealed_run.bytes()),
        };
        sections.push(V3CommitSection::new(
        ...
        Ok(Some(PendingV3PackedCommitSections {
            sections,
            locations,
            run: run_facts,
            retention,
    ...
    pub(super) fn resolve_pending_pack_refs(
        &self,
        pending: &mut PendingV3Snapshot,
        stored: &V3StoredCommit,
        locations: &[PendingV3PackRecordLocation],
    ) -> Result<()> {
        let mut resolved_count = 0_usize;
        let mut shared_carrier: Option<Arc<V3PackCarrierReference>> = None;
        for delta in pending.deltas_mut() {
            ...
            entry.payload_ref = match (
                ...
                (Some(pack_section_ordinal), Some(pack), Some(record)) => {
                    let candidate = V3PackCarrierReference {
...
pub(in crate::v3) struct V3PackedIndexRunReplay<'a> {
    pub(in crate::v3) parsed_header: &'a V3ParsedCommitHeader,
    pub(in crate::v3) version_id: Option<&'a BackendVersionId>,
    pub(in crate::v3) object_len: u64,
    pub(in crate::v3) section_ordinal: u32,
    pub(in crate::v3) stored_run: &'a [u8],
    pub(in crate::v3) level: u16,
    pub(in crate::v3) compaction_generation: u64,
    pub(in crate::v3) provider_profile: V3ProviderProfile,
}

pub(in crate::v3) fn apply_packed_index_run(
    keyring: &rs3_crypto::KeyRing,
    repository_id: &rs3_types::RepositoryId,
    state: &mut RepositoryState,
    replay: V3PackedIndexRunReplay<'_>,
) -> Result<crate::v3::V3IndexRootRunRef> {
    ...
    for container in &run.standalone_stream_containers {
        validate_v3_standalone_object(&container.object_id, container.stored_len)
            .map_err(v3_repository_error)?;
        if replay.provider_profile == V3ProviderProfile::RetainedVersionObjectLock
    ...
    let descriptor = replay
        ...
        .ok_or_else(|| v3_repository_error(V3FormatError::SectionBounds))?;
    if descriptor.section_type != V3SectionType::IndexRun
        || descriptor.length
            != u64::try_from(replay.stored_run.len())
                .map_err(|_| v3_repository_error(V3FormatError::SectionBounds))?
        || descriptor.digest != digest_v3_section(replay.stored_run)
    {
        return Err(v3_repository_error(V3FormatError::InvalidIndexRun));
    }
    let accepted_run = crate::v3::V3IndexRootRunRef {
        ...
        location: crate::v3::V3EmbeddedIndexRunLocation {
    ...
    let self_pack = match run.self_pack.as_ref() {
        Some(pack) => {
            let mut matches = replay
                ...
                .iter()
                .enumerate()
                .filter(|(_, section)| section.section_type == V3SectionType::PayloadPack);
    ...
    let self_pack_carrier = self_pack
        .map(|(pack_section_ordinal, section, pack)| {
            Ok::<Arc<V3PackCarrierReference>, RepositoryError>(Arc::new(V3PackCarrierReference {
                commit_key: commit_key.clone(),
                        ...
                        let carrier = pack_carriers
                            .get(
                                usize::try_from(container_ordinal).map_err(|_| {
                                    v3_repository_error(V3FormatError::InvalidIndexRun)
                                })?,
                            )
                            .ok_or_else(|| v3_repository_error(V3FormatError::InvalidIndexRun))?;
                        Some(PayloadReference::V3Pack {
                            carrier: Arc::clone(carrier),
                            record: pack_record_reference(record),
                        })
...
fn index_run_self_pack(layout: &V3PayloadPackLayout) -> IndexRunSelfPack {
    IndexRunSelfPack {
...
fn index_run_pack_container(carrier: &V3PackCarrierReference) -> IndexRunContainer {
    IndexRunContainer {
...
fn pack_carrier_from_index_run(container: &IndexRunContainer) -> V3PackCarrierReference {
    V3PackCarrierReference {
...
fn intern_standalone_stream_carrier(
    state: &mut RepositoryState,
    carrier: V3StandaloneStreamCarrierReference,
) -> Result<Arc<V3StandaloneStreamCarrierReference>> {
    let key = (carrier.object_id.clone(), carrier.version_id.clone());
    match state.v3_standalone_carriers.get(&key) {
        Some(existing) if existing.as_ref() != &carrier => {
            Err(v3_repository_error(V3FormatError::InvalidHeaderField))
        }
        Some(existing) => Ok(Arc::clone(existing)),
        None => {
            let carrier = Arc::new(carrier);
            state
                .v3_standalone_carriers
...
fn pack_record_reference(record: IndexPackRecordPointer) -> V3PackRecordReference {
    V3PackRecordReference {
        record_ordinal: record.record_ordinal,
        record_offset: record.physical_offset,
    }
}

fn index_pack_record_pointer(record: &V3PayloadPackRecord) -> IndexPackRecordPointer {
...
pub(in crate::v3) fn index_run_bounds(mutations: &[IndexMutation]) -> Result<IndexRunBounds> {
...
pub(in crate::v3) fn repository_context_from_refs(
    repository_id: &rs3_types::RepositoryId,
    keyring_reference: &crate::v3::V3KeyringEnvelopeRef,
) -> Result<Vec<u8>> {
...
fn verify_blind_key(
    ...
    if !matches {
        return Err(v3_repository_error(V3FormatError::CryptoOperation));
    }
    Ok(())
}
...
mod tests {
    ...
    use rs3_index::{PayloadLayout, V3StandaloneStreamCarrierReference};
    use rs3_types::{BackendObjectId, BackendVersionId, KeyId};
    use std::sync::Arc;

    fn carrier() -> V3StandaloneStreamCarrierReference {
        V3StandaloneStreamCarrierReference {
