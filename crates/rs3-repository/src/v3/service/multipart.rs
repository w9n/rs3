...
use super::*;
use crate::v3::{V3MultipartUpload, V3UploadedPart, V3VerifiedMultipartUpload};
...
impl V3ClientMultipartUpload {
    ...
    pub async fn abort(self) -> Result<()> {
        self.upload.abort().await.map_err(v3_repository_error)
    }
}

pub(in crate::v3) struct PreparedMultipartCompletion {
    id: MultipartUploadId,
    key: LogicalPath,
    options: RepositoryPutOptions,
    protection: (Option<RetentionPolicy>, Option<LegalHoldStatus>),
...
impl<S: BlobStore + Clone> V3Repository<S> {
    ...
    pub async fn create_multipart_upload(
        ...
        let id = MultipartUploadId::from_bytes(
            rs3_crypto::random_carrier_id()
                .map_err(|_| v3_repository_error(V3FormatError::RandomnessUnavailable))?,
        );
    ...
    pub(in crate::v3) async fn prepare_multipart_completion(
        &self,
        ...
        let parts = match session.upload.select_parts(&selection.parts) {
            ...
            Err(error) => {
                let _ = session.abort().await;
                return Err(v3_repository_error(error));
            }
        };
    ...
    pub(in crate::v3) async fn publish_multipart_completion<A: V3CommitAnchor>(
        &self,
...
fn invalid_completion() -> RepositoryError {
    v3_repository_error(V3FormatError::InvalidHeaderField)
}
