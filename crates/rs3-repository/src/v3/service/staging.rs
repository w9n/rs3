//! Bounded speculative state for v3 repository mutations.

use super::PendingV3Payload;
...
pub(super) const V3_MAX_PENDING_OPERATIONS: usize = rs3_index::run::INDEX_PACK_MAX_RECORDS as usize;
...
pub(super) struct PendingV3Checkpoint {
    deltas_len: usize,
    manifests_len: usize,
    payloads_len: usize,
}
...
impl<'a> PendingV3EffectiveHead<'a> {
...
#[derive(Clone, Debug)]
pub(super) struct PendingV3Snapshot {
    ...
    payloads: Vec<PendingV3Payload>,
}

impl PendingV3Snapshot {
    #[cfg(test)]
    ...
    pub(super) fn payloads(&self) -> &[PendingV3Payload] {
        &self.payloads
    }
...
#[derive(Debug)]
pub(super) struct PendingV3State {
    revision: u64,
    base: PendingV3Checkpoint,
    end: PendingV3Checkpoint,
...
impl PendingV3State {
    pub(super) fn new(accepted_sequence: Sequence) -> Self {
        Self {
            revision: 0,
            base: PendingV3Checkpoint::default(),
            end: PendingV3Checkpoint::default(),
            frozen: None,
    ...
    pub(super) fn checkpoint(&self) -> PendingV3Checkpoint {
        self.end
    }

    /// Freezes the current prefix while allowing bounded successor appends.
    pub(super) fn freeze(&mut self) -> Result<PendingV3Snapshot> {
    ...
    pub(super) fn append_operation(
        ...
        payload: Option<PendingV3Payload>,
    ) -> Result<PendingV3Checkpoint> {
    ...
    pub(super) fn rollback(&mut self, checkpoint: PendingV3Checkpoint) -> Result<()> {
    ...
    pub(super) fn validate_snapshot(&self, snapshot: &PendingV3Snapshot) -> Result<()> {
    ...
    fn relative_position(&self, position: PendingV3Checkpoint) -> Result<PendingV3Checkpoint> {
    ...
    pub(super) fn snapshot(&self) -> PendingV3Snapshot {
        let mut snapshot = PendingV3Snapshot {
    ...
    pub(super) fn effective_head<'a>(
    ...
    ) -> PendingV3EffectiveHead<'a> {
        for delta in self.deltas.iter().rev() {
            match delta {
                IndexDelta::Upsert { entry, .. } if entry.blind_key == *blind_key => {
                    return PendingV3EffectiveHead::Live(entry);
                }
                IndexDelta::Tombstone {
                    blind_key: candidate,
                    ..
                } if candidate == blind_key => return PendingV3EffectiveHead::Tombstoned,
        ...
        accepted
            .namespace
            .head(blind_key)
            .map_or(PendingV3EffectiveHead::Absent, PendingV3EffectiveHead::Live)
    }
...
fn ensure_bounded_append(current: usize, additional: usize) -> Result<()> {
    if current
        ...
        .is_none_or(|next| next > V3_MAX_PENDING_OPERATIONS)
    {
...
mod tests {
    use super::{
        PendingV3EffectiveHead, PendingV3State, V3_MAX_PENDING_OPERATIONS, maximum_delta_generation,
    };
    use crate::error::RepositoryError;
    use crate::state::{RepositoryState, TrustedManifest};
    use crate::v3::service::PendingV3Payload;
    use bytes::Bytes;
    ...
    fn overlay_lookup_prefers_newest_mutation_and_preserves_tombstone_state() {
        ...
        let mut pending = PendingV3State::new(Sequence::new(1));

        assert_eq!(
            pending.effective_head(&accepted, &blind_key),
            PendingV3EffectiveHead::Live(&accepted_entry)
        );
        pending
        ...
        assert_eq!(
            pending.effective_head(&accepted, &blind_key),
            PendingV3EffectiveHead::Live(&staged_entry)
        );
        pending
            .append_operation(
        ...
        assert_eq!(
            pending.effective_head(&accepted, &blind_key),
            PendingV3EffectiveHead::Tombstoned
        );
        assert!(
            pending
    ...
    fn frozen_coalesced_prefix_preserves_successor_and_absolute_rollback() {
        let mut pending = PendingV3State::new(Sequence::ZERO);
        let append = |pending: &mut PendingV3State, label: &str| {
    ...
    fn frozen_prefix_and_successor_share_the_service_limit() {
        let mut pending = PendingV3State::new(Sequence::ZERO);
        let half = V3_MAX_PENDING_OPERATIONS / 2;
        let append = |pending: &mut PendingV3State, start: usize| {
    ...
    fn rollback_truncates_every_vector_without_reusing_sequence() {
        let mut pending = PendingV3State::new(Sequence::new(7));
    ...
    fn append_rejects_the_hard_limit_atomically() {
        assert_eq!(V3_MAX_PENDING_OPERATIONS, 4_096);
        let mut pending = PendingV3State::new(Sequence::ZERO);
        let deltas = (0..V3_MAX_PENDING_OPERATIONS)
            .map(|index| {
                upsert(entry(
    ...
    fn reset_requires_empty_overlay() {
        let mut pending = PendingV3State::new(Sequence::new(3));
        let checkpoint = pending
            .append_operation(
                vec![upsert(entry(
                    blind_key("pending"),
            ...
            for (operation, key) in history {
                ...
                match operation {
                    0 | 1 => {
                        ...
                        let checkpoint = pending.append_operation(
                            ...
                            live.then(|| PendingV3Payload { manifest_id: id, body: Bytes::from(vec![key]) }),
                ...
                for key in 0_u8..8 {
                    ...
                    if winners.get(&key).is_some_and(|(_, live)| !live) {
                        prop_assert_eq!(head, PendingV3EffectiveHead::Tombstoned);
                    }
                }
            }
        }
