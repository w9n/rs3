...
use super::packed::{
    V3PackedIndexRunReplay, apply_packed_index_run, index_run_bounds, repository_context_from_refs,
};
use super::packed_compaction::{PackedCompactionSourceRun, plan_packed_run_compaction};
use super::{V3CoordinatedMutation, V3Repository, v3_repository_error};
use crate::error::{RepositoryError, Result};
use crate::state::RepositoryState;
use crate::v3::{
    V3_SECTION_FLAG_MUST_UNDERSTAND, V3CommitAnchor, V3CommitParentRef, V3CommitSection,
    V3CommitWrite, V3EmbeddedIndexRunLocation, V3FormatError, V3IndexRoot, V3IndexRootRunRef,
    V3MaintenanceGuard, V3MemoryAnchor, V3SectionType, V3StoredCommit, digest_v3_section,
    open_v3_index_run, probe_v3_index_run_header, seal_v3_index_root, seal_v3_index_run,
};
...
const V3_PACKED_COMPACTION_MAX_SOURCE_RUNS: usize = 256;
const V3_PACKED_COMPACTION_MAX_SOURCE_MUTATIONS: u64 = 131_072;
const V3_PACKED_COMPACTION_MAX_SOURCE_BYTES: u64 = 16 * 1024 * 1024;
...
fn compaction_window(sizes: &[(u32, u64)]) -> crate::v3::V3Result<std::ops::Range<usize>> {
    ...
    if sizes.iter().any(|&(mutations, bytes)| {
        mutations == 0
            || u64::from(mutations) > V3_PACKED_COMPACTION_MAX_SOURCE_MUTATIONS / 2
            || bytes == 0
            || bytes > V3_PACKED_COMPACTION_MAX_SOURCE_BYTES / 2
    }) {
    ...
    for start in 0..sizes.len() {
        ...
        for (offset, &(run_mutations, run_bytes)) in sizes[start..]
            .iter()
            .take(V3_PACKED_COMPACTION_MAX_SOURCE_RUNS)
            .enumerate()
        {
            mutations = mutations
            ...
            bytes = bytes
                ...
                .ok_or(V3FormatError::IndexRootLimitExceeded)?;
            if mutations > V3_PACKED_COMPACTION_MAX_SOURCE_MUTATIONS
                || bytes > V3_PACKED_COMPACTION_MAX_SOURCE_BYTES
            {
                break;
            }
    ...
    S: BlobStore + Clone,
{
    ...
    pub(in crate::v3) async fn compact_packed_index_runs_coordinated<A, G>(
        &self,
        ...
        G: V3MaintenanceGuard + ?Sized,
    {
        ...
        guard
            .verify_v3_maintenance(Some(&base_anchor))
            .await
            .map_err(v3_repository_error)?;
        ...
        let sizes = ordered_refs
            ...
            .collect::<Vec<_>>();
        let window = compaction_window(&sizes).map_err(v3_repository_error)?;
        let source_refs = &ordered_refs[window.clone()];
        if source_refs.len() < 2 {
            return Err(v3_repository_error(
        ...
        let compaction_generation = base_anchor
            .sequence
            .checked_next()
            .ok_or_else(|| v3_repository_error(V3FormatError::InvalidIndexRoot))?;
        ...
        let expected_parent = V3CommitParentRef {
        ...
        for run in output_runs {
            ...
            let uploaded = self
                ...
                .write_prepared_child_commit_with(&temporary_anchor, &sibling_plan, |commit_key| {
                    ...
                    let probe = probe_v3_index_run_header(sealed.bytes())?;
                    ...
                    sealed_facts = Some((
                        *sealed.run_id().as_bytes(),
                        ...
                        digest_v3_section(sealed.bytes()),
                    ));
                    let mut write = V3CommitWrite::delta(vec![V3CommitSection::new(
            ...
            let (run_id, frame_count, bounds, section_len, section_digest) =
                sealed_facts.ok_or_else(|| v3_repository_error(V3FormatError::InvalidIndexRun))?;
            let replay = self
                .commit_store
            ...
            let verified_run = open_v3_index_run(
                keyring.as_ref(),
                &context,
                &uploaded.anchor_state.commit_key,
                0,
            ...
            compacted_refs.push(V3IndexRootRunRef {
                run_id,
        ...
        let mut prepared = self
            ...
            .await?;
        let root_anchor = V3MemoryAnchor::with_state(base_anchor.clone());
        ...
        let recovery = self
            ...
            .await?;
        if root_anchor.read_v3().await.map_err(v3_repository_error)? != Some(candidate_anchor) {
...
fn source_self_pack_container(
    run: &IndexRun,
    replay: &crate::v3::repository::V3ReplayCommit,
    ...
    if matches.next().is_some()
        ...
        || section.flags != V3_SECTION_FLAG_MUST_UNDERSTAND
    {
        return Err(v3_repository_error(V3FormatError::InvalidIndexRun));
    }
