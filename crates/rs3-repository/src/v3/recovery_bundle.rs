...
use super::{V3AnchorState, V3FormatError, V3FormatRef, V3RecoveryBundle, V3Result, cbor, wire};
...
impl V3RecoveryBundle {
    ...
    pub fn to_object_bytes(&self) -> V3Result<Vec<u8>> {
        let mut out = Vec::new();
    ...
    pub fn from_object_bytes(bytes: &[u8]) -> V3Result<Self> {
        ...
        wire::require(
            reader.read_array_len()? == 7
                && reader.read_u64()? == u64::from(super::V3_FORMAT_VERSION),
        )?;
    ...
    pub fn offline_signature_payload(&self) -> V3Result<Vec<u8>> {
    ...
    fn encode(&self, out: &mut Vec<u8>, include_signature: bool) -> V3Result<()> {
        ...
        cbor::write_u64(out, u64::from(super::V3_FORMAT_VERSION));
...
mod tests {
    ...
    fn bundle() -> V3RecoveryBundle {
        ...
        let mut bundle = V3RecoveryBundle::from_anchor(anchor, Sequence::new(5));
    ...
    fn canonical_bundle_has_pinned_bytes_and_rejects_malformed_encodings() {
        ...
        assert_eq!(
            V3RecoveryBundle::from_object_bytes(&bytes).expect("decode"),
            bundle
        );
        for length in 0..bytes.len() {
            assert!(V3RecoveryBundle::from_object_bytes(&bytes[..length]).is_err());
        }
        ...
        for invalid in [
            ...
            assert!(V3RecoveryBundle::from_object_bytes(&invalid).is_err());
        }
    }
    ...
    fn offline_signature_covers_every_bundle_field() {
        ...
        let decoded =
            V3RecoveryBundle::from_object_bytes(&bundle.to_object_bytes().expect("encode"))
                .expect("decode");
        decoded
