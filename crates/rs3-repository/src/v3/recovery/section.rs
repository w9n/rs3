...
pub(in crate::v3) fn open(
    ...
    let key_id = KeyId::new(key).map_err(|_| V3FormatError::InvalidRecoveryHistory)?;
...
fn associated_data(context: &[u8], object: &BackendObjectId, header: &[u8]) -> V3Result<Vec<u8>> {
...
fn take<const N: usize>(bytes: &[u8], offset: &mut usize) -> V3Result<[u8; N]> {
    let end = offset
        .checked_add(N)
        .ok_or(V3FormatError::InvalidRecoveryHistory)?;
    let value = bytes
