//! Preview v3 commit-store workflow.

mod genesis;
mod multipart;
pub use genesis::V3PreparedGenesis;
pub use multipart::{V3MultipartUpload, V3UploadedPart, V3VerifiedMultipartUpload};

use super::commit::{
    V3_COMMIT_CONTENT_TYPE, V3_HEADER_META_LEN, V3_MAX_HEADER_SIZE,
    V3_SECTION_FLAG_MUST_UNDERSTAND, V3CommitHeader, V3CommitKey, V3CommitKind, V3CommitParentRef,
    V3CommitSelfRef, V3KeyringEnvelopeRef, V3ParsedCommit, V3ParsedCommitHeader,
    V3SectionDescriptor, V3SectionType, body_digest_for_v3_sections, digest_v3_section,
    generate_v3_commit_key, parse_v3_commit_header, parse_v3_commit_object,
    v3_commit_header_span_len, validate_commit_section_semantics, validate_v3_commit_object_len,
};
use super::error::{V3FormatError, V3Result};
use super::format::{V3FormatRef, V3KeyringEnvelopeRootRef};
...
pub const DEFAULT_V3_REPLAY_MAX_COMMITS: usize = 4_096;
/// Maximum cumulative commit-object bytes verified during one recovery replay.
pub const DEFAULT_V3_REPLAY_MAX_TOTAL_COMMIT_BYTES: u64 = 1024 * 1024 * 1024 * 1024;
/// Maximum encrypted index bytes retained in memory during one recovery replay.
pub const DEFAULT_V3_REPLAY_MAX_RETAINED_BYTES: u64 = 64 * 1024 * 1024;
/// Maximum bytes held by one recovery range read.
pub const DEFAULT_V3_REPLAY_READ_CHUNK_BYTES: u64 = 8 * 1024 * 1024;

/// Schema marker for trusted v3 recovery bundles.
pub const V3_RESTORE_BUNDLE_SCHEMA: &str = "rs3.restore-bundle.v3-preview.v1";

/// Default idle time allowed between streaming request-body chunks.
pub const DEFAULT_V3_STREAM_READ_STALL_TIMEOUT: Duration = Duration::from_secs(30);
...
const STREAMING_UPLOAD_AUXILIARY_BYTES: u64 = V3_MAX_HEADER_SIZE as u64 + 4096;

/// Returns a conservative peak working set for one v3 multipart streaming encoder.
///
...
pub const fn v3_streaming_upload_working_set_bytes(
...
/// Accepted v3 commit anchor state.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3AnchorState {
    ...
    pub format_ref: V3FormatRef,
}

impl Serialize for V3AnchorState {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        V3AnchorStateWire::from(self).serialize(serializer)
    }
}

impl<'de> Deserialize<'de> for V3AnchorState {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        V3AnchorStateWire::deserialize(deserializer)?
            .try_into()
...
struct V3AnchorStateWire {
    ...
    format: V3FormatRefWire,
}

impl From<&V3AnchorState> for V3AnchorStateWire {
    fn from(anchor: &V3AnchorState) -> Self {
        Self {
            format_generation: super::V3_FORMAT_VERSION,
            ...
            format: V3FormatRefWire::from(&anchor.format_ref),
        }
    }
}

impl TryFrom<V3AnchorStateWire> for V3AnchorState {
    type Error = de::value::Error;

    fn try_from(wire: V3AnchorStateWire) -> Result<Self, Self::Error> {
        if wire.format_generation != super::V3_FORMAT_VERSION {
...
impl From<&V3FormatRef> for V3FormatRefWire {
    fn from(format_ref: &V3FormatRef) -> Self {
        Self {
...
impl TryFrom<V3FormatRefWire> for V3FormatRef {
    type Error = de::value::Error;

    fn try_from(wire: V3FormatRefWire) -> Result<Self, Self::Error> {
        Ok(Self {
...
/// Compare-and-swap anchor used by v3 writers.
#[async_trait]
pub trait V3CommitAnchor: Send + Sync {
    /// Reads the current v3 anchor, returning `None` before genesis.
    async fn read_v3(&self) -> V3Result<Option<V3AnchorState>>;

    /// Advances the v3 anchor only if the current state equals `expected`.
...
/// In-memory v3 anchor for local tests and preview wiring.
#[derive(Clone, Debug, Default)]
pub struct V3MemoryAnchor {
    state: Arc<RwLock<Option<V3AnchorState>>>,
}

impl V3MemoryAnchor {
    /// Creates an empty in-memory v3 anchor.
    pub fn new() -> Self {
        Self::default()
    }

    /// Creates an in-memory v3 anchor with an initial state.
    pub fn with_state(state: V3AnchorState) -> Self {
        Self {
...
#[async_trait]
impl V3CommitAnchor for V3MemoryAnchor {
    async fn read_v3(&self) -> V3Result<Option<V3AnchorState>> {
        self.state
            .read()
            .map_err(|_| V3FormatError::AnchorReadFailed)
    ...
    async fn compare_and_advance_v3(
        ...
        let mut state = self
            .state
            .write()
            .map_err(|_| V3FormatError::AnchorAdvanceFailed)?;
...
/// Runtime options for the preview v3 commit store.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3CommitStoreOptions {
    ...
    pub recovery_maintenance_budgets: super::V3MaintenanceBudgets,
    ...
    pub maintenance_keyring_envelope_ref: Option<V3KeyringEnvelopeRootRef>,
    /// Resource budgets for startup and disaster-recovery replay.
    pub replay_limits: V3ReplayLimits,
}

impl V3CommitStoreOptions {
    ...
    pub fn for_profile(
        ...
        Self {
            ...
            recovery_maintenance_budgets: super::V3MaintenanceBudgets::default(),
            provider_profile: profile,
            stream_read_stall_timeout: DEFAULT_V3_STREAM_READ_STALL_TIMEOUT,
            retention: match profile {
                V3ProviderProfile::RetainedVersionObjectLock => Some(RetentionPolicy::new(
                    rs3_types::RetentionMode::Governance,
                    1,
                )),
                V3ProviderProfile::Dev | V3ProviderProfile::AtomicCreate => None,
            },
            legal_hold: None,
            keyring_envelope_ref,
            format_ref,
            maintenance_keyring_envelope_ref: None,
            replay_limits: V3ReplayLimits::default(),
        }
    }
    ...
    pub const fn with_recovery_maintenance_budgets(
        mut self,
        budgets: super::V3MaintenanceBudgets,
    ) -> Self {
    ...
    pub fn with_maintenance_keyring_envelope_ref(
        mut self,
        keyring_envelope_ref: V3KeyringEnvelopeRootRef,
    ) -> Self {
    ...
    pub const fn with_replay_limits(mut self, replay_limits: V3ReplayLimits) -> Self {
...
impl Default for V3ReplayLimits {
    fn default() -> Self {
        Self {
            max_commits: DEFAULT_V3_REPLAY_MAX_COMMITS,
            max_total_commit_bytes: DEFAULT_V3_REPLAY_MAX_TOTAL_COMMIT_BYTES,
            max_retained_bytes: DEFAULT_V3_REPLAY_MAX_RETAINED_BYTES,
            read_chunk_bytes: DEFAULT_V3_REPLAY_READ_CHUNK_BYTES,
        }
    }
}

/// Opaque section bytes included in a v3 commit.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3CommitSection {
    /// Section type.
...
impl V3CommitSection {
    /// Creates a section from raw bytes.
    pub fn new(section_type: V3SectionType, flags: u8, bytes: Bytes) -> Self {
        Self {
            section_type,
            flags,
            bytes,
...
impl V3CommitWrite {
    ...
    pub fn snapshot(sections: Vec<V3CommitSection>) -> Self {
        Self {
    ...
    pub fn delta(sections: Vec<V3CommitSection>) -> Self {
        Self {
            kind: V3CommitKind::Delta,
            sections,
            retention: None,
            legal_hold: None,
            required_retain_until_ms: None,
...
pub(crate) struct V3StoredStandalonePayload {
...
pub(crate) struct V3StandalonePayloadWrite<St> {
    pub(crate) object_id: BackendObjectId,
    ...
    pub(crate) cancellation: Arc<V3StandaloneUploadCancellation>,
}

#[derive(Clone, Copy)]
struct V3WritePostconditions {
...
#[derive(Clone, Debug)]
pub(super) struct V3PublicationPlan {
    pub(super) sampled_now_ms: i64,
    pub(super) parent: V3AnchorState,
    pub(super) parent_publish_time_ms: i64,
    pub(super) publish_time_ms: i64,
}

/// Result of a v3 commit write accepted by the anchor.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3StoredCommit {
...
pub(super) fn metadata_section_buffer_bytes(length: u64, read_chunk: u64) -> V3Result<u64> {
    if read_chunk == 0 {
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3ReplayChain {
    /// Verified commits, newest first, ending at the nearest snapshot or genesis.
    pub commits_newest_first: Vec<V3ReplayCommit>,
}

/// Recovery bundle used as the weak-subjectivity floor for v3 DR.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3RecoveryBundle {
...
impl Serialize for V3RecoveryBundle {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        ...
        let repository =
            self.repository_id
                .as_ref()
                .map(|repository_id| V3RecoveryBundleRepositoryWire {
        ...
        let wire = V3RecoveryBundleWire {
            schema: V3_RESTORE_BUNDLE_SCHEMA.to_owned(),
            repository,
            anchor: V3AnchorStateWire::from(&self.anchor),
...
#[derive(Serialize)]
struct V3RecoveryBundleWire {
    schema: String,
    #[serde(default)]
    repository: Option<V3RecoveryBundleRepositoryWire>,
    anchor: V3AnchorStateWire,
...
#[derive(Serialize)]
struct V3RecoveryBundleRepositoryWire {
    id: String,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    salt_digest: Option<String>,
}

impl V3RecoveryBundle {
    /// Creates a recovery bundle from an accepted anchor state.
    pub fn from_anchor(anchor: V3AnchorState, floor: Sequence) -> Self {
        Self {
            repository_id: None,
    ...
    pub fn verify_offline_signature(&self, public_key: &str) -> V3Result<()> {
        let signature = self
            .offline_signature
            .as_deref()
            .ok_or(V3FormatError::RecoveryBundleRequired)?;
...
/// Preview v3 commit store over a `BlobStore`.
pub struct V3CommitStore<S> {
    store: S,
    keyring: KeyRing,
    options: V3CommitStoreOptions,
    inflight_standalone_objects: Arc<RwLock<BTreeSet<BackendObjectId>>>,
    verified_publication_times: Arc<RwLock<VecDeque<(V3AnchorState, i64, bool)>>>,
    pub(in crate::v3) recovery_coverage:
        Arc<RwLock<Option<super::maintenance::coverage::RecoveryCoverage>>>,
    pub(in crate::v3) recovery_capacity:
        Arc<RwLock<Option<super::maintenance::capacity::RecoveryCapacity>>>,
    #[cfg(test)]
    publication_time_override: Arc<std::sync::atomic::AtomicI64>,
}

pub(crate) struct V3InflightStandaloneObject {
    object_id: BackendObjectId,
    roots: Arc<RwLock<BTreeSet<BackendObjectId>>>,
}

pub(crate) struct V3StandaloneUploadCancellation {
    cancelled: AtomicBool,
    notify: tokio::sync::Notify,
}

impl V3StandaloneUploadCancellation {
...
impl Drop for V3InflightStandaloneObject {
    fn drop(&mut self) {
...
where
    S: BlobStore,
{
    /// Creates a v3 commit store.
    pub fn new(store: S, keyring: KeyRing, options: V3CommitStoreOptions) -> Self {
        Self {
            store,
            keyring,
            options,
    ...
    async fn verified_parent_header_facts(&self, anchor: &V3AnchorState) -> V3Result<(i64, bool)> {
        {
            let mut cache = self
                .verified_publication_times
                .write()
    ...
    pub(crate) fn claim_inflight_standalone_object(
        &self,
        object_id: BackendObjectId,
    ) -> V3Result<V3InflightStandaloneObject> {
        ...
        // V3MaintenanceGuard quiescence, so it cannot race the single writer.
        let mut roots = self
    ...
    pub(super) fn rebind_store<T>(&self, store: T) -> V3CommitStore<T> {
        V3CommitStore {
            store,
    ...
    pub(super) const fn provider_profile(&self) -> V3ProviderProfile {
    ...
    pub(super) const fn options(&self) -> &V3CommitStoreOptions {
        &self.options
    }
    ...
    fn validate_write_protection_profile(
        ...
        if active_retention
            && self.options.provider_profile != V3ProviderProfile::RetainedVersionObjectLock
        {
            return Err(V3FormatError::ProviderProfileFailed);
        }
        Ok(())
    ...
    pub async fn write_genesis_snapshot<A>(&self, anchor: &A) -> V3Result<V3StoredCommit>
    where
        A: V3CommitAnchor,
    {
        ...
        A: V3CommitAnchor,
    {
        ...
        let Some(current) = current else {
            return Err(V3FormatError::MissingAnchor);
        };
    ...
    where
        A: V3CommitAnchor,
        F: FnMut(&V3CommitKey) -> V3Result<V3CommitWrite>,
    {
        if self.options.recovery_policy.is_some() {
            return Err(V3FormatError::InvalidRecoveryHistory);
        }
    ...
    pub(super) async fn prepare_child_publication(
        &self,
        parent: &V3AnchorState,
    ) -> V3Result<V3PublicationPlan> {
    ...
    where
        ...
        F: FnMut(&V3CommitKey) -> V3Result<V3CommitWrite>,
    {
        if anchor.read_v3().await?.as_ref() != Some(&plan.parent)
    ...
    pub async fn read_anchor_head<A>(&self, anchor: &A) -> V3Result<Option<V3ReplayCommit>>
    where
        A: V3CommitAnchor,
    {
        let Some(state) = anchor.read_v3().await? else {
            return Ok(None);
        };
    ...
    #[cfg(test)]
    pub(super) async fn load_chain_from_anchor<A: V3CommitAnchor>(
        &self,
        anchor: &A,
    ) -> V3Result<Option<V3CommitChain>> {
        ...
        for commit in chain.commits_newest_first {
            ...
            let body = self
                ...
                .await
                .map_err(|_| V3FormatError::StorageOperationFailed)?;
            let mut parsed = parse_v3_commit_object(key, body, &self.keyring)?;
            parsed.version_id = commit.version_id;
            commits.push(parsed);
        }
        Ok(Some(V3CommitChain {
    ...
    pub async fn load_replay_chain_from_anchor<A>(
        &self,
        anchor: &A,
    ) -> V3Result<Option<V3ReplayChain>>
    where
        A: V3CommitAnchor,
    {
        let Some(anchor_state) = anchor.read_v3().await? else {
            return Ok(None);
        };
    ...
    pub(super) async fn load_replay_chain_with_pending_limit(
        ...
        loop {
            ...
            total_commit_bytes = total_commit_bytes
                ...
                .ok_or(V3FormatError::ReplayBudgetExceeded)?;
            ...
            validate_v3_commit_object_len(&parsed_header, metadata.content_len)?;
            ...
            let retained_sections = self
                ...
                .await?;
            let is_root = parsed_header.header.kind == V3CommitKind::Root;
            let parent = parsed_header.header.parent.clone();
            commits.push(V3ReplayCommit {
                parsed_header,
        ...
        let head = commits.first().ok_or(V3FormatError::MissingAnchor)?;
    ...
    async fn verify_replay_sections(
        ...
        for (index, section) in parsed_header.header.section_index.iter().enumerate() {
            ...
            if section.section_type == V3SectionType::Recovery
            ...
            *retained_bytes = retained_bytes
            ...
            let capacity =
                usize::try_from(section.length).map_err(|_| V3FormatError::ReplayBudgetExceeded)?;
    ...
    pub(super) async fn read_metadata_section(
        ...
        let format_limit = match descriptor.section_type {
            V3SectionType::IndexRun => super::index_run::V3_INDEX_RUN_MAX_OBJECT_BYTES,
            V3SectionType::IndexRoot => super::index_root::V3_INDEX_ROOT_MAX_BYTES,
            V3SectionType::Recovery => super::recovery::section::MAX_RECOVERY_SECTION_BYTES,
        ...
        let capacity = usize::try_from(remaining).map_err(|_| V3FormatError::SectionBounds)?;
    ...
    pub(crate) async fn read_commit_header_at(
        ...
        if self.options.provider_profile == V3ProviderProfile::RetainedVersionObjectLock
            && version_id.is_none()
        {
            return Err(V3FormatError::InvalidHeaderField);
        }
        ...
        let prefix = self
            ...
            .get_range_at(
                ...
                ByteRange::Slice {
                    offset: 0,
                    len: V3_HEADER_META_LEN as u64,
                },
            )
            .await
            .map_err(|_| V3FormatError::StorageOperationFailed)?;
        let header_span_len = v3_commit_header_span_len(&prefix)?;
        let header_bytes = if header_span_len == V3_HEADER_META_LEN {
            prefix
        } else {
            let remaining_len = header_span_len
                .checked_sub(V3_HEADER_META_LEN)
                .ok_or(V3FormatError::HeaderTooLarge)?;
            let remaining = self
                .store
                .get_range_at(
                    object_id,
                    version_id,
                    ByteRange::Slice {
                        offset: V3_HEADER_META_LEN as u64,
        ...
        let parsed = parse_v3_commit_header(object_id, &header_bytes, &self.keyring)?;
    ...
    where
        A: V3CommitAnchor,
    {
        let Some(current) = anchor.read_v3().await? else {
            return Err(V3FormatError::MissingAnchor);
        };
        ...
        let verified_retain_until_ms = self
            ...
            .await?;
        let commit_key = V3CommitKey::parse(object_id)?;
        ...
        let sections_start = u64::try_from(parsed.parsed_header.sections_start)
            .map_err(|_| V3FormatError::SectionBounds)?;
        let anchor_state = V3AnchorState {
        ...
        anchor
            .compare_and_advance_v3(Some(&current), anchor_state.clone())
            .await?;
        Ok(V3StoredCommit {
        ...
        A: V3CommitAnchor,
    {
        ...
        let advance = anchor
            .compare_and_advance_v3(Some(expected_parent), uploaded.anchor_state.clone())
            .await;
        match advance {
            ...
            result => {
                ...
                match anchor.fence_and_read_v3().await {
                    ...
                    _ => return Err(V3FormatError::AnchorReconciliationRequired),
                }
            }
        }
        ...
        A: V3CommitAnchor,
    {
        ...
        {
            return Err(V3FormatError::RecoveryBundleRequired);
        }
        ...
        anchor
            .compare_and_advance_v3(None, bundle.anchor.clone())
            .await?;
        Ok(chain)
    }
    ...
    where
        ...
        F: FnMut(&V3CommitKey) -> V3Result<V3CommitWrite>,
    {
        let parent = expected.as_ref().map(|state| V3CommitParentRef {
        ...
        for _ in 0..MAX_RANDOM_KEY_ATTEMPTS {
            let commit_key = generate_v3_commit_key(sequence)?;
            ...
            let object_len =
                u64::try_from(object_body.len()).map_err(|_| V3FormatError::SectionBounds)?;
            ...
            anchor
                .compare_and_advance_v3(expected.as_ref(), anchor_state.clone())
                .await?;
            return Ok(V3StoredCommit {
        ...
        if last_collision {
            Err(V3FormatError::RandomnessUnavailable)
        } else {
            Err(V3FormatError::StorageOperationFailed)
        }
    }
    ...
    fn build_header(
        ...
        write: &V3CommitWrite,
        section_index: Vec<V3SectionDescriptor>,
        body_digest: [u8; 32],
        publish_time_ms: i64,
    ) -> V3Result<V3CommitHeader> {
    ...
    pub(crate) async fn write_standalone_streaming_payload<St>(
        &self,
        write: V3StandalonePayloadWrite<St>,
    ) -> V3Result<V3StoredStandalonePayload>
    where
        St: Stream<Item = crate::Result<Bytes>> + Unpin + Send,
    {
        let V3StandalonePayloadWrite {
            object_id,
        ...
        if cancellation.is_cancelled() {
            return Err(V3FormatError::ObjectBodyReadFailed);
        }
        ...
        let mut multipart = self
            ...
            .await
            .map_err(storage_to_v3)?;
        let mut object_digest = Sha256Hasher::new();
        if cancellation.is_cancelled() {
            abort_v3_commit_multipart(multipart, "standalone_cancelled").await;
            return Err(V3FormatError::ObjectBodyReadFailed);
        }
        ...
        loop {
            let next_chunk = match tokio::select! {
                ...
                Err(_elapsed) => {
                    abort_v3_commit_multipart(multipart, "standalone_stream_timeout").await;
                    return Err(V3FormatError::ObjectBodyReadFailed);
                }
            };
            ...
            let chunk = match chunk {
                ...
                Err(crate::RepositoryError::ObjectBodyReadFailed) => {
                    abort_v3_commit_multipart(multipart, "standalone_stream_read").await;
                    return Err(V3FormatError::ObjectBodyReadFailed);
                }
                Err(_error) => {
                    abort_v3_commit_multipart(multipart, "standalone_stream_read").await;
                    return Err(V3FormatError::StorageOperationFailed);
                }
            };
            plaintext_seen = plaintext_seen
                .checked_add(u64::try_from(chunk.len()).map_err(|_| V3FormatError::SectionBounds)?)
                .ok_or(V3FormatError::SectionBounds)?;
            if plaintext_seen > max_plaintext_len {
                abort_v3_commit_multipart(multipart, "standalone_plaintext_length").await;
                return Err(if expected_plaintext_len.is_some() {
                    V3FormatError::ObjectLengthMismatch
                } else {
                    V3FormatError::ObjectTooLarge
                });
            }
            ...
            while !remaining.is_empty() {
                ...
                if segment.len() == payload_segment_size {
                    if let Some((ready_index, ready_segment)) = pending_segment.take()
                        ...
                        .is_err()
                    {
                        abort_v3_commit_multipart(multipart, "standalone_payload_segment").await;
                        return Err(V3FormatError::StorageOperationFailed);
                    }
        ...
        if expected_plaintext_len.is_some_and(|expected| plaintext_seen != expected) {
            abort_v3_commit_multipart(multipart, "standalone_plaintext_length").await;
            return Err(V3FormatError::ObjectLengthMismatch);
        }
        if !segment.is_empty() {
            if let Some((ready_index, ready_segment)) = pending_segment.take()
                && push_standalone_payload_segment(
        ...
        if assembler.flush_final_part(&mut multipart).await.is_err() {
            abort_v3_commit_multipart(multipart, "standalone_final_part").await;
            return Err(V3FormatError::StorageOperationFailed);
        }
        if cancellation.is_cancelled() {
            abort_v3_commit_multipart(multipart, "standalone_cancelled").await;
        ...
        let metadata = assembler.complete(multipart).await.map_err(storage_to_v3)?;
        if metadata.object_id != object_id || metadata.content_len != object_len {
            return Err(V3FormatError::ProviderProfileFailed);
        }
        let retention_version = match self.options.provider_profile {
            V3ProviderProfile::RetainedVersionObjectLock => Some(
                metadata
                    .version_id
                    .as_ref()
                    .ok_or(V3FormatError::ProviderProfileFailed)?,
            ),
            V3ProviderProfile::Dev | V3ProviderProfile::AtomicCreate => {
        ...
        if required_retain_until_ms.is_some() {
            let retention = retention.ok_or(V3FormatError::ProviderProfileFailed)?;
            self.store
        ...
        self.store
            .put(
                ...
                PutOptions {
                    retention,
                    legal_hold,
                    content_type: Some(V3_COMMIT_CONTENT_TYPE.to_owned()),
                    do_not_recreate: self.options.provider_profile
                        != V3ProviderProfile::RetainedVersionObjectLock,
                },
            )
    ...
    async fn verify_commit_postconditions(
        ...
        postconditions: V3WritePostconditions,
    ) -> V3Result<BlobMetadata> {
        let exact = self
        ...
        if let Some(expected_digest) = postconditions.expected_stored_digest {
            ...
            let visible = self
                ...
                .await
                .map_err(|_| V3FormatError::ProviderProfileFailed)?;
            if visible.len() != 1 {
                return Err(V3FormatError::ProviderProfileFailed);
            }
        }
    ...
    async fn verify_commit_protection_postconditions(
        ...
        match self.options.provider_profile {
            V3ProviderProfile::Dev | V3ProviderProfile::AtomicCreate => Ok(exact),
            V3ProviderProfile::RetainedVersionObjectLock => {
    ...
    async fn verify_exact_stored_object_digest(
        ...
        {
            let chunk_len =
                u64::try_from(bytes.len()).map_err(|_| V3FormatError::ProviderProfileFailed)?;
    ...
    async fn verify_existing_commit_postconditions(
        ...
        match self.options.provider_profile {
            V3ProviderProfile::Dev | V3ProviderProfile::AtomicCreate => Ok(None),
            V3ProviderProfile::RetainedVersionObjectLock => {
...
fn build_section_region(
    sections: &[V3CommitSection],
) -> V3Result<(Vec<V3SectionDescriptor>, Bytes)> {
    let mut section_index = Vec::with_capacity(sections.len());
    let mut region = Vec::new();
    for section in sections {
        let offset = u64::try_from(region.len()).map_err(|_| V3FormatError::SectionBounds)?;
        region.extend_from_slice(&section.bytes);
        let length =
            u64::try_from(section.bytes.len()).map_err(|_| V3FormatError::SectionBounds)?;
        section_index.push(V3SectionDescriptor {
            section_type: section.section_type,
            offset,
            length,
            flags: section.flags,
            digest: digest_v3_section(&section.bytes),
        });
    }
    Ok((section_index, Bytes::from(region)))
}

async fn abort_v3_commit_multipart(multipart: Box<dyn BlobMultipartUpload>, phase: &'static str) {
...
fn storage_to_v3(_error: StorageError) -> V3FormatError {
...
impl MultipartObjectAssembler {
    fn new(part_size: usize) -> V3Result<Self> {
        if part_size == 0 {
