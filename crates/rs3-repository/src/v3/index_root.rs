...
use super::{V3FormatError, V3FormatRef, V3KeyringEnvelopeRef, V3Result};
use bytes::Bytes;
...
pub const V3_INDEX_ROOT_ID_LEN: usize = 32;
/// Maximum complete encrypted index-root section size.
pub const V3_INDEX_ROOT_MAX_BYTES: usize = 8 * 1024 * 1024;
/// Maximum active embedded run references in one root.
pub const V3_INDEX_ROOT_MAX_RUNS: usize = 1_024;
/// Maximum cumulative mutations claimed by one root.
pub const V3_INDEX_ROOT_MAX_TOTAL_MUTATIONS: u64 = 16 * 1024 * 1024;
/// Maximum cumulative encrypted run bytes claimed by one root.
pub const V3_INDEX_ROOT_MAX_TOTAL_RUN_BYTES: u64 = 8 * 1024 * 1024 * 1024;
/// Maximum storage tier accepted in one index-root run reference.
pub const V3_INDEX_ROOT_MAX_LEVEL: u16 = 1;
/// Fixed public-envelope bytes required before the metadata key identifier.
pub const V3_INDEX_ROOT_FIXED_HEADER_BYTES: usize = 72;
...
pub struct V3IndexRootId([u8; V3_INDEX_ROOT_ID_LEN]);

impl V3IndexRootId {
    /// Returns the exact random identity bytes.
    #[must_use]
    pub const fn as_bytes(&self) -> &[u8; V3_INDEX_ROOT_ID_LEN] {
        &self.0
    }

    fn generate() -> V3Result<Self> {
        let bytes =
...
pub struct V3EmbeddedIndexRunLocation {
...
pub struct V3IndexRootRunRef {
    ...
    pub location: V3EmbeddedIndexRunLocation,
}
...
impl V3IndexRoot {
    ...
    pub fn new(
        ...
        format_ref: V3FormatRef,
        keyring_envelope_ref: V3KeyringEnvelopeRef,
        mut runs: Vec<V3IndexRootRunRef>,
    ) -> V3Result<Self> {
    ...
    #[must_use]
    pub const fn format_ref(&self) -> &V3FormatRef {
        &self.format_ref
    }

    /// Returns the active keyring-envelope reference.
    #[must_use]
    pub const fn keyring_envelope_ref(&self) -> &V3KeyringEnvelopeRef {
        &self.keyring_envelope_ref
    }

    /// Returns the recomputed bounded resource claims.
    #[must_use]
    pub const fn claims(&self) -> V3IndexRootClaims {
        self.claims
    }

    /// Returns active run references in canonical run-ID order.
    #[must_use]
    pub fn runs(&self) -> &[V3IndexRootRunRef] {
        &self.runs
    }
}
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3SealedIndexRoot {
    root_id: V3IndexRootId,
    bytes: Bytes,
}

impl V3SealedIndexRoot {
    /// Returns the random identity authenticated by this root envelope.
    #[must_use]
    pub const fn root_id(&self) -> V3IndexRootId {
        self.root_id
    }
...
pub fn seal_v3_index_root(
    keyring: &KeyRing,
    repository_context: &[u8],
    containing_object: &BackendObjectId,
    section_ordinal: u32,
    root: &V3IndexRoot,
) -> V3Result<V3SealedIndexRoot> {
    validate_context(repository_context, containing_object)?;
    validate_root(root)?;
    let plaintext = encode_root(root)?;
    if plaintext.len() > V3_INDEX_ROOT_MAX_BYTES {
        return Err(V3FormatError::IndexRootLimitExceeded);
    }
    ...
    let header_len = V3_INDEX_ROOT_FIXED_HEADER_BYTES
        .checked_add(key_id_len)
        .ok_or(V3FormatError::IndexRootLimitExceeded)?;
    let stored_len = header_len
        .checked_add(INDEX_ROOT_SEAL_OVERHEAD)
        .and_then(|value| value.checked_add(plaintext.len()))
        .ok_or(V3FormatError::IndexRootLimitExceeded)?;
    if stored_len > V3_INDEX_ROOT_MAX_BYTES {
        return Err(V3FormatError::IndexRootLimitExceeded);
    }
    let root_id = V3IndexRootId::generate()?;
...
pub fn open_v3_index_root(
    ...
    let ciphertext_start =
        usize::try_from(header.header_len).map_err(|_| V3FormatError::InvalidIndexRoot)?;
...
#[cfg(feature = "fuzzing")]
pub(crate) fn decode_v3_index_root_plaintext_for_fuzzing(input: &[u8]) -> V3Result<()> {
...
#[cfg(feature = "fuzzing")]
pub(crate) fn encode_v3_index_root_plaintext_for_fuzzing(root: &V3IndexRoot) -> V3Result<Vec<u8>> {
    encode_root(root)
}

fn validate_root(root: &V3IndexRoot) -> V3Result<()> {
...
fn validate_runs(
    covered_generation: Sequence,
    expected_live_object_count: u64,
    runs: &[V3IndexRootRunRef],
) -> V3Result<V3IndexRootClaims> {
    if runs.len() > V3_INDEX_ROOT_MAX_RUNS {
        return Err(V3FormatError::IndexRootLimitExceeded);
    }
    ...
    for run in runs {
        if previous_id.is_some_and(|previous| previous >= run.run_id)
            ...
            || run.level > V3_INDEX_ROOT_MAX_LEVEL
    ...
    if total_stored_run_bytes > V3_INDEX_ROOT_MAX_TOTAL_RUN_BYTES
        || total_mutation_count > V3_INDEX_ROOT_MAX_TOTAL_MUTATIONS
...
fn validate_run_location(location: &V3EmbeddedIndexRunLocation) -> V3Result<()> {
    validate_len(
...
fn validate_format_ref(reference: &V3FormatRef) -> V3Result<()> {
    validate_len(
    ...
    let digest = hex::decode(&reference.digest).map_err(|_| V3FormatError::InvalidIndexRoot)?;
...
fn validate_keyring_ref(reference: &V3KeyringEnvelopeRef) -> V3Result<()> {
    validate_len(
        reference.object_id.as_str().len(),
        INDEX_ROOT_MAX_OBJECT_ID_LEN,
    )
...
fn validate_len(actual: usize, maximum: usize) -> V3Result<()> {
    if actual == 0 || actual > maximum {
        Err(V3FormatError::IndexRootLimitExceeded)
    } else {
        Ok(())
    }
}

fn encode_root(root: &V3IndexRoot) -> V3Result<Vec<u8>> {
    ...
    let format_digest =
        hex::decode(&root.format_ref.digest).map_err(|_| V3FormatError::InvalidIndexRoot)?;
    ...
    if output.len() > V3_INDEX_ROOT_MAX_BYTES {
        return Err(V3FormatError::IndexRootLimitExceeded);
    }
    Ok(output)
}

fn encode_run_ref(run: &V3IndexRootRunRef) -> V3Result<Vec<u8>> {
    let mut output = Vec::new();
...
fn decode_root(input: &[u8]) -> V3Result<V3IndexRoot> {
    if input.len() > V3_INDEX_ROOT_MAX_BYTES {
        return Err(V3FormatError::IndexRootLimitExceeded);
    }
    ...
    let run_count =
        usize::try_from(claims.run_count).map_err(|_| V3FormatError::IndexRootLimitExceeded)?;
    if run_count > V3_INDEX_ROOT_MAX_RUNS {
        return Err(V3FormatError::IndexRootLimitExceeded);
    }
    ...
    for _ in 0..run_count {
        let record_len =
            usize::try_from(reader.u32()?).map_err(|_| V3FormatError::IndexRootLimitExceeded)?;
...
fn decode_run_ref(input: &[u8]) -> V3Result<V3IndexRootRunRef> {
    let mut reader = Reader::new(input);
    ...
    let location = V3EmbeddedIndexRunLocation {
...
fn encode_header(output: &mut Vec<u8>, header: &RootHeader) -> V3Result<()> {
    ...
    if output.len() != V3_INDEX_ROOT_FIXED_HEADER_BYTES {
        return Err(V3FormatError::InvalidIndexRoot);
    }
...
fn parse_header(input: &[u8]) -> V3Result<RootHeader> {
    if input.len() < V3_INDEX_ROOT_FIXED_HEADER_BYTES || input.len() > V3_INDEX_ROOT_MAX_BYTES {
        return Err(V3FormatError::InvalidIndexRoot);
    }
    ...
    let expected_header_len = V3_INDEX_ROOT_FIXED_HEADER_BYTES
        .checked_add(key_id_len)
        .ok_or(V3FormatError::InvalidIndexRoot)?;
    ...
    let key_bytes = input
        .get(V3_INDEX_ROOT_FIXED_HEADER_BYTES..expected_header_len)
        .ok_or(V3FormatError::InvalidIndexRoot)?;
    let key_id =
        KeyId::new(std::str::from_utf8(key_bytes).map_err(|_| V3FormatError::InvalidIndexRoot)?)?;
    Ok(RootHeader {
        header_len,
        stored_len,
        section_ordinal,
...
fn push_optional_string(output: &mut Vec<u8>, value: Option<&str>) -> V3Result<()> {
    match value {
        None => output.push(0),
        Some(value) => {
            output.push(1);
...
fn push_string(output: &mut Vec<u8>, value: &str) -> V3Result<()> {
...
fn read_u16(input: &[u8], offset: usize) -> V3Result<u16> {
...
fn read_u32(input: &[u8], offset: usize) -> V3Result<u32> {
...
fn read_u64(input: &[u8], offset: usize) -> V3Result<u64> {
...
fn to_u16(value: usize) -> V3Result<u16> {
    u16::try_from(value).map_err(|_| V3FormatError::IndexRootLimitExceeded)
}

fn to_u32<T>(value: T) -> V3Result<u32>
where
    T: TryInto<u32>,
{
    value
...
fn to_u64<T>(value: T) -> V3Result<u64>
where
    T: TryInto<u64>,
{
    value
...
impl<'a> Reader<'a> {
    ...
    fn take(&mut self, len: usize) -> V3Result<&'a [u8]> {
        let end = self
            .offset
            .checked_add(len)
            .ok_or(V3FormatError::InvalidIndexRoot)?;
    ...
    fn array_32(&mut self) -> V3Result<[u8; 32]> {
        self.take(32)?
            .try_into()
            .map_err(|_| V3FormatError::InvalidIndexRoot)
    }

    fn string(&mut self, maximum: usize) -> V3Result<String> {
        let len = usize::from(self.u16()?);
        if len == 0 || len > maximum {
            return Err(V3FormatError::IndexRootLimitExceeded);
        }
        let value =
            std::str::from_utf8(self.take(len)?).map_err(|_| V3FormatError::InvalidIndexRoot)?;
        Ok(value.to_owned())
    }

    fn optional_string(&mut self, maximum: usize) -> V3Result<Option<String>> {
        match self.take(1)?[0] {
            0 => Ok(None),
            1 => self.string(maximum).map(Some),
            _ => Err(V3FormatError::InvalidIndexRoot),
        }
    }

    fn finish(self) -> V3Result<()> {
        if self.offset == self.input.len() {
            Ok(())
        } else {
            Err(V3FormatError::InvalidIndexRoot)
        }
    }
}
...
mod tests {
    use super::{
        V3_INDEX_ROOT_FIXED_HEADER_BYTES, V3_INDEX_ROOT_MAX_LEVEL, V3_INDEX_ROOT_MAX_RUNS,
        V3EmbeddedIndexRunLocation, V3IndexRoot, V3IndexRootRunRef, open_v3_index_root,
        seal_v3_index_root,
    };
    use crate::v3::{V3FormatError, V3FormatRef, V3KeyringEnvelopeRef};
    ...
    fn must<T>(result: crate::v3::V3Result<T>) -> T {
    ...
    fn keyring_ref() -> V3KeyringEnvelopeRef {
    ...
    fn run_ref(id: u8, sequence: u64, path: &str) -> V3IndexRootRunRef {
        V3IndexRootRunRef {
            run_id: [id; 32],
    ...
    fn fixture() -> V3IndexRoot {
        must(V3IndexRoot::new(
            Sequence::new(9),
            2,
            format_ref(),
            keyring_ref(),
    ...
    fn encrypted_root_round_trips_and_sorts_runs() {
        ...
        let sealed = must(seal_v3_index_root(
            &keyring,
            REPOSITORY_CONTEXT,
            &object,
            0,
        ...
        let opened = must(open_v3_index_root(
            &keyring,
            REPOSITORY_CONTEXT,
            &object,
            0,
    ...
    fn completion_fixture() -> V3IndexRoot {
    ...
    fn fuzz_plaintext_decoder_accepts_current_canonical_root() {
        let encoded = must(super::encode_root(&fixture()));
        must(super::decode_v3_index_root_plaintext_for_fuzzing(&encoded));
    }
    ...
    fn compacted_levels_round_trip_canonically() {
        ...
        assert_eq!(root.claims().maximum_level(), V3_INDEX_ROOT_MAX_LEVEL);
        let sealed = must(seal_v3_index_root(
            &keyring,
            REPOSITORY_CONTEXT,
            &object,
    ...
    fn rejects_context_object_ordinal_and_ciphertext_transplants() {
        ...
        assert!(matches!(
            open_v3_index_root(&keyring, b"other-context", &object, 3, sealed.bytes()),
            Err(V3FormatError::CryptoOperation)
        ));
        assert!(matches!(
            open_v3_index_root(
                &keyring,
        ...
        assert!(matches!(
            open_v3_index_root(&keyring, REPOSITORY_CONTEXT, &object, 4, sealed.bytes()),
            Err(V3FormatError::InvalidIndexRoot)
        ));
        let mut tampered = sealed.bytes().to_vec();
        let last = tampered.len() - 1;
        tampered[last] ^= 1;
        assert!(open_v3_index_root(&keyring, REPOSITORY_CONTEXT, &object, 3, &tampered).is_err());
    }
    ...
    fn rejects_every_truncation_and_trailing_bytes() {
        ...
        trailing.push(0);
        assert!(open_v3_index_root(&keyring, REPOSITORY_CONTEXT, &object, 0, &trailing).is_err());
    }
    ...
    fn rejects_duplicate_runs_invalid_bounds_and_limits() {
        ...
        let too_many = (0..=V3_INDEX_ROOT_MAX_RUNS)
            .map(|index| {
            ...
            .collect();
        assert!(matches!(
            V3IndexRoot::new(Sequence::new(9), 1, format_ref(), keyring_ref(), too_many),
            Err(V3FormatError::IndexRootLimitExceeded)
        ));
    }
    ...
    fn rejects_invalid_level_and_compaction_generation_pairs() {
        ...
        level_above_limit.level = V3_INDEX_ROOT_MAX_LEVEL + 1;
    ...
    fn rejects_legacy_root_wire_version() {
        ...
        assert!(matches!(
            open_v3_index_root(&keyring, REPOSITORY_CONTEXT, &object, 0, &legacy),
            Err(V3FormatError::InvalidIndexRoot)
        ));
    }
    ...
    fn rejects_noncanonical_run_order() {
        ...
        root.runs.reverse();
        assert!(matches!(
            seal_v3_index_root(&keyring, REPOSITORY_CONTEXT, &object, 0, &root),
            Err(V3FormatError::InvalidIndexRoot)
        ));
    }
    ...
    fn rejects_fixed_header_tampering() {
        ...
        let mut tampered = sealed.bytes().to_vec();
        tampered[V3_INDEX_ROOT_FIXED_HEADER_BYTES - 1] ^= 1;
        assert!(open_v3_index_root(&keyring, REPOSITORY_CONTEXT, &object, 0, &tampered).is_err());
    }
}
