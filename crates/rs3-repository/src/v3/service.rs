//! v3 repository operations over the existing trusted namespace service.

use super::V3MaintenanceGuard;
use super::commit::{V3_SECTION_FLAG_COMPRESSED, V3_SECTION_FLAG_MUST_UNDERSTAND};
use super::error::V3FormatError;
use super::repository::{
    V3CommitAnchor, V3CommitSection, V3CommitStore, V3CommitStoreOptions, V3CommitWrite,
    V3MemoryAnchor, V3ReplayChain, V3ReplayCommit, V3StandalonePayloadWrite,
    V3StandaloneUploadCancellation, V3StoredCommit, V3StoredStandalonePayload,
};
use super::{
    V3_INDEX_ROOT_MAX_RUNS, V3EmbeddedIndexRunLocation, V3IndexRoot, V3IndexRootRunRef,
    V3KeyringEnvelopeRef, V3PayloadPackFacts, V3PayloadPackId, V3PayloadPackRecordContext,
    V3PayloadPackRecordRef, V3ProviderProfile, V3SectionType, V3StreamPayloadCacheIdentity,
    V3StreamPayloadCarrierCacheIdentity, digest_v3_section,
    open_v3_payload_pack_cached_record_span, open_v3_payload_pack_record_span_with_segments,
    plan_v3_payload_pack_record_range, seal_v3_index_root,
};
...
use rs3_index::{
    IndexDelta, NamespaceEntry, PayloadLayout, PayloadReference, V3StandaloneStreamCarrierReference,
};
...
pub use recovery_view::{
    V3RecoveryCursor, V3RecoveryPointInfo, V3RecoveryPointPage, V3RecoveryView,
};
mod staging;

use super::recovery::publication::AcceptedRecoveryState;

use super::standalone::{generate_v3_standalone_object_id, validate_v3_standalone_object};
pub use compaction::V3FullMaintenanceReport;
pub use read_stream::V3AuthenticatedReadBody;
use staging::{PendingV3Checkpoint, PendingV3Snapshot, PendingV3State};

const V3_PAYLOAD_FILL_LOCK_STRIPES: usize = 64;

/// Preview v3 repository service.
///
...
/// semantics. v3 only changes how the pending sealed index delta becomes
/// durable: it is embedded in a signed v3 commit instead of a v1 checkpoint.
pub struct V3Repository<S> {
    repository: RepositoryResources,
    commit_store: V3CommitStore<S>,
    accepted: StdRwLock<V3AcceptedState>,
    mutation_lock: Mutex<()>,
    publication_lock: TokioRwLock<()>,
    payload_segment_fill_locks: Vec<Mutex<()>>,
    pending: StdMutex<PendingV3State>,
    mutation_owner: Arc<AtomicU8>,
    recovery_required: AtomicBool,
    payload_sections: StdRwLock<V3PayloadSectionCache>,
...
const V3_MUTATION_OWNER_IDLE: u8 = 0;
const V3_MUTATION_OWNER_COORDINATOR: u8 = 1;
const V3_MUTATION_OWNER_DIRECT: u8 = 2;

pub(super) struct V3CoordinatorLease {
    owner: Arc<AtomicU8>,
}

pub(super) struct V3CoordinatedMutation<'a, A> {
    lease: &'a V3CoordinatorLease,
    anchor: &'a A,
    guard: Option<&'a dyn V3MaintenanceGuard>,
}

impl<'a, A> V3CoordinatedMutation<'a, A> {
    pub(super) fn with_guard(mut self, guard: Option<&'a dyn V3MaintenanceGuard>) -> Self {
        self.guard = guard;
        self
    }

    pub(super) fn new(lease: &'a V3CoordinatorLease, anchor: &'a A) -> Self {
        Self {
            lease,
            anchor,
            guard: None,
...
struct V3DirectMutationLease {
    owner: Arc<AtomicU8>,
}

impl Drop for V3CoordinatorLease {
    fn drop(&mut self) {
        self.owner.store(V3_MUTATION_OWNER_IDLE, Ordering::Release);
    }
}

impl Drop for V3DirectMutationLease {
    fn drop(&mut self) {
        self.owner.store(V3_MUTATION_OWNER_IDLE, Ordering::Release);
    }
}
...
#[derive(Default)]
struct V3AcceptedState {
    recovery: Option<AcceptedRecoveryState>,
    repository: RepositoryState,
    runs: Vec<V3IndexRootRunRef>,
    anchor: Option<super::repository::V3AnchorState>,
}

/// Immutable publication input captured before successor staging is admitted.
pub(crate) struct V3PendingPublication {
    snapshot: PendingV3Snapshot,
}

struct PendingV3Install {
    recovery: Option<AcceptedRecoveryState>,
    completion_receipts: Option<(Sequence, rs3_index::completion::CompletionReceipts)>,
    sequence: Sequence,
    mutations: Vec<PendingV3InstallMutation>,
    run: Option<V3IndexRootRunRef>,
}

enum PendingV3InstallMutation {
    Upsert {
...
#[derive(Clone, Debug)]
struct PendingV3Payload {
    manifest_id: ManifestId,
    body: Bytes,
}

#[derive(Debug)]
pub(crate) struct V3StagedPutRollback {
    checkpoint: PendingV3Checkpoint,
}
...
pub(super) struct V3StandalonePayloadUpload {
    stored: V3StoredStandalonePayload,
    _inflight: super::repository::V3InflightStandaloneObject,
}

/// Client-visible object resolved against an accepted v3 namespace state.
#[derive(Clone, Debug)]
pub struct V3ResolvedObject {
    metadata: RepositoryObjectMetadata,
    entry: NamespaceEntry,
}

impl V3ResolvedObject {
    ...
    S: BlobStore + Clone,
{
    ...
    /// Creates a v3 repository service over a blob store.
    pub fn new(
        store: S,
        keyring: KeyRing,
        repository_options: RepositoryOptions,
        commit_options: V3CommitStoreOptions,
    ) -> Self {
        ...
        Self {
            ...
            commit_store: V3CommitStore::new(store, keyring, commit_options),
            accepted: StdRwLock::new(V3AcceptedState::default()),
            mutation_lock: Mutex::new(()),
            publication_lock: TokioRwLock::new(()),
            payload_segment_fill_locks: (0..V3_PAYLOAD_FILL_LOCK_STRIPES)
                .map(|_| Mutex::new(()))
                .collect(),
            pending: StdMutex::new(PendingV3State::new(Sequence::ZERO)),
            mutation_owner: Arc::new(AtomicU8::new(V3_MUTATION_OWNER_IDLE)),
            recovery_required: AtomicBool::new(false),
            payload_sections: StdRwLock::new(V3PayloadSectionCache::with_max_bytes(
    ...
    /// Returns the v3 commit store.
    pub fn commit_store(&self) -> &V3CommitStore<S> {
        &self.commit_store
    }

    /// Writes the required empty v3 genesis snapshot.
    pub async fn write_genesis_snapshot<A>(&self, anchor: &A) -> Result<V3StoredCommit>
    where
        A: V3CommitAnchor,
    {
    ...
    pub async fn write_genesis_snapshot_with_guard<A>(
        &self,
        anchor: &A,
        guard: Option<&dyn V3MaintenanceGuard>,
    ) -> Result<V3StoredCommit>
    where
        A: V3CommitAnchor,
    {
    ...
    pub async fn write_index_snapshot<A>(&self, anchor: &A) -> Result<V3StoredCommit>
    where
        A: V3CommitAnchor,
    {
        let _mutation_lease = self.claim_direct_mutation()?;
    ...
    pub(super) async fn write_index_snapshot_coordinated<A>(
        &self,
        mutation: V3CoordinatedMutation<'_, A>,
    ) -> Result<V3StoredCommit>
    where
        A: V3CommitAnchor,
    {
        ...
        A: V3CommitAnchor,
    {
        self.write_index_snapshot_if_needed(anchor, guard, false)
            .await?
            .ok_or_else(|| v3_repository_error(V3FormatError::InvalidRecoveryHistory))
    }

    pub(super) async fn maybe_publish_recovery_expiry_checkpoint_coordinated<A>(
        &self,
        mutation: V3CoordinatedMutation<'_, A>,
    ) -> Result<Option<V3StoredCommit>>
    where
        A: V3CommitAnchor,
    {
        self.validate_coordinator_lease(mutation.lease)?;
        ...
        A: V3CommitAnchor,
    {
        ...
        let base_anchor = anchor
            ...
            .await
            .map_err(v3_repository_error)?
            .ok_or_else(|| v3_repository_error(V3FormatError::MissingAnchor))?;
        if self
            .accepted
            .read()
        ...
        if only_expiry {
            ...
            let Some(policy) = self.commit_store.options().recovery_policy else {
                return Err(v3_repository_error(V3FormatError::InvalidRecoveryPolicy));
            };
            if !history
        ...
        let (covered_generation, expected_live_object_count, expected_runs, receipts) = {
            ...
            (
                ...
                u64::try_from(accepted.repository.list_entries.len())
                    .map_err(|_| v3_repository_error(V3FormatError::IndexRootLimitExceeded))?,
                accepted.runs.clone(),
        ...
        let mut prepared = self
            ...
            .await?;
        let temporary_anchor = V3MemoryAnchor::with_state(base_anchor.clone());
        ...
        let uploaded = self
            ...
            .write_prepared_child_commit_with(&temporary_anchor, &prepared.plan, |commit_key| {
                ...
                self.append_recovery_section(
                    V3CommitWrite::snapshot(vec![V3CommitSection::new(
        ...
        let recovery = self
            ...
            .await?;
        if anchor.read_v3().await.map_err(v3_repository_error)? != Some(base_anchor.clone()) {
            return Err(v3_repository_error(V3FormatError::StaleAnchor));
        }
    ...
    fn verify_exact_index_root(&self, chain: &V3ReplayChain, expected: &V3IndexRoot) -> Result<()> {
        let [commit] = chain.commits_newest_first.as_slice() else {
            return Err(v3_repository_error(V3FormatError::InvalidIndexRoot));
        };
        let mut roots = commit
            ...
            .iter()
            .enumerate()
            .filter(|(_, section)| section.section_type == V3SectionType::IndexRoot);
    ...
    /// Loads and replays the commit chain selected by the v3 anchor.
    pub async fn load_chain_from_anchor<A>(&self, anchor: &A) -> Result<Option<V3ReplayChain>>
    where
        A: V3CommitAnchor,
    {
        let _mutation_lease = self.claim_direct_mutation()?;
    ...
    async fn load_chain_from_anchor_inner<A>(&self, anchor: &A) -> Result<Option<V3ReplayChain>>
    where
        A: V3CommitAnchor,
    {
        let _guard = self.mutation_lock.lock().await;
        let _publication_guard = self.publication_lock.write().await;
        let Some(anchor_state) = anchor.read_v3().await.map_err(v3_repository_error)? else {
            return Ok(None);
        };
        let chain = self
            .commit_store
        ...
        if anchor.read_v3().await.map_err(v3_repository_error)? != Some(anchor_state.clone()) {
            return Err(v3_repository_error(V3FormatError::StaleAnchor));
        }
    ...
    async fn replay_bounded_chain_to_state_and_runs(
        &self,
        chain: &V3ReplayChain,
    ) -> Result<(RepositoryState, Vec<V3IndexRootRunRef>)> {
    ...
    /// Writes an object and returns after the covering v3 commit is accepted.
    ...
    ) -> Result<V3StandalonePayloadUpload>
    where
        St: Stream<Item = Result<Bytes>> + Unpin + Send,
    {
        ...
        St: Stream<Item = Result<Bytes>> + Unpin + Send,
    {
        ...
        let object_id = generate_v3_standalone_object_id().map_err(v3_repository_error)?;
        let inflight = self
            .commit_store
        ...
        let stored = self
            .commit_store
            .write_standalone_streaming_payload(V3StandalonePayloadWrite {
                object_id,
    ...
    pub(super) async fn publish_standalone_streaming_known_len_coordinated<A>(
        ...
        key: LogicalPath,
        plaintext_len: u64,
        upload: V3StandalonePayloadUpload,
        options: RepositoryPutOptions,
    ) -> Result<RepositoryObjectMetadata>
    where
        A: V3CommitAnchor,
        ...
        A: V3CommitAnchor,
    {
        ...
        let carrier = Arc::new(V3StandaloneStreamCarrierReference {
    ...
    async fn publish_staged_standalone_locked<A>(
        &self,
        anchor: &A,
        base_anchor: &super::repository::V3AnchorState,
        staged: &StagedV3Put,
    ...
    where
        A: V3CommitAnchor,
    {
        validate_v3_standalone_object(&carrier.object_id, carrier.stored_len)
            .map_err(v3_repository_error)?;
        ...
        for delta in pending.deltas_mut() {
            ...
            entry.payload_ref = Some(PayloadReference::V3StandaloneStream {
        ...
        let uploaded = self
            ...
            .write_prepared_child_commit_with(&temporary_anchor, &prepared.plan, |commit_key| {
                let packed = self
                    ...
                    .ok_or(V3FormatError::InvalidHeaderField)?;
                accepted_run = Some(packed.run);
                self.append_recovery_section(
                    V3CommitWrite::delta(packed.sections)
        ...
        let accepted_run = accepted_run
            ...
            .ok_or_else(|| v3_repository_error(V3FormatError::InvalidHeaderField))?;
        ...
        St: Stream<Item = Result<Bytes>> + Unpin + Send,
    {
        ...
        let Some(first) = first else {
            if expected_plaintext_len.is_some_and(|length| length != 0) {
                return Err(v3_repository_error(V3FormatError::ObjectLengthMismatch));
            }
            let _guard = self.mutation_lock.lock().await;
            let _publication_guard = self.publication_lock.write().await;
            self.ensure_accepted_anchor_matches(anchor).await?;
        ...
        let upload = self
            .upload_standalone_streaming(
                ...
                &options,
                multipart_part_size,
                Arc::new(V3StandaloneUploadCancellation::new()),
            )
            .await?;
    ...
    pub(super) fn capture_copy_source(
        ...
        if source.entry.content_len != 0
            && !matches!(
                ...
                Some(PayloadReference::V3Pack { .. } | PayloadReference::V3StandaloneStream { .. })
            )
        {
            return Err(v3_repository_error(V3FormatError::InvalidHeaderField));
        }
    ...
    pub(super) async fn stage_copy_coordinated<A: V3CommitAnchor>(
        &self,
        mutation: V3CoordinatedMutation<'_, A>,
        source: V3ResolvedObject,
        destination: LogicalPath,
        guard: Option<&dyn super::V3MaintenanceGuard>,
    ) -> Result<(RepositoryObjectMetadata, V3StagedPutRollback)> {
        self.validate_coordinator_lease(mutation.lease)?;
        ...
        if needs_protection {
            let guard = guard
                .ok_or_else(|| v3_repository_error(V3FormatError::MaintenanceAccessRequired))?;
        ...
        Ok((staged.metadata, V3StagedPutRollback { checkpoint }))
    }

    /// Stages an object write without publishing the covering v3 commit.
    ///
    /// This is used by the v3 commit coordinator. Callers must publish the
    ...
    fn stage_put_metadata_sync_with_rollback(
    ...
    ) -> Result<(StagedV3Put, PendingV3Checkpoint)> {
        if options
            .expected_md5
        ...
        let payload = match payload {
            StagedPayload::Buffered(body) => Some(PendingV3Payload {
                manifest_id: manifest_id.clone(),
                body,
            }),
    ...
    pub async fn get_resolved_range(
        ...
        if let PayloadReference::V3Pack { carrier, record } = payload_ref {
            return self
    ...
    async fn read_pack_range_from_commit(
        ...
        {
            return Err(v3_repository_error(V3FormatError::SectionBounds));
        }
        let pack_stored_len = u32::try_from(pack.length)
            .map_err(|_| v3_repository_error(V3FormatError::InvalidPayloadPack))?;
        let facts = V3PayloadPackFacts::new(
            V3PayloadPackId::from_bytes(pack.pack_id),
            pack.attempt_id,
        ...
        .map_err(v3_repository_error)?;
        let record = V3PayloadPackRecordRef::new(pack.record_ordinal, pack.record_offset);
        let context = packed::repository_context_from_refs(
            &self.commit_store.options().repository_id,
        ...
        let span = plan_v3_payload_pack_record_range(&facts, &record, pack.content_len, requested)
            .map_err(v3_repository_error)?;
        ...
        if let Some(segments) = self.repository.cached_decrypted_segment_span(
            ...
            return open_v3_payload_pack_cached_record_span(
                &facts,
                &record,
                pack.content_len,
                &span,
            ...
            .read_commit_range_at(
                ...
                ByteRange::Slice {
                    offset: pack
                        ...
                        .ok_or_else(|| v3_repository_error(V3FormatError::SectionBounds))?,
                    len: span.stored_len,
                },
            )
            .await
            .map_err(v3_repository_error)?;
        let record_context = V3PayloadPackRecordContext::new(
            &context,
            &pack.commit_key,
        ...
        .map_err(v3_repository_error)?;
        let opened = open_v3_payload_pack_record_span_with_segments(
            keyring,
            &record_context,
            &span,
            &ciphertext,
    ...
    async fn read_stream_payload_range(
        &self,
        keyring: &KeyRing,
        payload: V3StandalonePayloadRead,
        range: ByteRange,
        cache_key: V3PayloadSectionCacheKey,
    ) -> Result<Bytes> {
        ...
        if range == ByteRange::Full {
            ...
            if digest_v3_section(&body) != payload.stored_digest() {
                return Err(RepositoryError::InvalidObjectFormat {
                    object_id: payload.payload_id().clone(),
                });
            }
        ...
        let keyring_envelope_ref = V3KeyringEnvelopeRef {
        ...
        let payload_cache_ref = V3StreamPayloadCacheIdentity {
    ...
    async fn validated_stream_payload_start(
        ...
        validate_v3_standalone_object(&payload.object_id, payload.stored_len)
            .map_err(v3_repository_error)?;
        if self.commit_store.provider_profile()
            == super::provider::V3ProviderProfile::RetainedVersionObjectLock
    ...
    async fn read_stream_payload(&self, payload: &V3StandalonePayloadRead) -> Result<Bytes> {
        let body = self
            .commit_store
            .store()
            .get_range_at(
            ...
            .await
            .map_err(|_| v3_repository_error(V3FormatError::StorageOperationFailed))?;
        if u64::try_from(body.len()).ok() != Some(payload.stored_len) {
            return Err(v3_repository_error(V3FormatError::TruncatedBody));
        }
        Ok(body)
    }
    ...
    fn stage_delete(&self, key: &LogicalPath) -> Result<PendingV3Checkpoint> {
        let keyring = self.repository.keyring();
        let lookup_blind_keys = keyring.derive_blind_index_keys_for_lookup(key)?;
        let accepted = self
            .accepted
    ...
    pub(crate) fn rollback_staged_puts(&self, rollbacks: Vec<V3StagedPutRollback>) -> Result<()> {
        #[cfg(test)]
    ...
    fn rollback_state_mutations(&self, rollbacks: Vec<PendingV3Checkpoint>) -> Result<()> {
        let mut pending = self
            .pending
            .lock()
            .map_err(|_| RepositoryError::StatePoisoned)?;
    ...
    pub(crate) fn fill_accepted_run_catalog_for_tests(&self) -> Result<()> {
        self.resize_accepted_run_catalog_for_tests(V3_INDEX_ROOT_MAX_RUNS)
    }
    ...
    fn publication_error(&self, error: V3FormatError) -> RepositoryError {
        if error == V3FormatError::AnchorReconciliationRequired {
            self.mark_local_recovery_required();
            RepositoryError::AcceptedRecoveryRequired
        } else {
            v3_repository_error(error)
        }
    }

    pub(crate) fn freeze_pending_publication(&self) -> Result<V3PendingPublication> {
        let snapshot = self
            .pending
            .lock()
            .map_err(|_| RepositoryError::StatePoisoned)?
            .freeze()?;
        Ok(V3PendingPublication { snapshot })
    }

    pub(crate) async fn publish_frozen_pending_index_delta<A>(
        &self,
        anchor: &A,
        publication: V3PendingPublication,
        guard: Option<&dyn V3MaintenanceGuard>,
    ) -> Result<Option<V3StoredCommit>>
    where
        A: V3CommitAnchor,
        ...
        A: V3CommitAnchor,
    {
        ...
        let uploaded = self
            ...
            .write_prepared_child_commit_with(&temporary_anchor, &prepared.plan, |commit_key| {
                let packed = self
                    .pending_packed_sections_for_commit(commit_key, &pending)
                    .map_err(|_| V3FormatError::InvalidHeaderField)?
                    .ok_or(V3FormatError::ObjectTooLarge)?;
        ...
        if let Some(run) = accepted_run.as_ref() {
            let accepted = self
                ...
                .read()
                .map_err(|_| RepositoryError::StatePoisoned)?;
            if accepted.runs.len() >= V3_INDEX_ROOT_MAX_RUNS {
                return Err(v3_repository_error(V3FormatError::IndexRootLimitExceeded));
            }
            if accepted
                .runs
                .last()
            ...
            {
                return Err(v3_repository_error(V3FormatError::InvalidIndexRun));
            }
        }
    ...
    fn accepted_run_ref(
        &self,
        run: packed::PendingV3IndexRunFacts,
        ...
        V3IndexRootRunRef {
            ...
            location: V3EmbeddedIndexRunLocation {
    ...
    fn validate_accepted_run_append(&self, run: &V3IndexRootRunRef) -> Result<()> {
        let accepted = self
            .accepted
            .read()
            .map_err(|_| RepositoryError::StatePoisoned)?;
    ...
    async fn ensure_accepted_anchor_matches<A>(
        &self,
        anchor: &A,
    ) -> Result<super::repository::V3AnchorState>
    where
        A: V3CommitAnchor,
    {
        let current = anchor
    ...
    fn pending_snapshot(&self) -> Result<PendingV3Snapshot> {
        self.pending
            .lock()
            .map_err(|_| RepositoryError::StatePoisoned)
    ...
    fn prepare_pending_install(
        &self,
        pending: &PendingV3Snapshot,
        sequence: Sequence,
        run: Option<V3IndexRootRunRef>,
    ) -> Result<PendingV3Install> {
        ...
        for delta in pending.deltas() {
            match delta {
                IndexDelta::Upsert { entry, .. } => {
                    ...
                    mutations.push(PendingV3InstallMutation::Upsert {
                        entry: Box::new((**entry).clone()),
                        manifest: (*manifest).clone(),
                    });
                }
                IndexDelta::Tombstone { blind_key, .. } => {
                    mutations.push(PendingV3InstallMutation::Tombstone {
        ...
        let completion_receipts = if let Some(receipt) = &pending.completion_receipt {
            ...
            receipts
                ...
                .map_err(|_| v3_repository_error(V3FormatError::InvalidIndexRun))?;
    ...
    fn install_pending_commit(
        &self,
        install: PendingV3Install,
        anchor: super::repository::V3AnchorState,
    ) -> Result<()> {
        #[cfg(test)]
        ...
        for mutation in install.mutations {
            match mutation {
                PendingV3InstallMutation::Upsert { entry, manifest } => {
                    accepted
                        .repository
                        .manifests
                ...
                PendingV3InstallMutation::Tombstone { blind_key } => {
    ...
    pub(crate) fn accepted_pack_carrier_counts_for_tests(&self) -> Result<(usize, usize)> {
        ...
        let mut unique = Vec::<Arc<rs3_index::V3PackCarrierReference>>::new();
        for entry in accepted.repository.namespace.live_entries() {
            let Some(PayloadReference::V3Pack { carrier, .. }) = entry.payload_ref.as_ref() else {
                continue;
            };
    ...
    fn cached_payload_section(&self, key: &V3PayloadSectionCacheKey) -> Result<Option<Bytes>> {
        let payload = self
            .payload_sections
            .read()
            .map_err(|_| RepositoryError::StatePoisoned)?
    ...
    fn cache_payload_section(&self, key: V3PayloadSectionCacheKey, payload: Bytes) -> Result<()> {
        let mut cache = self
            .payload_sections
            .write()
            .map_err(|_| RepositoryError::StatePoisoned)?;
    ...
    ) -> Result<Vec<V3IndexRootRunRef>> {
        let mut accepted_runs = Vec::new();
        for (index, section) in commit.parsed_header.header.section_index.iter().enumerate() {
            if section.flags & V3_SECTION_FLAG_COMPRESSED != 0 {
                return Err(v3_repository_error(V3FormatError::UnsupportedSection));
            }
            match section.section_type {
                ...
                _ => return Err(v3_repository_error(V3FormatError::UnsupportedSection)),
            }
        }
        Ok(accepted_runs)
    }
...
impl<S> V3Repository<S> {
    pub(super) fn claim_commit_coordinator(&self) -> Result<Arc<V3CoordinatorLease>> {
        self.ensure_local_state_ready()?;
        self.mutation_owner
            .compare_exchange(
                V3_MUTATION_OWNER_IDLE,
                V3_MUTATION_OWNER_COORDINATOR,
                Ordering::AcqRel,
                Ordering::Acquire,
            )
    ...
    fn claim_direct_mutation(&self) -> Result<V3DirectMutationLease> {
        self.ensure_local_state_ready()?;
        self.mutation_owner
            .compare_exchange(
                V3_MUTATION_OWNER_IDLE,
                V3_MUTATION_OWNER_DIRECT,
                Ordering::AcqRel,
                Ordering::Acquire,
            )
            .map_err(|_| RepositoryError::CommitFailed {
    ...
    fn validate_coordinator_lease(&self, lease: &V3CoordinatorLease) -> Result<()> {
        self.ensure_local_state_ready()?;
        if Arc::ptr_eq(&self.mutation_owner, &lease.owner)
            && self.mutation_owner.load(Ordering::Acquire) == V3_MUTATION_OWNER_COORDINATOR
        {
            return Ok(());
        }
...
fn payload_fill_lock_index(payload_id: &BackendObjectId, start_segment: usize) -> usize {
    ...
    (u64::from_be_bytes(prefix) % V3_PAYLOAD_FILL_LOCK_STRIPES as u64) as usize
}

#[derive(Clone, Debug)]
struct V3StandalonePayloadRead {
...
impl V3StandalonePayloadRead {
    ...
    fn section_cache_key(&self) -> V3PayloadSectionCacheKey {
        V3PayloadSectionCacheKey {
            carrier: V3PayloadSectionCarrierCacheKey::Standalone {
    ...
    fn cache_identity(
        &self,
        _payload_start: u64,
    ) -> Result<V3StreamPayloadCarrierCacheIdentity<'_>> {
        Ok(V3StreamPayloadCarrierCacheIdentity::Standalone {
...
fn stream_payload_read(
    ...
    content_len: u64,
) -> Option<V3StandalonePayloadRead> {
    match payload_ref {
        PayloadReference::V3StandaloneStream { carrier } => {
            let V3StandaloneStreamCarrierReference {
                object_id,
                version_id,
                object_digest,
                stored_len,
...
struct V3PayloadSectionCacheKey {
    carrier: V3PayloadSectionCarrierCacheKey,
...
#[derive(Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Hash)]
enum V3PayloadSectionCarrierCacheKey {
    Standalone {
        object_id: BackendObjectId,
        version_id: Option<BackendVersionId>,
        digest: [u8; 32],
...
#[derive(Debug)]
struct V3PayloadSectionCache {
    sections: LruCache<V3PayloadSectionCacheKey, Bytes>,
    max_entries: usize,
    max_bytes: u64,
    current_bytes: u64,
}

impl V3PayloadSectionCache {
    ...
    fn peek(&self, key: &V3PayloadSectionCacheKey) -> Option<Bytes> {
        self.sections.peek_cloned(key)
    }

    fn touch(&mut self, key: &V3PayloadSectionCacheKey) {
        self.sections.touch(key);
    }

    fn insert(&mut self, key: V3PayloadSectionCacheKey, payload: Bytes) {
...
fn replay_section_bytes(commit: &V3ReplayCommit, section_index: usize) -> Result<&[u8]> {
    commit
        .retained_sections
        .get(section_index)
...
fn v3_repository_error(error: V3FormatError) -> RepositoryError {
    match error {
        V3FormatError::ContentMd5Mismatch => return RepositoryError::ContentMd5Mismatch,
        V3FormatError::ObjectTooLarge => return RepositoryError::ObjectTooLarge,
        V3FormatError::ObjectLengthMismatch => {
            return RepositoryError::ObjectLengthMismatch;
        }
        V3FormatError::ObjectBodyReadFailed => {
...
#[cfg(test)]
mod tests {
    use crate::v3::service::{V3CommitPackRead, pack_payload_cache_ref};
    ...
    fn sample_pack_read() -> V3CommitPackRead {
        V3CommitPackRead {
