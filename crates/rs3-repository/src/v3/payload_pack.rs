...
pub const V3_PAYLOAD_PACK_MAX_RECORDS: usize = rs3_index::run::INDEX_PACK_MAX_RECORDS as usize;
/// Maximum complete pack bytes accepted by the bounded in-memory codec.
pub const V3_PAYLOAD_PACK_MAX_BYTES: usize = 32 * 1024 * 1024;
/// Canonical independently authenticated plaintext segment size.
pub const V3_PAYLOAD_PACK_SEGMENT_BYTES: usize = rs3_types::PAYLOAD_PACK_SEGMENT_BYTES;
/// Random pack identifier bytes.
pub const V3_PAYLOAD_PACK_ID_LEN: usize = 32;
...
pub struct V3PayloadPackRecordInput {
...
pub struct V3PayloadPackId([u8; V3_PAYLOAD_PACK_ID_LEN]);

impl V3PayloadPackId {
    /// Constructs an identifier from its exact bytes.
    #[must_use]
    pub const fn from_bytes(bytes: [u8; V3_PAYLOAD_PACK_ID_LEN]) -> Self {
        Self(bytes)
    }

    /// Returns the exact identifier bytes for encrypted index serialization.
    #[must_use]
    pub const fn as_bytes(&self) -> &[u8; V3_PAYLOAD_PACK_ID_LEN] {
        &self.0
    }

    /// Consumes the identifier and returns its exact bytes.
    #[must_use]
    pub const fn into_bytes(self) -> [u8; V3_PAYLOAD_PACK_ID_LEN] {
        self.0
    }
}
...
#[derive(Clone, PartialEq, Eq)]
pub struct V3PayloadPackFacts {
    pack_id: V3PayloadPackId,
...
impl V3PayloadPackFacts {
    ...
    pub fn new(
        ...
        if content_key_id.as_str().is_empty()
            ...
            || stored_len == 0
            || stored_len as usize > V3_PAYLOAD_PACK_MAX_BYTES
            || record_count == 0
            || record_count as usize > V3_PAYLOAD_PACK_MAX_RECORDS
        {
            return Err(V3FormatError::InvalidPayloadPack);
        }
        Ok(Self {
            pack_id,
            attempt_id,
    ...
    #[must_use]
    pub const fn pack_id(&self) -> V3PayloadPackId {
        self.pack_id
    }
...
#[derive(Clone, PartialEq, Eq)]
pub struct V3PayloadPackRecordRef {
    record_ordinal: u32,
    physical_offset: u32,
}
...
#[derive(Clone, PartialEq, Eq)]
pub struct V3PayloadPackRecord {
    reference: V3PayloadPackRecordRef,
    plaintext_len: u64,
}
...
impl V3PayloadPackRecord {
    /// Returns the compact record reference stored alongside an index upsert.
    #[must_use]
    pub const fn reference(&self) -> &V3PayloadPackRecordRef {
        &self.reference
    }
    ...
    pub fn stored_len(&self) -> V3Result<u32> {
        record_stored_len(self.plaintext_len)
    }
}

impl std::ops::Deref for V3PayloadPackRecord {
    type Target = V3PayloadPackRecordRef;
...
#[derive(Clone, PartialEq, Eq)]
pub struct V3PayloadPackLayout {
    facts: V3PayloadPackFacts,
    records: Vec<V3PayloadPackRecord>,
}
...
impl V3PayloadPackLayout {
    /// Validates a complete logical and physical pack layout.
    pub fn new(facts: V3PayloadPackFacts, records: Vec<V3PayloadPackRecord>) -> V3Result<Self> {
        ...
        for (ordinal, record) in records.iter().enumerate() {
            ...
            validate_v3_payload_pack_record_ref(&facts, &record.reference, record.plaintext_len)?;
        }
    ...
    #[must_use]
    pub const fn facts(&self) -> &V3PayloadPackFacts {
        &self.facts
    }

    /// Returns record references in logical ordinal order.
    #[must_use]
    pub fn records(&self) -> &[V3PayloadPackRecord] {
        &self.records
    }

    /// Returns one logical record reference by ordinal.
    #[must_use]
    pub fn record(&self, ordinal: u32) -> Option<&V3PayloadPackRecord> {
...
#[derive(Clone, PartialEq, Eq)]
pub struct V3SealedPayloadPack {
    layout: V3PayloadPackLayout,
    bytes: Bytes,
}
...
impl V3SealedPayloadPack {
    /// Returns the facts needed to construct encrypted index pointers.
    #[must_use]
    pub const fn layout(&self) -> &V3PayloadPackLayout {
        &self.layout
    }
...
pub struct V3PayloadPackRecordSpan {
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct V3OpenedPayloadPackRecordSpan {
...
#[derive(Clone, Copy)]
pub struct V3PayloadPackRecordContext<'a> {
    object: PayloadPackContext<'a>,
    facts: &'a V3PayloadPackFacts,
    record: &'a V3PayloadPackRecordRef,
    plaintext_len: u64,
}
...
impl<'a> V3PayloadPackRecordContext<'a> {
    ...
    pub fn new(
        ...
        validate_v3_payload_pack_record_ref(facts, record, plaintext_len)?;
        Ok(Self {
...
pub fn seal_v3_payload_pack(
    keyring: &KeyRing,
    repository_context: &[u8],
    containing_object: &BackendObjectId,
    section_ordinal: u32,
    records: &[V3PayloadPackRecordInput],
) -> V3Result<V3SealedPayloadPack> {
    let pack_id =
        rs3_crypto::random_carrier_id().map_err(|_| V3FormatError::RandomnessUnavailable)?;
    let order = random_physical_order(records.len())?;
    seal_v3_payload_pack_with_layout(
        keyring,
        repository_context,
        containing_object,
        section_ordinal,
        records,
        V3PayloadPackId::from_bytes(pack_id),
        &order,
    )
}

fn seal_v3_payload_pack_with_layout(
    keyring: &KeyRing,
    repository_context: &[u8],
    containing_object: &BackendObjectId,
    section_ordinal: u32,
    inputs: &[V3PayloadPackRecordInput],
    pack_id: V3PayloadPackId,
    physical_order: &[usize],
) -> V3Result<V3SealedPayloadPack> {
    validate_public_context(repository_context, containing_object)?;
    if inputs.is_empty() || inputs.len() > V3_PAYLOAD_PACK_MAX_RECORDS {
        return Err(V3FormatError::PayloadPackLimitExceeded);
    }
    ...
    for (ordinal, input) in inputs.iter().enumerate() {
        let plaintext_len = u64::try_from(input.plaintext.len())
            .map_err(|_| V3FormatError::PayloadPackLimitExceeded)?;
        record_stored_len(plaintext_len)?;
        records.push(V3PayloadPackRecord {
            reference: V3PayloadPackRecordRef::new(
                u32::try_from(ordinal).map_err(|_| V3FormatError::PayloadPackLimitExceeded)?,
                0,
            ),
            plaintext_len,
        });
    ...
    for logical_ordinal in physical_order {
        let record = records
            ...
            .ok_or(V3FormatError::InvalidPayloadPack)?;
        record.reference.physical_offset = stored_len;
        stored_len = stored_len
            .checked_add(record.stored_len()?)
            .ok_or(V3FormatError::PayloadPackLimitExceeded)?;
    }
    if stored_len as usize > V3_PAYLOAD_PACK_MAX_BYTES {
        return Err(V3FormatError::PayloadPackLimitExceeded);
    }
    ...
    let facts = V3PayloadPackFacts::new(
        ...
        content_key_id,
        stored_len,
        u32::try_from(records.len()).map_err(|_| V3FormatError::PayloadPackLimitExceeded)?,
    )?;
    let layout = V3PayloadPackLayout::new(facts, records)?;
...
/// coverage is enforced only by [`V3PayloadPackLayout::new`].
pub fn validate_v3_payload_pack_record_ref(
    facts: &V3PayloadPackFacts,
    record: &V3PayloadPackRecordRef,
    plaintext_len: u64,
) -> V3Result<()> {
...
pub fn plan_v3_payload_pack_record_range(
    facts: &V3PayloadPackFacts,
    record: &V3PayloadPackRecordRef,
    plaintext_len: u64,
    requested: Range<u64>,
) -> V3Result<V3PayloadPackRecordSpan> {
    validate_v3_payload_pack_record_ref(facts, record, plaintext_len)?;
    ...
    let segment_size = V3_PAYLOAD_PACK_SEGMENT_BYTES as u64;
    ...
    Ok(V3PayloadPackRecordSpan {
        offset,
        stored_len,
        start_segment: u32::try_from(start_segment)
            .map_err(|_| V3FormatError::InvalidPayloadPack)?,
...
pub fn open_v3_payload_pack_record_span(
    keyring: &KeyRing,
    context: &V3PayloadPackRecordContext<'_>,
    span: &V3PayloadPackRecordSpan,
    ciphertext_span: &[u8],
) -> V3Result<Bytes> {
    open_v3_payload_pack_record_span_with_segments(keyring, context, span, ciphertext_span)
        .map(|opened| opened.plaintext)
}

/// Authenticates an exact span while retaining complete segments for caching.
pub fn open_v3_payload_pack_record_span_with_segments(
    keyring: &KeyRing,
    context: &V3PayloadPackRecordContext<'_>,
    span: &V3PayloadPackRecordSpan,
    ciphertext_span: &[u8],
) -> V3Result<V3OpenedPayloadPackRecordSpan> {
    let expected = plan_v3_payload_pack_record_range(
        context.facts,
        context.record,
    ...
    for relative_segment in 0..selected_count {
        ...
        segments.push((
            u32::try_from(segment_ordinal).map_err(|_| V3FormatError::InvalidPayloadPack)?,
...
pub fn open_v3_payload_pack_cached_record_span(
    facts: &V3PayloadPackFacts,
    record: &V3PayloadPackRecordRef,
    plaintext_len: u64,
    span: &V3PayloadPackRecordSpan,
    ...
    let expected =
        plan_v3_payload_pack_record_range(facts, record, plaintext_len, span.requested.clone())?;
    ...
    for (relative_segment, plaintext) in plaintext_segments.iter().enumerate() {
        let segment_ordinal = start_segment
            .checked_add(
                u64::try_from(relative_segment).map_err(|_| V3FormatError::InvalidPayloadPack)?,
            )
            .ok_or(V3FormatError::InvalidPayloadPack)?;
...
pub fn open_v3_payload_pack_record_range(
    keyring: &KeyRing,
    context: &V3PayloadPackRecordContext<'_>,
    pack: &[u8],
    requested: Range<u64>,
    ...
    let span = plan_v3_payload_pack_record_range(
        context.facts,
        context.record,
        context.plaintext_len,
        requested,
    )?;
    let start = usize::try_from(span.offset).map_err(|_| V3FormatError::InvalidPayloadPack)?;
    let end = span
        .offset
        .checked_add(span.stored_len)
        .and_then(|end| usize::try_from(end).ok())
        .ok_or(V3FormatError::InvalidPayloadPack)?;
    let ciphertext = pack.get(start..end).ok_or(V3FormatError::TruncatedBody)?;
    open_v3_payload_pack_record_span(keyring, context, &span, ciphertext)
}

/// Opens one complete logical record from complete ciphertext-only pack bytes.
pub fn open_v3_payload_pack_record(
    keyring: &KeyRing,
    context: &V3PayloadPackRecordContext<'_>,
    pack: &[u8],
) -> V3Result<Bytes> {
    open_v3_payload_pack_record_range(keyring, context, pack, 0..context.plaintext_len)
}
...
fn finish_opened_span(
    ...
    let selected_start = u64::from(span.start_segment)
        .checked_mul(V3_PAYLOAD_PACK_SEGMENT_BYTES as u64)
        .ok_or(V3FormatError::InvalidPayloadPack)?;
    let trim_start = span
        .requested
        .start
    ...
    let requested = selected_plaintext
        ...
        .ok_or(V3FormatError::InvalidPayloadPack)?;
    Ok(V3OpenedPayloadPackRecordSpan {
...
fn seal_record_into(
    ...
    for segment_ordinal in 0..count {
        ...
        {
            return Err(V3FormatError::CryptoOperation);
        }
...
fn random_physical_order(record_count: usize) -> V3Result<Vec<usize>> {
    if record_count == 0 || record_count > V3_PAYLOAD_PACK_MAX_RECORDS {
        return Err(V3FormatError::PayloadPackLimitExceeded);
    }
    let mut keyed = Vec::with_capacity(record_count);
    for ordinal in 0..record_count {
        let key = rs3_crypto::random_physical_order_key()
            .map_err(|_| V3FormatError::RandomnessUnavailable)?;
...
fn validate_physical_order(order: &[usize], count: usize) -> V3Result<()> {
    if order.len() != count {
        return Err(V3FormatError::InvalidPayloadPack);
    }
...
fn segment_count(plaintext_len: u64) -> V3Result<u64> {
    if plaintext_len == 0 {
        return Err(V3FormatError::InvalidPayloadPack);
    }
    plaintext_len
        .checked_add(V3_PAYLOAD_PACK_SEGMENT_BYTES as u64 - 1)
        .map(|rounded| rounded / V3_PAYLOAD_PACK_SEGMENT_BYTES as u64)
        .ok_or(V3FormatError::PayloadPackLimitExceeded)
}

fn record_stored_len(plaintext_len: u64) -> V3Result<u32> {
    let stored_len = plaintext_len
        ...
        .ok_or(V3FormatError::PayloadPackLimitExceeded)?;
    let stored_len =
        u32::try_from(stored_len).map_err(|_| V3FormatError::PayloadPackLimitExceeded)?;
    if stored_len as usize > V3_PAYLOAD_PACK_MAX_BYTES {
        return Err(V3FormatError::PayloadPackLimitExceeded);
    }
    Ok(stored_len)
...
fn segment_plaintext_len(plaintext_len: u64, segment_ordinal: u64) -> V3Result<u64> {
    let count = segment_count(plaintext_len)?;
    ...
    Ok(plaintext_len
        ...
        .min(V3_PAYLOAD_PACK_SEGMENT_BYTES as u64))
}

fn segment_ciphertext_offset(segment_ordinal: u64) -> V3Result<u64> {
    segment_ordinal
        .checked_mul(V3_PAYLOAD_PACK_SEGMENT_BYTES as u64 + AEAD_TAG_LEN)
        .ok_or(V3FormatError::InvalidPayloadPack)
}
...
mod tests {
    ...
    fn must_v3<T>(result: V3Result<T>) -> T {
        match result {
            Ok(value) => value,
    ...
    fn sample_pack() -> V3SealedPayloadPack {
        must_v3(seal_v3_payload_pack_with_layout(
            &keyring(),
            REPOSITORY_CONTEXT,
            &object_id("commits/opaque-commit"),
            SECTION_ORDINAL,
            &[
                V3PayloadPackRecordInput {
            ...
            V3PayloadPackId::from_bytes([9_u8; V3_PAYLOAD_PACK_ID_LEN]),
            &[1, 0],
        ))
    }

    fn open_record(pack: &V3SealedPayloadPack, ordinal: u32) -> V3Result<Bytes> {
        let containing_object = object_id("commits/opaque-commit");
        let record = pack
            .layout()
            .record(ordinal)
            .ok_or(V3FormatError::InvalidPayloadPack)?;
        let context = V3PayloadPackRecordContext::new(
            REPOSITORY_CONTEXT,
            &containing_object,
            SECTION_ORDINAL,
            pack.layout().facts(),
        ...
        open_v3_payload_pack_record(&keyring(), &context, pack.bytes())
    }
    ...
    fn pack_is_ciphertext_only_and_round_trips_shuffled_records() {
        ...
        assert_eq!(
            must_v3(open_record(&pack, 0)),
            Bytes::from_static(b"abcdefghijk")
        );
        assert_eq!(
            must_v3(open_record(&pack, 1)),
    ...
    fn production_seal_attempts_use_fresh_pack_identities() {
        let records = [V3PayloadPackRecordInput {
            plaintext: Bytes::from_static(b"same logical record"),
        }];
        let containing_object = object_id("commits/opaque-commit");
        let first = must_v3(seal_v3_payload_pack(
            &keyring(),
            REPOSITORY_CONTEXT,
            &containing_object,
            SECTION_ORDINAL,
        ...
        let second = must_v3(seal_v3_payload_pack(
            &keyring(),
            REPOSITORY_CONTEXT,
            &containing_object,
            SECTION_ORDINAL,
        ...
        assert_eq!(must_v3(open_record(&first, 0)), records[0].plaintext);
        assert_eq!(must_v3(open_record(&second, 0)), records[0].plaintext);
    }

    #[test]
    fn segment_reorder_and_cross_record_transplant_fail_closed() {
        let plaintext_len = V3_PAYLOAD_PACK_SEGMENT_BYTES * 2;
        let records = [
            V3PayloadPackRecordInput {
        ...
        let pack = must_v3(seal_v3_payload_pack_with_layout(
            &keyring(),
            REPOSITORY_CONTEXT,
            &object_id("commits/opaque-commit"),
            SECTION_ORDINAL,
            &records,
            V3PayloadPackId::from_bytes([0x31; V3_PAYLOAD_PACK_ID_LEN]),
            &[0, 1],
        ));
        let segment_stored_len = V3_PAYLOAD_PACK_SEGMENT_BYTES + AEAD_TAG_LEN as usize;
        ...
        let containing_object = object_id("commits/opaque-commit");
        let context = must_v3(V3PayloadPackRecordContext::new(
            REPOSITORY_CONTEXT,
            &containing_object,
            SECTION_ORDINAL,
            pack.layout().facts(),
        ...
        assert!(open_v3_payload_pack_record(&keyring(), &context, &reordered).is_err());
        ...
        assert!(open_v3_payload_pack_record(&keyring(), &context, &transplanted).is_err());
    }

    #[test]
    fn range_plan_fetches_only_intersecting_canonical_segments_and_caches_them() {
        let plaintext = Bytes::from(
            (0..(V3_PAYLOAD_PACK_SEGMENT_BYTES * 2 + 32))
        ...
        let pack = must_v3(seal_v3_payload_pack_with_layout(
            ...
            V3PayloadPackId::from_bytes([13_u8; V3_PAYLOAD_PACK_ID_LEN]),
            &[0],
        ));
        let facts = pack.layout().facts();
        let record = pack.layout().record(0).expect("record exists");
        let requested =
            (V3_PAYLOAD_PACK_SEGMENT_BYTES as u64 - 8)..(V3_PAYLOAD_PACK_SEGMENT_BYTES as u64 + 8);
        let span = must_v3(plan_v3_payload_pack_record_range(
            facts,
            record,
            record.plaintext_len(),
            requested.clone(),
        ...
        assert_eq!(
            span.stored_len,
            (V3_PAYLOAD_PACK_SEGMENT_BYTES * 2) as u64 + AEAD_TAG_LEN * 2
        );
        ...
        let opened = must_v3(open_v3_payload_pack_record_span_with_segments(
            &keyring(),
            &context,
            &span,
        ...
        let cached = opened
            ...
            .collect::<Vec<_>>();
        assert_eq!(
            must_v3(open_v3_payload_pack_cached_record_span(
                facts,
                record,
                record.plaintext_len(),
                &span,
        ...
        assert!(
            open_v3_payload_pack_cached_record_span(
                facts,
                record,
                record.plaintext_len(),
                &span,
    ...
    fn empty_records_require_index_only_values() {
        let result = seal_v3_payload_pack_with_layout(
            &keyring(),
            REPOSITORY_CONTEXT,
            &object_id("commits/opaque-commit"),
            SECTION_ORDINAL,
            ...
            V3PayloadPackId::from_bytes([3_u8; V3_PAYLOAD_PACK_ID_LEN]),
            &[0],
        );
        assert!(matches!(result, Err(V3FormatError::InvalidPayloadPack)));
    }
    ...
    fn context_and_every_authenticated_index_fact_fail_closed() {
        ...
        let record = pack.layout().record(0).expect("record exists");
        let containing_object = object_id("commits/opaque-commit");
        let open = |facts: &V3PayloadPackFacts, record: &V3PayloadPackRecordRef| {
            let context = V3PayloadPackRecordContext::new(
                REPOSITORY_CONTEXT,
                &containing_object,
                SECTION_ORDINAL,
        ...
        let open_with = |repository_context: &[u8],
            ...
            open_v3_payload_pack_record(&keyring(), &context, bytes)
        };
        ...
        bad_facts.pack_id = V3PayloadPackId::from_bytes([8_u8; V3_PAYLOAD_PACK_ID_LEN]);
    ...
    fn every_ciphertext_truncation_fails_closed() {
        ...
        for length in 0..ciphertext.len() {
            assert!(
                open_v3_payload_pack_record_span(
                    &keyring(),
                    &context,
                    &span,
                    &ciphertext[..length],
        ...
        for length in 0..pack.bytes().len() {
            assert!(
                open_v3_payload_pack_record(&keyring(), &context, &pack.bytes()[..length]).is_err()
            );
        }
    }
    ...
    fn sixty_four_small_values_have_only_record_ciphertext_overhead() {
        let records = (0..64)
            .map(|ordinal| V3PayloadPackRecordInput {
        ...
        let pack = must_v3(seal_v3_payload_pack_with_layout(
            ...
            SECTION_ORDINAL,
            &records,
            V3PayloadPackId::from_bytes([11_u8; V3_PAYLOAD_PACK_ID_LEN]),
            &order,
        ));
    ...
    fn bulk_pack_accepts_and_opens_the_bounded_record_ceiling() {
        assert_eq!(V3_PAYLOAD_PACK_MAX_RECORDS, 4_096);
        let keyring = keyring();
        let containing_object = object_id("commits/opaque-commit");
        let records = (0..V3_PAYLOAD_PACK_MAX_RECORDS)
            .map(|ordinal| V3PayloadPackRecordInput {
                plaintext: Bytes::from(vec![ordinal as u8]),
            })
            .collect::<Vec<_>>();
        let pack = must_v3(seal_v3_payload_pack(
            &keyring,
            REPOSITORY_CONTEXT,
            &containing_object,
            SECTION_ORDINAL,
        ...
        assert_eq!(
            pack.bytes().len(),
            V3_PAYLOAD_PACK_MAX_RECORDS * (1 + AEAD_TAG_LEN as usize)
        );
        assert_eq!(pack.layout().records().len(), V3_PAYLOAD_PACK_MAX_RECORDS);
        for ordinal in [
            0,
            V3_PAYLOAD_PACK_MAX_RECORDS / 2,
            V3_PAYLOAD_PACK_MAX_RECORDS - 1,
        ] {
            ...
            assert_eq!(
                must_v3(open_v3_payload_pack_record(
                    &keyring,
                    &context,
                    pack.bytes()
                )),
    ...
    fn record_references_validate_bounds_without_requiring_full_layout() {
        ...
        assert!(validate_v3_payload_pack_record_ref(facts, &live, live.plaintext_len()).is_ok());

        let mut invalid = live.reference().clone();
        invalid.record_ordinal = facts.record_count();
        assert_eq!(
            validate_v3_payload_pack_record_ref(facts, &invalid, live.plaintext_len()),
            Err(V3FormatError::InvalidPayloadPack)
        );
        ...
        let partial = V3PayloadPackLayout::new(facts.clone(), vec![live]);
        assert_eq!(partial, Err(V3FormatError::InvalidPayloadPack));
    }
    ...
    fn invalid_ranges_record_counts_and_sizes_are_rejected() {
        ...
        let record = pack.layout().record(0).expect("record exists");
        assert!(
            plan_v3_payload_pack_record_range(facts, record, record.plaintext_len(), 4..4).is_err()
        );
        assert!(
            plan_v3_payload_pack_record_range(facts, record, record.plaintext_len(), 0..12)
                .is_err()
        );
        assert!(
            seal_v3_payload_pack_with_layout(
                ...
                SECTION_ORDINAL,
                &[],
                V3PayloadPackId::from_bytes([0_u8; V3_PAYLOAD_PACK_ID_LEN]),
                &[],
            )
            .is_err()
        );
        ...
        assert!(record_stored_len(V3_PAYLOAD_PACK_MAX_BYTES as u64).is_err());
    }
    ...
    fn writer_rejects_content_key_ids_the_reader_cannot_encode() {
        ...
        let result = seal_v3_payload_pack(
            &keyring,
            REPOSITORY_CONTEXT,
            &object_id("commits/opaque-commit"),
            SECTION_ORDINAL,
        ...
        assert_eq!(result, Err(V3FormatError::PayloadPackLimitExceeded));
    }

    #[test]
    fn debug_output_redacts_plaintext_and_pack_identity() {
        let input = V3PayloadPackRecordInput {
        ...
        assert!(!debug.contains(&format!("{:?}", [9_u8; V3_PAYLOAD_PACK_ID_LEN])));
