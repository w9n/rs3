...
async fn recovery_reachability_deduplicates_overlapping_points_and_keeps_deleted_payload() {
    ...
    let payload = must_v2(anchor.read_v2().await).expect("payload anchor");
    ...
    let deadline = must_v2(RecoveryPolicy::PRESET.promised_until_ms(1_003));
    assert_eq!(
    ...
    assert_eq!(
        graph.expiry_due,
        Some(must_v2(RecoveryPolicy::PRESET.promised_until_ms(1_001)) + 60_000)
    );
}
...
async fn recovery_reachability_expiry_does_not_reactivate_an_old_root_registry() {
    ...
    let plan = must_v2(writer.prepare_child_publication(&parent.anchor_state).await);
    let mut captured = must_v2(
    ...
    must_v2(
        writer
            .write_prepared_child_commit_with(&anchor, &plan, |key| {
                ...
                let recovery = crate::v2::recovery::section::seal(
                    &keys,
                    &context,
                    &key.object_id,
                    1,
...
async fn recovery_reachability_streams_more_than_4096_points_beyond_startup_chain_limit() {
    ...
    let writer = V2CommitStore::new(store, keys.clone(), options.clone());
    ...
    let empty = must_v2(crate::v2::V2IndexRoot::new(
        Sequence::ZERO,
        0,
        options.format_ref.clone(),
        options.keyring_envelope_ref.clone(),
    ...
    let deadline = must_v2(policy.promised_until_ms(10_000));
    let first = must_v2(writer.write_genesis_snapshot(&anchor).await);
    ...
    for _ in 0..4096 {
        let base = must_v2(anchor.read_v2().await).expect("accepted checkpoint");
        let plan = must_v2(writer.prepare_child_publication(&base).await);
        let stored = must_v2(
            writer
                .write_prepared_child_commit_with(&anchor, &plan, |key| {
                    let bytes =
                        crate::v2::seal_v2_index_root(&keys, &context, &key.object_id, 0, &empty)?;
    ...
    assert!(
        graph.accounted_metadata_bytes < V2MaintenanceBudgets::default().max_history_metadata_bytes
    );
    assert!(
        graph.pending_section_bytes < V2MaintenanceBudgets::default().max_history_pending_bytes
    );
    println!(
