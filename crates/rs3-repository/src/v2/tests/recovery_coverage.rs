...
async fn retained_fixture(
    ...
    TrackedStore,
    Arc<V2Repository<TrackedStore>>,
    V2MemoryAnchor,
    V2CommitCoordinator<TrackedStore, V2MemoryAnchor>,
) {
...
async fn faulted_fixture(
    ...
    V2MemoryAnchor,
    V2CommitCoordinator<FaultInjectingBlobStore<MemoryBlobStore>, V2MemoryAnchor>,
) {
    let memory = MemoryBlobStore::new();
    let options = commit_store_options_with_maintenance_roots(
        &memory,
...
async fn retained_packed_successors_reuse_coverage_and_restart_rebuilds_it() {
    ...
    let genesis = must_v2(anchor.read_v2().await).expect("genesis anchor");
    let first = must_repo(
        coordinator
            .put_committed(
                key(0),
    ...
    drop(repository);
    let restarted = Arc::new(V2Repository::new(
        store.clone(),
        keyring,
        RepositoryOptions::default(),
        options,
    ...
    let restarted_coordinator = must_repo(V2CommitCoordinator::with_options(
        Arc::clone(&restarted),
        anchor,
        CommitCoordinatorOptions::new(1, Duration::ZERO),
    ))
...
async fn retained_coverage_exact_head_or_renewal_failure_blocks_successor_anchor() {
    for operation in [
        ...
        let before = must_v2(anchor.read_v2().await).expect("genesis anchor");
        store
...
async fn publish_time(repository: &V2Repository<TrackedStore>, anchor: &V2AnchorState) -> i64 {
    must_v2(
        repository
            .commit_store()
...
async fn retained_overwrite_delete_preserve_old_packed_bytes_across_policy_reduction() {
    ...
    let reduced_policy = must_v2(RecoveryPolicy::new(1, 86_400, 60_000));
    drop(coordinator);
    ...
    let deleted = must_v2(anchor.read_v2().await).expect("delete anchor");
    let replacement_floor =
        must_v2(reduced_policy.coverage_until_ms(publish_time(&restarted, &deleted).await));
