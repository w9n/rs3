...
fn history_options(policy: Option<RecoveryPolicy>) -> V2CommitStoreOptions {
...
async fn recovery_publication_preserves_fixed_predecessor_policy_across_restart_reduction_and_root_cut()
 {
    ...
    let first = must_v2(anchor.read_v2().await).expect("accepted");
    let chain = must_v2(
        repository
            .commit_store()
    ...
    let history =
        must_v2(repository.commit_store().replay_recovery_history(&chain)).expect("history");
    ...
    assert_eq!(
        ...
        must_v2(old_policy.promised_until_ms(1_001))
    );
    assert_eq!(must_repo(repository.head(&key)).etag, metadata.etag);

    let reduced = must_v2(RecoveryPolicy::new(1, 86_400, 60_000));
    let restarted = V2Repository::new(
        store.clone(),
        keys.clone(),
        RepositoryOptions::default(),
    ...
    let history =
        must_v2(restarted.commit_store().replay_recovery_history(&chain)).expect("history");
    ...
    assert_eq!(
        ...
        must_v2(old_policy.promised_until_ms(2_000))
    );
    ...
    let accepted = must_v2(anchor.read_v2().await).expect("accepted");
    let chain = must_v2(
        cold.commit_store()
            .load_replay_chain_from_state(&accepted)
            .await,
    );
    let history = must_v2(cold.commit_store().replay_recovery_history(&chain)).expect("history");
...
async fn recovery_publication_refuses_missing_section_and_cannot_disable_accepted_history() {
    let store = MemoryBlobStore::new();
    let keys = must_crypto(KeyRing::generate_random());
    let native = V2Repository::new(
        store.clone(),
        keys.clone(),
        RepositoryOptions::default(),
        history_options(None),
    );
    let native_anchor = V2MemoryAnchor::new();
    must_repo(native.write_genesis_snapshot(&native_anchor).await);
    let requires = V2Repository::new(
        store.clone(),
        keys.clone(),
        RepositoryOptions::default(),
    ...
    let historical = V2Repository::new(
        store.clone(),
        keys,
        RepositoryOptions::default(),
        history_options(None),
    ...
    let before = must_v2(anchor.read_v2().await);
    ...
    assert_eq!(must_v2(anchor.read_v2().await), before);
...
async fn recovery_publication_raw_child_apis_fail_before_put_for_configured_and_loaded_history() {
    ...
    let puts = store.operation_counts().expect("counts").put;
    let native = V2CommitStore::new(store.clone(), keys, history_options(None));
    for candidate in [&writer, &native] {
        let write = V2CommitWrite::delta(vec![V2CommitSection::new(
        ...
        assert_eq!(
            candidate
                ...
                .await
                .err(),
            Some(V2FormatError::InvalidRecoveryHistory),
        );
        let mut built = false;
        assert_eq!(
            candidate
...
async fn recovery_publication_delayed_candidate_cannot_shorten_supersession_promise() {
    ...
    let writer = V2CommitStore::new(store.clone(), keys.clone(), options.clone());
    ...
    let previous = must_v2(writer.replay_recovery_history(&chain)).expect("history");
    let plan = must_v2(
        writer
            .prepare_child_publication(&genesis.anchor_state)
            .await,
    );
    let captured = must_v2(
        crate::v2::recovery::publication::CapturedRecoveryPublication::new(
            plan.clone(),
            previous,
    ...
    let root = must_v2(crate::v2::V2IndexRoot::new(
        Sequence::new(0),
        0,
    ...
    let candidate = must_v2(
        writer
            .write_prepared_child_commit_with(&temporary, &plan, |key| {
                let index =
                    crate::v2::seal_v2_index_root(&keys, &context, &key.object_id, 0, &root)?;
                let history = crate::v2::recovery::section::seal(
                    &keys,
                    &context,
                    &key.object_id,
                    1,
    ...
    let no_policy = V2CommitStore::new(store.clone(), keys.clone(), history_options(None));
    assert_eq!(
        no_policy
            .adopt_unanchored_child(
                &anchor,
...
async fn recovery_page_reads_only_exact_recovery_span_and_rejects_changed_claims() {
    ...
    let page = RecoveryPage {
        points: vec![RecoveryPoint {
            ...
            protected_until_ms: must_v2(policy.promised_until_ms(parent.publish_time_ms)),
    ...
    let section = RecoverySection {
        ...
        delta: RecoveryDelta {
            register: Some(RecoveryPoint {
                ...
                protected_until_ms: must_v2(policy.promised_until_ms(1_002)),
                policy_id: policy.identity(),
            }),
    ...
    let parent_anchor = must_v2(anchor.read_v2().await).expect("parent");
    let plan = must_v2(writer.prepare_child_publication(&parent_anchor).await);
    let carrier = must_v2(
        writer
    ...
    let reference = RecoveryPageRef {
        ...
        claims: must_v2(page.claims()),
    };
    let before = store.operation_log().expect("log").len();
    assert_eq!(must_v2(writer.read_recovery_page(&reference).await), page);
...
async fn recovery_publication_extends_detached_payload_without_changing_logical_retention_or_rewalking_base()
 {
    ...
    let current = must_v2(repository.commit_store().read_anchor_head(&anchor).await).expect("head");
    let required = must_v2(policy.promised_until_ms(current.parsed_header.header.publish_time_ms));
    let payloads = memory
...
async fn recovery_publication_quiet_expiry_writes_once_only_when_a_known_promise_expires() {
    let store = MemoryBlobStore::new();
    let policy = must_v2(RecoveryPolicy::new(1, 120, 60_000));
    let repository = V2Repository::new(
        store.clone(),
        must_crypto(KeyRing::generate_random()),
        RepositoryOptions::default(),
    ...
    let before = must_v2(anchor.read_v2().await).expect("anchor");
    let lease = must_repo(repository.claim_commit_coordinator());
    let mutation = || crate::v2::service::V2CoordinatedMutation::new(&lease, &anchor);
    assert!(
        must_repo(
            repository
    ...
    let due = must_v2(policy.promised_until_ms(1_001)) + i64::from(policy.clock_uncertainty_ms());
    repository
        .commit_store()
    ...
    assert_eq!(store.operation_counts().expect("counts").put, puts);
    assert_eq!(must_v2(anchor.read_v2().await), Some(expired.anchor_state));
}
