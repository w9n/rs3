use super::*;
use rs3_index::{NamespaceEntry, PayloadReference, V2PackCarrierReference, V2PackRecordReference};
...
async fn fixture(
    ...
    V2CommitStore<CopyProtectionStore>,
    V2MemoryAnchor,
    NamespaceEntry,
) {
    ...
    let entry = NamespaceEntry {
        ...
        payload_ref: Some(PayloadReference::V2Pack {
...
async fn retained_copy_extends_only_three_exact_dependencies_to_fixed_copy_deadline() {
    ...
    let base = must_v2(anchor.read_v2().await).expect("anchor");
...
async fn retained_copy_rejects_lost_fence_stale_anchor_and_missing_exact_version_before_mutation() {
    ...
    if let Some(PayloadReference::V2Pack { carrier, .. }) = &mut source.payload_ref {
...
async fn retained_copy_refuses_missing_roots_key_material_and_deadline_overflow_without_io() {
    ...
    for case in 0..4 {
        ...
        match case {
            ...
            2 => {
                if let Some(PayloadReference::V2Pack { carrier, .. }) = &mut captured.payload_ref {
                    Arc::make_mut(carrier).content_key_id = key_id("missing-content-key");
                }
            }
            _ => modified = i64::MAX,
        }
        let invalid = V2CommitStore::new(
