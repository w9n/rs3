...
async fn fixture(fault: DeadlineFault) -> (V2CommitStore<DeadlineStore>, V2MemoryAnchor) {
    ...
    must_v2(setup.write_genesis_snapshot(&anchor).await);
    ...
    (V2CommitStore::new(store, keyring, options), anchor)
}
...
async fn native_snapshot(
    repository: &V2CommitStore<DeadlineStore>,
    anchor: &V2MemoryAnchor,
    floor: i64,
    hold: Option<LegalHoldStatus>,
) -> super::super::V2Result<super::super::V2StoredCommit> {
    ...
    let root = super::super::V2IndexRoot::new(
        Sequence::ZERO,
        0,
    ...
    repository
        .write_child_commit_with(anchor, |key| {
            let sealed = super::super::seal_v2_index_root(
                repository.keyring(),
                &context,
                &key.object_id,
                0,
...
async fn native_recovery_physical_floor_exceeds_logical_days_and_returns_verified_exact_observation()
 {
    ...
    let stored = must_v2(native_snapshot(&repository, &anchor, floor, None).await);
    ...
    assert_eq!(must_v2(anchor.read_v2().await), Some(stored.anchor_state));
}
