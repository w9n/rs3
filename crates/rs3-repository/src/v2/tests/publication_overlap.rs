...
struct Fixture {
    ...
    repository: Arc<V2Repository<SlowCommitGetStore>>,
    anchor: V2MemoryAnchor,
    coordinator: Arc<V2CommitCoordinator<SlowCommitGetStore, V2MemoryAnchor>>,
}

impl Fixture {
    async fn new() -> Self {
        Self::with_profile(V2ProviderProfile::Dev).await
    }

    async fn with_profile(profile: V2ProviderProfile) -> Self {
    ...
    fn put(
        ...
        name: &str,
        bytes: &'static [u8],
    ) -> tokio::task::JoinHandle<crate::Result<super::super::V2CommittedPut>> {
    ...
    async fn reopened(&self, anchor: V2AnchorState) -> V2Repository<SlowCommitGetStore> {
        ...
        must_repo(
            fresh
                .load_chain_from_anchor(&V2MemoryAnchor::with_state(anchor))
                .await,
        );
        fresh
    }
...
async fn canceled_waiter_does_not_cancel_owned_prefix_publication() {
    let f = Fixture::new().await;
    let before = must_v2(f.anchor.read_v2().await).expect("base anchor");
...
async fn accepted_prefix_install_failure_poison_rejects_unpublished_successor() {
    ...
    let fresh = f
        .reopened(must_v2(f.anchor.read_v2().await).expect("accepted prefix"))
        .await;
    assert_eq!(
        must_repo(fresh.get_range(&key("shared"), ByteRange::Full).await),
        Bytes::from_static(b"first")
...
impl V2CommitAnchor for LostAdvanceReply {
...
async fn incompatible_protection_waits_for_frozen_prefix() {
    let f = Fixture::with_profile(V2ProviderProfile::RetainedVersionObjectLock).await;
    let first = f.pause(false).await;
...
async fn canceled_immediate_publisher_keeps_ownership_until_resolution() {
    ...
    drop(coordinator);
    assert!(V2CommitCoordinator::new(Arc::clone(&repository), anchor.clone()).is_err());
    store.commit_put_pause.release.notify_one();
