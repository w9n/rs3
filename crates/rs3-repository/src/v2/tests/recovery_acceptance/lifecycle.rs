...
async fn apply(fixture: &Fixture, reclaim: bool) -> crate::v2::V2FullGcApplyReport {
    must_v2(
        fixture
            .repository
            .commit_store()
...
async fn carrier(fixture: &Fixture, anchor: &V2AnchorState) -> BlobMetadata {
    fixture
        .store
...
async fn recovery_acceptance_old_current_data_gets_full_supersession_window_and_decoded_restore() {
    ...
    let reduced = must_v2(RecoveryPolicy::new(1, 120, 60_000));
    ...
    assert_eq!(
        ...
        must_v2(policy.promised_until_ms(superseded_at))
    );
    assert!(
        carrier(&fixture, &original)
            .await
