...
pub(super) fn choose_publish_time(now_ms: i64, parent_ms: Option<i64>) -> V2Result<i64> {
    let next = match parent_ms {
        Some(parent) if parent >= 0 => parent
            .checked_add(1)
            .ok_or(V2FormatError::InvalidPublicationTime)?,
        Some(_) => return Err(V2FormatError::InvalidPublicationTime),
        None => 0,
    };
...
pub(super) fn validate_parent_time(parent_ms: Option<i64>, child_ms: i64) -> V2Result<()> {
...
pub(super) fn validate_publication_time(now_ms: i64, chosen_ms: i64) -> V2Result<()> {
    let maximum = now_ms
