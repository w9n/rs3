...
use crate::v2::{
    V2AnchorState, V2CommitKey, V2CommitParentRef, V2FormatError, V2FormatRef, V2Result, cbor,
};
...
pub(in crate::v2) const MAX_RECOVERY_TAIL_RECORDS: usize = 4_096;
pub(in crate::v2) const MAX_RECOVERY_PAGES: usize = 1_024;
pub(in crate::v2) const MAX_RECOVERY_PAGE_RECORDS: usize = 4_096;
...
pub(in crate::v2) struct RecoveryPoint {
...
pub(in crate::v2) struct RecoveryDelta {
...
pub(in crate::v2) struct RecoveryPage {
    pub points: Vec<RecoveryPoint>,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(in crate::v2) struct RecoveryPageClaims {
...
#[derive(Clone, PartialEq, Eq)]
pub(in crate::v2) enum RecoveryPageLocation {
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub(in crate::v2) struct RecoveryPageRef {
...
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub(in crate::v2) struct RecoverySnapshot {
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub(in crate::v2) struct RecoverySection {
...
impl RecoveryPage {
    pub fn claims(&self) -> V2Result<RecoveryPageClaims> {
...
impl RecoveryPageRef {
    ...
    pub fn verify_page(&self, page: &RecoveryPage) -> V2Result<()> {
...
impl RecoverySnapshot {
    pub fn validate_normalized(&self) -> V2Result<()> {
...
impl RecoverySection {
    pub fn encode(&self) -> V2Result<Vec<u8>> {
        self.validate()?;
    ...
    pub fn decode(bytes: &[u8]) -> V2Result<Self> {
    ...
    pub fn validate_for_commit(
        &self,
        parent: Option<(&V2AnchorState, i64, &RecoveryPolicy)>,
        publish_time_ms: i64,
        is_root: bool,
        safe_expiry_ms: i64,
    ) -> V2Result<()> {
        let parent_ref = parent.map(|(anchor, _, _)| V2CommitParentRef {
    ...
    pub fn apply_delta(
        ...
        enclosing: &V2AnchorState,
        section_ordinal: u32,
    ) -> V2Result<RecoverySnapshot> {
        self.validate()?;
    ...
    pub fn normalized_snapshot(
        ...
        enclosing: &V2AnchorState,
        section_ordinal: u32,
    ) -> V2Result<Option<RecoverySnapshot>> {
        self.validate()?;
        self.snapshot
            .as_ref()
    ...
    pub fn page(&self, page_index: u32) -> V2Result<&RecoveryPage> {
        self.local_pages
...
fn validate_point(point: &RecoveryPoint) -> V2Result<()> {
...
fn validate_points(points: &[RecoveryPoint], maximum: usize) -> V2Result<()> {
...
fn validate_snapshot(snapshot: &RecoverySnapshot) -> V2Result<()> {
...
fn validate_claims(claims: &RecoveryPageClaims) -> V2Result<()> {
...
fn validate_anchor(anchor: &V2AnchorState) -> V2Result<()> {
    let key = V2CommitKey::parse(&anchor.commit_key)
    ...
    let digest = hex::decode(&format.digest).map_err(|_| V2FormatError::InvalidRecoveryHistory)?;
...
fn validate_ordinal(ordinal: u32) -> V2Result<()> {
    if ordinal as usize >= crate::v2::V2_MAX_COMMIT_SECTIONS {
        return Err(V2FormatError::InvalidRecoveryHistory);
    }
    Ok(())
}

fn bounded_text(text: &str, maximum: usize) -> V2Result<()> {
    if text.is_empty() {
        return Err(V2FormatError::InvalidRecoveryHistory);
    }
...
fn capacity(length: usize, maximum: usize) -> V2Result<()> {
    if length > maximum {
...
fn read<T>(value: Result<T, rs3_types::cbor::CborError>) -> V2Result<T> {
    value.map_err(|_| V2FormatError::InvalidRecoveryHistory)
}

fn fixed_array(reader: &mut cbor::Reader<'_>, expected: usize) -> V2Result<()> {
...
fn bounded_array(reader: &mut cbor::Reader<'_>, maximum: usize) -> V2Result<usize> {
    let count = read(reader.read_array_len())?;
    capacity(count, maximum)?;
    Ok(count)
}

fn read_u32(reader: &mut cbor::Reader<'_>) -> V2Result<u32> {
    u32::try_from(read(reader.read_u64())?).map_err(|_| V2FormatError::InvalidRecoveryHistory)
}

fn read_digest(reader: &mut cbor::Reader<'_>) -> V2Result<[u8; 32]> {
...
impl Writer {
    fn room(&self, additional: usize) -> V2Result<()> {
        capacity(
            self.bytes
                .len()
    ...
    fn u64(&mut self, value: u64) -> V2Result<()> {
        self.room(cbor_head_len(value))?;
        cbor::write_u64(&mut self.bytes, value);
        Ok(())
    }
    fn i64(&mut self, value: i64) -> V2Result<()> {
    ...
    fn text(&mut self, value: &str) -> V2Result<()> {
        self.room(
            value
                .len()
                .saturating_add(cbor_head_len(value.len() as u64)),
...
fn encode_anchor(writer: &mut Writer, anchor: &V2AnchorState) -> V2Result<()> {
    writer.array(6)?;
...
fn decode_anchor(reader: &mut cbor::Reader<'_>) -> V2Result<V2AnchorState> {
...
fn encode_point(writer: &mut Writer, point: &RecoveryPoint) -> V2Result<()> {
    writer.array(4)?;
...
fn decode_point(reader: &mut cbor::Reader<'_>) -> V2Result<RecoveryPoint> {
    fixed_array(reader, 4)?;
...
fn encode_points(writer: &mut Writer, points: &[RecoveryPoint]) -> V2Result<()> {
...
fn decode_points(reader: &mut cbor::Reader<'_>, maximum: usize) -> V2Result<Vec<RecoveryPoint>> {
...
fn encode_delta(writer: &mut Writer, delta: &RecoveryDelta) -> V2Result<()> {
    writer.array(3)?;
...
fn decode_delta(reader: &mut cbor::Reader<'_>) -> V2Result<RecoveryDelta> {
...
fn encode_claims(writer: &mut Writer, claims: RecoveryPageClaims) -> V2Result<()> {
    writer.array(5)?;
...
fn decode_claims(reader: &mut cbor::Reader<'_>) -> V2Result<RecoveryPageClaims> {
...
fn encode_page_ref(writer: &mut Writer, reference: &RecoveryPageRef) -> V2Result<()> {
    writer.array(2)?;
...
fn decode_page_ref(reader: &mut cbor::Reader<'_>) -> V2Result<RecoveryPageRef> {
...
fn encode_snapshot(writer: &mut Writer, snapshot: &RecoverySnapshot) -> V2Result<()> {
    writer.array(3)?;
...
fn decode_snapshot(reader: &mut cbor::Reader<'_>) -> V2Result<RecoverySnapshot> {
    fixed_array(reader, 3)?;
...
mod tests {
    ...
    fn parent_ref(anchor: &V2AnchorState) -> V2CommitParentRef {
        V2CommitParentRef {
            sequence: anchor.sequence,
            commit_key: anchor.commit_key.clone(),
            body_digest: anchor.body_digest,
        ...
        assert!(
            section
                .apply_delta(
                    &previous,
                    &accepted,
                    crate::v2::V2_MAX_COMMIT_SECTIONS as u32
                )
                .is_err()
        );
