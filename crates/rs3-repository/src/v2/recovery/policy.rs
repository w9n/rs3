...
impl RecoveryPolicy {
    ...
    pub fn promised_until_ms(self, superseded_at_ms: i64) -> V2Result<i64> {
        checked_deadline(
            superseded_at_ms,
    ...
    pub fn coverage_until_ms(self, superseded_at_ms: i64) -> V2Result<i64> {
        checked_deadline(
    ...
    pub fn expiry_cutoff_ms(self, trusted_now_ms: i64) -> V2Result<i64> {
        if trusted_now_ms < 0 {
...
fn checked_deadline(base_ms: i64, offset_ms: i64) -> V2Result<i64> {
    base_ms
...
pub(crate) fn physical_retention_for_deadline(
    ...
    now_ms: i64,
) -> V2Result<Option<RetentionPolicy>> {
    ...
    let days = u32::try_from(remaining_ms / DAY_MS + i64::from(remaining_ms % DAY_MS != 0))
        .map_err(|_| V2FormatError::InvalidRecoveryPolicy)?;
    ...
    fn policy_and_time_bounds_fail_closed() {
        for values in [
            ...
            assert_eq!(
                ...
                Err(V2FormatError::InvalidRecoveryPolicy)
            );
        }
