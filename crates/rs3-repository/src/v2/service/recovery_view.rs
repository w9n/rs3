...
use super::*;
use crate::v2::V2AnchorState;
use crate::v2::recovery::history::RecoveryPoint;
...
pub struct V2RecoveryPointInfo {
...
pub struct V2RecoveryView<S> {
    reader: V2Repository<S>,
    selected: V2AnchorState,
...
fn registry_binding(anchor: &V2AnchorState) -> Result<[u8; 32]> {
    let bytes =
        serde_json::to_vec(anchor).map_err(|_| unavailable("invalid recovery authority"))?;
    Ok(digest_v2_section(&bytes))
}
...
fn point_info(point: &RecoveryPoint) -> V2RecoveryPointInfo {
    V2RecoveryPointInfo {
...
impl<S: BlobStore> V2CommitStore<S> {
    async fn operator_registry<A: V2CommitAnchor>(&self, anchor: &A) -> Result<Registry> {
        let state = anchor
            .read_v2()
            .await
        ...
        let published_at_ms = chain
            ...
            .header
            .publish_time_ms;
        if anchor.read_v2().await.map_err(v2_repository_error)? != Some(state.clone()) {
...
impl<S: BlobStore> V2Repository<S> {
    ...
    pub async fn recovery_points<A: V2CommitAnchor>(
        &self,
        anchor: &A,
        limit: usize,
        cursor: Option<&V2RecoveryCursor>,
    ) -> Result<V2RecoveryPointPage> {
        ...
        let next = if next_offset < records.len() {
            ...
            points.push(V2RecoveryPointInfo {
        ...
        if anchor.read_v2().await.map_err(v2_repository_error)? != Some(registry.anchor.clone()) {
            return Err(v2_repository_error(V2FormatError::StaleAnchor));
        }
        Ok(V2RecoveryPointPage {
            registry_sequence: registry.anchor.sequence.get(),
            points,
            next_cursor: next.map(|(group, offset)| V2RecoveryCursor {
                sequence: registry.anchor.sequence.get(),
                digest: binding,
                group,
                offset,
...
impl<S: BlobStore + Clone> V2RecoveryView<S> {
    /// Exact root selected at construction, stable across live compaction.
    pub fn selected_anchor(&self) -> &V2AnchorState {
        &self.selected
    }
    ...
    pub async fn check_authority<A: V2CommitAnchor>(&self, anchor: &A) -> Result<()> {
        ...
        if selected != self.selected
            || anchor.read_v2().await.map_err(v2_repository_error)? != Some(registry.anchor.clone())
        {
            return Err(v2_repository_error(V2FormatError::StaleAnchor));
        }
        Ok(())
