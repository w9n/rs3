...
use super::super::repository::{V2CommitAnchor, V2ReplayChain};
use super::super::{
    ...
    V2MaintenanceGuard, V2SectionType,
};
    ...
    S: BlobStore + Clone,
{
    ...
    fn current_head_mixed_payload_summary(
        ...
        for commit in &chain.commits_newest_first {
            ...
            for (section_ordinal, section) in header.section_index.iter().enumerate() {
                if section.section_type == V2SectionType::PayloadPack {
    ...
    fn live_payload_refs_from_state(
        state: &crate::state::RepositoryState,
    ) -> Result<BTreeSet<V2LivePayloadPackRecordKey>> {
