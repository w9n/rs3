...
impl KeyRing {
    ...
    /// the active content key. It may be reused only for an exact retry with
    /// identical associated data and plaintext. Reusing it for different
    /// plaintext would repeat an XChaCha20-Poly1305 nonce and is forbidden.
    pub fn seal_payload_pack_segment(
        ...
        let key = self.primary_key(KeyPurpose::Content)?;
        let nonce = derive_payload_pack_segment_nonce(&key.secret, associated_data, nonce_context)?;
        let ciphertext = encrypt_payload_with_key(&key.secret, associated_data, plaintext, &nonce)?;
    ...
    /// Opens one payload-pack segment using exact authenticated layout facts.
    ///
    /// `nonce_context` must come from authenticated pack metadata. AEAD
    /// authentication binds each independently readable segment to that
    /// context and to its complete associated data.
...
mod tests {
    ...
    #[test]
    fn payload_pack_segment_vector_is_stable() {
        let sealed = match keyring(2).seal_payload_pack_segment(
            b"commit-and-record-aad",
            b"pack-7/record-3/segment-1",
            b"payload pack segment",
        ) {
        ...
        assert_eq!(
            ...
            "5439ae9baef155d97bdabb45775ae67351a9507187804ccd468983f1b8f4cab710a97ab7"
        );
    }
