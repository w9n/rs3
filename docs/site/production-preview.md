...
- if the configured backend prefix is empty, initialize exactly one repository
  using the supplied repository ID, the generated or pinned salt, and the
  wrapping-key source
...
Current v03 performance is reported in [Performance](performance.md#backend-comparison-september-8-2026):
...
- configure the owner-controlled signing identity, registry, artifact
  retention, SBOM, provenance, scanning, and digest-based publication
  path; and
