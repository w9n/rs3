...
| Most recent local v02 release gate | The complete gate passed on 2026-07-11 before foreground standalone payloads and wire v6. On 2026-07-12 the current candidate passed the exact standalone gate and direct Kopia gateway backup/restore. A fresh kind dynamic-PVC run installed RustFS, rs3, OpenEBS, and Velero; completed backup; verified the v2 anchor; and restarted the gateway successfully. The orchestration crossed the repository's five-minute command ceiling during post-restart work before a Restore object was submitted, so it was stopped and its cluster cleaned. The dynamic-PVC restore and Postgres lanes still require a permitted longer run. |
