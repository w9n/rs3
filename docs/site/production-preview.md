...
| Evidence | Result |
| --- | --- |
| Current v02 low-amplification scale lane | Three wire-v6 release runs on 2026-07-12 used 4,096-record batches with 1,000,000 unique 512 B objects. Elapsed was 43.850-43.963 s, recovery was 4.873-4.942 s, and total reload was 4.909-4.980 s. Every run recorded 246 PUTs, 253 GETs, 248 HEADs, 1.268292436x write amplification, 245 active runs, peak RSS of 1,681,272,832-1,681,723,392 B including the in-memory backend, and one exact `GET` at 1.03125x per cold sentinel. The dirty build identifier requires repetition at the exact committed candidate. |
| Current local fresh-process filesystem lane | Three wire-v6 ext4 1M runs used separate writer and reader processes. Writer elapsed was 46.673-46.824 s and RSS was 955,678,720-956,338,176 B; fresh-reader recovery was 5.280-5.350 s, verification was 5.316-5.391 s, and RSS was 1,010,085,888-1,010,372,608 B. Every run recovered the exact million entries and 245 runs at 1.268284240x write amplification. These repository-process values exclude an in-memory backend. The build was dirty, and this is not pinned-runner timing, an HTTP gateway measurement, or provider qualification. |
...
| Most recent local v02 release gate | Passed on 2026-07-11 with `just preview-gate-release` after the embedded framed stream-carrier changes. Direct Kopia gateway backup/restore passed. Fresh kind clusters completed Velero dynamic-PVC gateway-restart and Velero/Postgres backup/restore checks. This evidence predates foreground standalone payloads and wire v6; the current release recipe now includes the standalone concurrency/amplification gate and must be rerun. |
