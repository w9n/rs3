...
| Local v2 gateway perf baseline | Refreshed on 2026-05-16 with current commit-backed `v2-preview` gateway runs. Sequential writes emitted one backend commit PUT per client object, parallel writes batched to 8 PUTs for 64 small objects and 4 PUTs for 32 medium objects, and repeated reads of one object reused a verified payload-section cache after the first commit GET. |
