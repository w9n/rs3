...
| Current local fresh-process filesystem lane | Current-revision rerun required. The previous three controlled local-filesystem 1M runs predate the record-digest removal and namespace-key table, so they do not qualify the exact candidate. This lane reports repository-process RSS excluding an in-memory backend; it is not pinned-runner timing, an HTTP gateway measurement, or provider qualification. |
