...
| v02 recovery does not retain cumulative attacker-sized deltas | Descriptor-first recovery verifies exact bounded run sections sequentially from a signed catalog under count and byte ceilings. Runtime publication keeps one accepted state plus a hard-bounded 1,024-mutation overlay; compaction uses per-run scratch state and shared identifiers rather than a cumulative replay scratch or candidate namespace. The accepted state omits the legacy prefix-token projection and structurally shares ordered path keys and exact authenticated carrier facts without changing serialized or authentication inputs. | Three automatic-compaction 1M runs at revision `7023c65` passed at 1,953,574,912-1,953,808,384 B process peak RSS including the in-memory backend. Three local separate-process filesystem runs passed at 957,411,328-958,234,624 B writer RSS and 1,137,709,056-1,138,118,656 B reader RSS; pinned-runner and provider qualification remain. |
