...
| v02 GC retains every live payload without retaining unrelated payload ancestry | Effective run records mark exact payload-pack and streamed-carrier versions; catalog-named runs, the replay chain, format root, keyring envelope, and protected roots are marked before exact-version sweep. The strongest represented finite retention is propagated across required restore dependencies. Bounded inventory and one immutable plan renew and post-verify exact versions before guarded deletion. Existing graphs that require legal hold fail closed. | Implemented for embedded, standalone, compacted catalog, same-format protected-root, and restore-authority paths, including ambiguous renewal failure. Cross-format protected-root renewal, dependency-wide legal hold, mixed-pack cleaning, and retained-provider restart/fault qualification remain unavailable. |
