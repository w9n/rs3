...
| Old content remains readable after data-key rotation | Enabled historical content keys are accepted for reads. Rotation is not exposed for v02, and destructive maintenance rejects protected roots from a different exact format generation. | Repository key rotation tests; cross-format protected-root renewal remains a release blocker. |
...
| v02 GC retains every live payload without retaining unrelated payload ancestry | Effective run records mark exact payload-pack and streamed-carrier versions; catalog-named runs, the replay chain, format root, keyring envelope, and protected roots are marked before exact-version sweep. The strongest represented finite retention is propagated across required restore dependencies. One read-only store-boundary ledger meters every planning HEAD, bounded range GET, and LIST page before forwarding; the verified current chain and namespace state are reused instead of replayed twice. Bounded inventory and one immutable plan renew and post-verify exact versions before guarded deletion. Existing graphs that require legal hold fail closed. | Implemented for embedded, standalone, compacted catalog, same-format protected-root, and restore-authority paths, including actual-operation budget parity, zero-budget non-forwarding, invalid page rejection, and ambiguous renewal failure. S3 list XML is capped before deserialization and returned member counts are checked against requested `MaxKeys`. Cross-format protected-root renewal, dependency-wide legal hold, mixed-pack cleaning, and retained-provider restart/fault qualification remain unavailable. |
