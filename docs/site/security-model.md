...
| v02 recovery does not retain cumulative attacker-sized deltas | Descriptor-first recovery verifies exact bounded run sections sequentially from a signed catalog under count and byte ceilings. Runtime publication keeps one accepted state plus a hard-bounded 1,024-mutation overlay; compaction uses per-run scratch state and shared identifiers rather than a cumulative replay scratch or candidate namespace. The accepted v02 state does not build the legacy prefix-token projection because v02 listing uses a separate trusted path-ordered projection. | Embedded and compacted recovery tests pass. Three automatic-compaction 1M runs passed at 2,197,200,896-2,197,594,112 B process peak RSS, including the in-memory backend; the pinned fresh-process filesystem lane remains. |
...
- Packed-run catalog compaction and automatic pre-ceiling backpressure pass the
  in-memory 1M lane, but not retained-provider restart or the full adversarial
