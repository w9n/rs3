...
| `v03_index_run/{external-pack,standalone,completion}.bin` | Wire 9 container tables, namespace/listing projections and front-coded paths. Namespace upserts carry an optional bounded canonical checksum; listing projections do not duplicate it. `frozen_container_tables_preserve_canonical_wire_bytes` checks exact encoding, decoding and all truncations. The inline run vector remains an additional readable hex assertion. |
| `v03_index_root/{root,completion}.bin` | Canonical root plaintext, wire generation 5, including a bounded completion-receipt snapshot. Receipts use eight canonical CBOR fields and can preserve an optional checksum. `frozen_root_plaintext_preserves_canonical_bytes` and `canonical_logical_encoding_is_stable` pin bytes and SHA-256. Encrypted frame/context tests exercise sealing separately. |
