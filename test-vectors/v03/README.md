...
| `v03_index_run/{external-pack,standalone,completion}.bin` | Wire 10 container tables, namespace/listing projections and front-coded paths. Namespace upserts carry a mandatory fixed-width MD5 ETag and an optional bounded canonical checksum; listing projections do not duplicate either. `frozen_container_tables_preserve_canonical_wire_bytes` checks exact encoding, decoding and all truncations. The inline run vector remains an additional readable hex assertion. |
| `v03_index_root/{root,completion}.bin` | Canonical root plaintext, wire generation 6, including a bounded completion-receipt snapshot. Receipts use eight canonical CBOR fields with a fixed-width multipart MD5 ETag and can preserve an optional checksum. `frozen_root_plaintext_preserves_canonical_bytes` and `canonical_logical_encoding_is_stable` pin bytes and SHA-256. Encrypted frame/context tests exercise sealing separately. |
