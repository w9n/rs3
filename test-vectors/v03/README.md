...
| `v03_index_run/{external-pack,standalone}.bin` | Wire 7 container tables, namespace/listing projections and front-coded paths. `frozen_container_tables_preserve_canonical_wire_bytes` checks exact encoding, decoding and all truncations. The inline run vector remains an additional readable hex assertion. |
| `v03_index_root/root.bin` | Canonical root plaintext, wire generation 3. `frozen_root_plaintext_preserves_canonical_bytes` and `canonical_logical_encoding_is_stable` pin bytes and SHA-256. Encrypted frame/context tests exercise sealing separately. |
