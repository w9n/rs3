...
use rs3_repository::v3::V3RecoveryBundle;
...
fuzz_target!(|data: &[u8]| {
    ...
    let Ok(bundle) = V3RecoveryBundle::from_object_bytes(data) else {
        return;
    };
    let encoded = bundle
        .to_object_bytes()
        .unwrap_or_else(|error| panic!("parsed restore bundle failed to re-encode: {error}"));
    let decoded = V3RecoveryBundle::from_object_bytes(&encoded)
